...
| File | What to update |
|---|---|
| `CHANGELOG.md` | Add a new vX.Y.Z entry at the TOP, above the previous entry. Include the test-surface delta table + sections for Added / Fixed / etc. + Carry-over for vX.Y.Z+1. **This is the ONLY file that narrates the arc.** |
| `README.md` | Bump the **Version:** line in the header block. Refresh the test/skill count stats in the intro blockquote so they stay accurate (e.g. `~3,706 tests on every push` / `10 reusable Claude Skills`). **Do NOT add a "What's new" section** — those were stripped on 2026-05-05; CHANGELOG.md is canonical for change history. |
| `CLAUDE.md` | Bump the **Version:** line and refresh the inline stats on that same line (per-push test count + skill count). **Keep it a one-liner pointing at CHANGELOG.md** — do NOT inline an arc paragraph. |
...
See `CHANGELOG_ENTRY_TEMPLATE.md` for the canonical CHANGELOG entry shape and `TODO_HANDOFF_TEMPLATE.md` for the 🏁 block. (There is intentionally no README "What's new" template — the section was retired on 2026-05-05; CHANGELOG.md is the only place per-version arcs are narrated.)
...
README.md: vX.Y.(Z-1) -> vX.Y.Z + intro stat refresh (test count + skill
count). NO What's-new section.

CLAUDE.md: version-line refresh + inline stat refresh on the same line.
NO arc paragraph.
...
- README + CLAUDE.md only carry the **current** version pin and a stat refresh — they don't accumulate per-version arcs, so stacking releases doesn't bloat them. (The 22 stacked "What's new" sections that used to live in README were stripped on 2026-05-05 once we recognized they duplicated CHANGELOG.md.)
...
- **README + CLAUDE.md never get a "What's new" section or arc paragraph.** Just bump the version pin and refresh the inline stats. If you catch yourself writing prose about what shipped this release into either file, stop — that prose belongs in the CHANGELOG entry. (Rule established 2026-05-05 after stripping 22 stacked sections that had grown to ~45% of README.)
