...
- **2026-05-05 ~03:00 — `0a13386` — autonomous-loop design pattern: split logging from judgment.** The cron originally tried to do BOTH wave-cleanup observations AND skill creation in one prompt. Skill creation is high-judgment work — agents either over-skilled (every tiny one-off becomes a skill, bloating the surface) or under-skilled (real patterns get missed). Splitting the responsibilities — autonomous loop only LOGS observations to a known section; human-triggered review walks each entry and decides skill / standing-rule / archive / drop — preserves the autonomy benefit while keeping the judgment work where it belongs. **Generalizable pattern: any "automated curator" prompt should separate evidence-gathering (high-frequency, low-judgment, cron-friendly) from synthesis (low-frequency, high-judgment, human-in-loop).** The split lets the loop's prompt stay tight + machine-readable, and lets the review prompt assume human availability.
- **2026-05-05 ~03:30 — `e8cce09` — two valid "spec needs local backend" guard styles, both effective.** Surveying the 5 e2e specs that spawn child Node processes / touch backend filesystem (`backup-engine-api`, `migration-safety`, `recurring-invoice-api`, `retention-api`, `scheduled-email-api`, `wellness-ops-api`), I found two coexisting patterns: (a) **`IS_LOCAL_STACK` regex on `BASE_URL`** at describe-level — coarse, "is the test run cross-machine?" check (used by backup-engine-api + now migration-safety); (b) **`probePrismaClient()` / `dbAvailable()`** at spec-level — granular, "is the backend's Prisma client actually reachable?" check (used by the other 4). Both correctly cause e2e-full's runs against demo to skip these specs without false negatives. Pattern (a) is simpler when the spec is fundamentally local-only; pattern (b) is more flexible when the spec has SOME tests that work cross-machine and SOME that don't. **Don't refactor (b)→(a) for consistency** — they're not equivalent (a remote-but-Prisma-installed runner would be skipped by (a) but accepted by (b), which IS the right behavior for some test surfaces). Both patterns earn their keep.
- **2026-05-05 ~04:00 — `6d2a435` — JSX text content does NOT interpret JS escape sequences.** A literal `…` written between JSX tags (e.g. `<div>Loading…</div>`) renders as the six characters `\`, `u`, `2`, `0`, `2`, `6` — not as `…`. JS string escape rules only apply inside JS expression contexts (`{...}`); JSX text content is treated as XML text where `\u` has no special meaning. Use the actual unicode character (`…`) or an HTML entity (`&hellip;`) instead. This bit `PerLocationDashboard.jsx:79` (#430). **Probably worth a CLAUDE.md standing rule:** "JSX text != JS string — use real unicode chars or HTML entities, never `\\u`-escapes between JSX tags."
- **2026-05-05 ~04:00 — `n/a` — Edit tool normalizes `…`-style escapes when reading.** When I tried `Edit({old_string: 'comparison\\u2026', new_string: 'comparison…'})` to fix the #430 bug, the tool reported "old_string and new_string are exactly the same" — meaning it normalized the JS escape into the unicode char on read, so the byte-precise replacement appeared as a no-op. **Workaround:** drop to Bash + Python's `chr(92) + 'u2026'` for the search side and `chr(0x2026)` for the replacement side, which forces byte-level handling. **Generalization:** any time you need to make Unicode/escape-sequence-precise edits, the Edit tool may normalize away the difference; reach for Python via Bash to bypass.
- **2026-05-05 ~05:00 — `4e116ad` + `560ca62` — autonomous bug-fix cluster pattern: pre-grep-then-batch-edit-then-single-commit.** When the user says "fix these issues" with a list of 5+ items, the right shape is: (1) drift-grep each candidate first (catches Pattern A pre-fix in 30s/issue, often more than half qualify); (2) cluster the genuine fixes by file-locality so the diff stays readable in one commit body; (3) ship in ONE commit with a structured per-issue section in the message. Closing comments on each issue point at the same commit SHA — one signal, multiple resolutions. Counter-pattern (one-commit-per-issue) bloats the git log + makes "did this fix go out together?" hard to answer; multi-commit-batches are right when the fixes touch the same file and are independent (e.g. 7 separate frontend pages each getting their own debounce). **Rule of thumb:** N fixes in M files with no shared touchpoint → 1 commit. N fixes with shared touchpoint that you want to bisect → N commits. Today's #439/#440/#441/#448/#452/#456-partial cluster touched 6 files with no overlap → 1 commit; #433 came after via a sibling discovery so got its own commit (`560ca62`).
