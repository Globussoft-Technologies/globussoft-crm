...
- **Version:** v3.4.6 (wave-18 continuation of v3.4.5: wellness.js 3-way split COMPLETE — G-17 wellness-dashboard-api / G-18 wellness-reports-api / G-19 wellness-telecaller-api all ✅ shipped, totaling 146 dedicated tests across 3 specs against the 4,050-line / 41%-coverage file; #425 G-23 commit-message allowlist closed (4 markers `[allow-unique]` / `[allow-drop]` / `[allow-not-null]` / `[allow-narrow]` plus cross-class isolation invariants tested); #413 batch 2 ships 10 more `@relation` declarations on auth/security/integration models (RevokedToken / ScimToken / SsoConfig / Pipeline / Playbook / BookingPage / FieldPermission / RetentionPolicy / ApprovalRequest / SandboxSnapshot — drift dropped 39 → 29); zero healing commits required this wave; per-push gate now **~69 Playwright specs / ~2,442 tests + 37 vitest files / 995 unit tests = ~3,437 per-push (+4%)** with **5 mandatory deploy gates**. See [CHANGELOG.md](CHANGELOG.md))
