...
- **2026-05-07 wave-9 — `30f46b6` (Agent T) — concurrent stash/pop recovery now confirmed across THREE instances.** Agent T mid-flight encountered a sibling commit (Agent S's WIP on estimates-api). Stashed during rebase, popped after — push went through clean. This is the third instance of "concurrent agents in shared CWD recovering gracefully" in today's session: Wave 2 (Agent D detected Agent E's revert-and-prove dirty state, refused to commit), Wave 7 (Agent P detected Agent O's mid-flight datetime sweep, backed off + filed TODOS follow-up), Wave 9 (Agent T stash/popped past Agent S's WIP). **Pattern is now firmly promotion-eligible** — three different sub-patterns of the same "graceful concurrent recovery" meta-pattern, all of which avoided the dispatching-parallel-agent-wave skill's documented merge-collision class. Worth elevating from "promotion-eligible" to "should be promoted" on the next manual cron-learning review pass.

- **2026-05-07 wave-10 update to wave-8 entry — gap-card-vs-reality drift now FIVE instances today; pattern is overwhelmingly skill-worthy.** Adding to the wave-8 list above: Wave 10 Agent U found `tasks.js`'s `ensureDateInRange` window is `[2000, 2100]` not `[1990, 2100]` as the card claimed (4th instance); Wave 10 Agent V found `routes/report_schedules.js` emits `EXTERNAL_RECIPIENT_FORBIDDEN` not `PII_EXFIL_BLOCKED` as the card claimed (5th instance). Five-of-five regression-coverage agents this session surfaced some flavor of drift between gap-card claim and code reality. **Promotion is overdue.** The recommended standing rule is in the wave-8 entry; on the next manual review pass this should land as a one-liner under "Standing rules for new code" alongside the existing CI / spec / shape-sweep rules.

- **2026-05-07 wave-10 — `bc838d9` (Agent V) — backend bug surfaced inline with regression spec is now the DEFAULT outcome, not the exception.** Agent V's #20 spec also fixed a `validateRecipientsAgainstTenant` bug in `routes/report_schedules.js`: the helper returned shape errors as `{error, code}` without a `status` field, causing `res.status(undefined)` to fall through to a generic 500 response on inputs like `["@@@"]` instead of returning the intended 400 INVALID_RECIPIENT. The bug had been live since v3.4.11 (when the spec was first shipped with 8 sanitization tests); it didn't fail any existing assertion because the row indeed wasn't saved (so the data-integrity property held), but the API caller surface was misleading. **This is the SIXTH wave today where a regression-coverage agent shipped a backend gap fix inline with the spec** (Wave 4 Agent J's 3 fixes, Wave 5 Agent L's formatMoney port, Wave 7 Agent O datetime migration, Wave 7 Agent P junkSourceFilter helper, Wave 8 Agent R PATCH /:id alias, Wave 10 Agent U tasks.js migration, Wave 10 Agent V status:400 fix). The "spec ships missing code alongside" pattern is now the rule, not the exception. The wave-4 entry above that flagged this as a sizing concern (1d card → 2d real) has been validated 6 more times — the standing rule should formalize it, with sizing guidance ("regression-coverage dispatches typically include 1-3 backend gaps; budget +50-100% on top of the card's stated effort").

- **2026-05-07 wave-11 — `3d5bbc6` + `ae913a9` (Agents X + W) — well-implemented engines flip the "backend-fix-inline = default" pattern; Path A goes 100%, BOTH SIBLINGS independently.** Agent X's orchestrator-api spec (#16 — 29 tests) found ALL 5 acceptance points were already implemented in `cron/orchestratorEngine.js` + `routes/wellness.js` recommendation handlers. Zero Path B work. Idempotency keys correct (`(type + payloadHash)` AND `(type + title-prefix)` scoped to today), status exclusivity enforced via STATUS_RANK collapse, audit emission already wired post-flip, content-scrubbing already filters seed-pollution patterns, cost arithmetic already bounded. Agent W's sequences-authoring-api (#15 — 23 tests) ALSO found ALL 6 acceptance points Path A — `routes/sequences.js` was comprehensive. Both engines are wellness-vertical surfaces built ground-up (orchestrator v3.1, sequences refactored at v3.4.x). **Counter-example to the wave-10 "backend-fix-inline is default" finding** — well-developed engines shed Path B work entirely; patch-fixed routes (every other wave) accumulate gaps. **Refinement of the wave-10 sizing guidance:** "regression-coverage dispatches against engines built ground-up with their own tests typically stay Path A. Dispatches against routes accreted via N-issue patches typically include 1-3 backend gaps. Read the route's commit history with `git log --oneline -- backend/routes/<file>` for a sniff test before sizing." Agents X + W combined surfaced **9 more drift findings** (X: 4 — lowercase status enum, uppercase action verbs, GET un-gated, POST gated. W: 5 — defaults to `isActive: false` not `status: DRAFT`, GET /:id missing → use list-and-find, PATCH not PUT, error envelope is `{error, code}` not `{error, code, hint}`, field is `delayMinutes` not `delay`). **Gap-card drift count now TWELVE instances** across 7 waves today; the cumulative pattern is overwhelming and standing-rule promotion is **months overdue** at this rate.
