...
- **Version:** v3.4.4 (autonomous-orchestrator session: G-20 tenant-isolation flagship — 3 waves / 29 resources / 93 cross-tenant assertions in a single new spec; 7 reusable Claude Skills authored under `.claude/skills/` (Tier 1: writing-api-gate-spec / wiring-spec-into-gate / writing-vitest-unit-test; Tier 2: adding-admin-trigger-endpoint / bumping-version-docs / dispatching-parallel-agent-wave; visibility: reporting-agent-progress); live agent-activity widget at `/developer` (backend route + 3-second-poll React widget + log.sh helper) so parallel waves are no longer opaque; 5 audit-followup fixes (#412 #416 #417 #418 #419 #420 closed); R-4 medium-route batch (attribution / document-templates / email-threading / booking-pages); R-5 batch 2 cron-engine vitests (forecast / leadScoring / sentiment / sla); T2.1 mobile sidebar drawer at <900px; per-push gate now **~64 Playwright specs / ~2,237 tests + 35 vitest files / 677 unit tests = ~2,914 per-push (+18%)**. See [CHANGELOG.md](CHANGELOG.md))
