...
- **2026-05-07 wave-11 — `3d5bbc6` (Agent X) — well-implemented engines flip the "backend-fix-inline = default" pattern; Path A goes 100%.** Agent X's orchestrator-api spec (#16 — 29 tests) found ALL 5 acceptance points were already implemented in `cron/orchestratorEngine.js` + `routes/wellness.js` recommendation handlers. Zero Path B work. Idempotency keys correct (`(type + payloadHash)` AND `(type + title-prefix)` scoped to today), status exclusivity enforced via STATUS_RANK collapse, audit emission already wired post-flip, content-scrubbing already filters seed-pollution patterns, cost arithmetic already bounded. **Counter-example to the wave-10 "backend-fix-inline is default" finding** — well-developed engines (the wellness orchestrator was a ground-up build per the v3.1 release with extensive concurrent testing) shed Path B work entirely; patch-fixed routes (every other wave) accumulate gaps. **Refinement of the wave-10 sizing guidance:** "regression-coverage dispatches against engines built ground-up with their own tests typically stay Path A. Dispatches against routes accreted via N-issue patches typically include 1-3 backend gaps. Read the route's commit history with `git log --oneline -- backend/routes/<file>` for a sniff test before sizing." Agent X also surfaced 4 more drift findings (lowercase status enum, uppercase action verbs, GET un-gated, POST gated) — gap-card drift count now SEVEN instances today. Standing rule promotion is overdue.
