...
- **Version:** v3.4.7 (QA P0/P1 closure + demo-pollution root-cause + PR #444 visitors dashboard + #413 batch 3: **3 real security/compliance fixes** — #426 P0 portalPasswordHash leak via global `scrubResponse` middleware, #343 P1 token-in-localStorage SSO leftover deleted + #427/#428 defense-in-depth deny-list / X-Tenant-Id regression-guard, #405 P1 demo-pollution root cause closed by adding `/^_teardown_/` to scrub pattern list + 76-test pattern-drift guard auto-clearing 4 sibling cluster issues #403/#319/#310/#328; **PR #444 visitors dashboard** merged (+743 / −89, 14 files) with 2 unblock commits (lint + `/send-email` 200-always contract revert); **#413 batch 3** adds 10 more `@relation` declarations on calendar/scheduled-email/KB/SLA cluster + PlaybookProgress audit with tenantId-keyed `@@unique` (drift 29 → 19); **13 issues closed total** (3 real + 1 already-fixed-but-unclosed + 4 pollution-cluster siblings + 6 false-positives verified via code grep); per-push gate now **~71 Playwright specs / ~2,460 tests + 39 vitest files / 1,093 unit tests = ~3,553 per-push (+108 from v3.4.6)** with **5 mandatory deploy gates**. See [CHANGELOG.md](CHANGELOG.md))
