...
- **Version:** v3.4.8 (v3.4.7 follow-up arc — 6 issues closed: **T2.2 PHI read-audit** complete (6 staff GET handlers in `routes/wellness.js` gain `writeAudit`; 8-test contract spec pins staff-vs-patient actor split); **#180 JWT revocation** contract pinned (10-test spec — implementation was already shipped in v3.2.1, gate coverage was missing); **#413 schema-relation hygiene COMPLETE** (drift 49 → 0 across batches 1-4 — every multi-tenant model now has `tenant Tenant @relation(onDelete: Cascade)` + back-relation); **#436 Tasks queue empty for Owner** (two interlocking bugs: global `stripDangerous` middleware deleted body `userId` which is `Task.userId`/assignee, and Sidebar query used uppercase status while enum is Title-case — fix adds `targetUserId` body field + `normalizeStatusFilter` + `?mine=true`); **#443 GDPR DSAR audit-trail** wired (both /export/me and /export/contact/:id now route through `writeAudit('User'\|'Contact', 'GDPR_EXPORT', ...)`; 11-test spec); **#398 Sequences XSS** spec landed (route already sanitized, spec was missing); **Service-scrub gap** closed (v3.4.7 release-validation found 3 surviving `_teardown_iso_*` services because neither `e2e/global-teardown.js` nor `backend/scripts/scrub-test-data-pollution.js` iterated Service — fixed + 8-test scrub-coverage invariant). 4-agent parallel wave was clean (no merge collisions, no rebased retries). Per-push gate now **~75 Playwright specs / ~2,500 tests + 39 vitest files / 1,101 unit tests = ~3,601 per-push (+48 from v3.4.7)** with **5 mandatory deploy gates**. See [CHANGELOG.md](CHANGELOG.md))
