...
- **Version:** v3.4.10 (v3.4.9 carry-over arc — deploy-gate unblock + #447 P1 XSS + new `triaging-stuck-deploy-gate` skill: **deploy gate stuck red 11+ pushes / ~2 hours** since `b44291b` (T2.2 wellness-audit landing) — unblocked by 4 bundled fixes in `940b4f0` per the new skill (auth-revocation `[401, 403]` relaxation + `WELLNESS_DEMO_OTP` env-var added to CI api_tests env block + spec-bad-fixture switch from `status:'completed'` to `status:'booked'` + `sanitizeJson` Option A revert with new `sanitizeJsonForStringColumn` wrapper at the SequenceStep call sites); **#447 P1 landing-page XSS closed** (commit `0618882`) — new `safeUrl(input, kind)` helper in `services/landingPageRenderer.js` with three kinds (image-src / link-href / iframe-src) + scheme allowlist; applied at 3 render sites (image, button, video — button was the actually-executable XSS); 55-test regression suite extends `landingPageRenderer.test.js` (45 → 100); **/api/health hardcoded-version follow-up** (commit `44747b4`) — `APP_VERSION = require("./package.json").version` at boot replaces the hardcoded `"3.2.0"` literal that survived 5+ release tags; static-grep regression test fails CI on future literals; **new `triaging-stuck-deploy-gate` skill** captures the canonical 5-step triage flow + 7 classification buckets (5 carry-overs + 2 NEW: CI env-block gap + spec-bad-fixture, both battle-tested in 940b4f0); **3 new standing rules** added below (CI env-block parity, /api/health caveat, JSON-string columns call-site-stringify pattern); **#182 SMS reminder regressions** closed (`cf296dd` — double-word "appointment appointment", `[reminder:24h]` debug markers leaking to customer SMS, smoke-test pollution); **v3.4.8 carry-over #4** closed (`0b26e84` — `routes/shared_inbox.js` body-`userId` collision via mirror-pattern fix of #436); **#195 + #213** verified already-shipped via the `verifying-issue-before-pickup` skill (4-of-10 picked-from-TODOS issues across v3.4.8+v3.4.9+v3.4.10 were already done; ~40% drift rate). Per-push gate now **~76 Playwright specs / ~2,514 tests + 42 vitest files / ~1,184 unit tests = ~3,698 per-push (+69 from v3.4.9)** with **5 mandatory deploy gates** + **10 reusable Claude Skills**. See [CHANGELOG.md](CHANGELOG.md))
...
- **JSON-string columns** → if a Prisma column is `String? @db.Text` storing JSON (e.g. `SequenceStep.conditionJson`, any `*Json` field), the **call site** stringifies before storing — the helper itself stays shape-preserving so other callers can use it for true JSON columns. See `routes/sequences.js sanitizeJson()` (shape-preserving) + `sanitizeJsonForStringColumn()` (the wrapper used at the SequenceStep call sites). The 940b4f0 wave reverted an earlier always-stringify change after it broke 16 unit tests pinning the helper's shape-preservation contract.
