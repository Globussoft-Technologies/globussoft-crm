...
A v3.4.9-carry-over arc that started red and ended with two new skills' worth of distilled learning. The deploy gate went red on `b44291b` (T2.2 wellness-audit landing in v3.4.8) and stayed red across **11+ consecutive pushes / ~2 hours**, blocking demo deploys while testers reported regressions against stale code. This arc unblocked it, closed a P1 XSS, and codified the lessons in a new skill that battle-tested its diagnosis rules within the same session.

- **#447 P1 [landing-pages][security] image URL XSS closed**: `backend/services/landingPageRenderer.js` previously HTML-escaped attribute values but didn't validate URL schemes. Code-grep verification revealed the bug at THREE render sites (image / button / video) — with the **button case actually executable** (`<a href="javascript:...">` runs). New `safeUrl(input, kind)` helper with three kinds (image-src / link-href / iframe-src), each with its own scheme allowlist + safe fallback. **55-test regression suite** (45 → 100 tests in `landingPageRenderer.test.js`) covers scheme allow/deny by kind, mixed-case + whitespace-prefix bypass attempts, end-to-end XSS-payload integration assertions.
- **Deploy gate unstuck via the new triaging skill**: 4 fixes bundled in ONE commit (`940b4f0`) per the new skill — `[401, 403]` relaxation on missing-token tests + `WELLNESS_DEMO_OTP` env var added to CI's api_tests env block + spec-bad-fixture switch from `status:'completed'` to `status:'booked'` + sanitizeJson Option A revert (shape-preserving helper + new `sanitizeJsonForStringColumn` wrapper at the SequenceStep call sites). All 6 deploy gates green for the first time in ~2 hours.
- **`/api/health` hardcoded-version follow-up**: the 940b4f0 triage's "verify demo divergence" step revealed `backend/server.js:435+443` hardcoded `version: "3.2.0"` (literal, surviving 5+ release tags). Fix: `APP_VERSION = require("./package.json").version` once at boot. Static-grep regression test fails CI on any future hardcoded literal.
- **New `triaging-stuck-deploy-gate` skill (project skill #10)**: captures the canonical 5-step triage flow + 7 classification buckets (5 carry-overs + 2 NEW battle-tested in 940b4f0 — CI env-block gap + spec-bad-fixture). Skill description: "stop-the-line response when the api_tests gate is red on 2+ consecutive pushes". Triggered by the 940b4f0 incident; will trigger automatically next time.
- **3 new CLAUDE.md standing rules** (CI env-block parity, /api/health hardcoded-version caveat, JSON-string columns call-site-stringify pattern): codified the wave's anti-patterns so future code can't ship them.
- **#182 SMS reminder regressions + v3.4.8 carry-over #4 closed**: 3 SMS-body bugs (double-word "appointment appointment", `[reminder:24h]` debug markers leaking to customer, smoke-test pollution); shared_inbox.js POST `/:id/members` + `/assign-message` userId-collision (mirror-pattern fix of #436). #195 + #213 verified already-shipped via the `verifying-issue-before-pickup` skill (4 of 10 picked-from-TODOS issues across v3.4.8+v3.4.9+v3.4.10 were already done — drift rate steady at ~40%).

Per-push gate is now **~76 specs / ~2,514 API tests + 42 vitest files / ~1,184 unit tests = ~3,698 tests on every push** (+69 vs v3.4.9).

See [CHANGELOG.md](CHANGELOG.md#v3410--2026-05-04--deploy-gate-stuck-unblocked--447-p1-xss--apihealth-hardcoded-version-follow-up--new-triaging-stuck-deploy-gate-skill) for the full v3.4.10 entry.

## What's new in v3.4.9 (May 4 2026 — v3.4.8 carry-over wave: 4 drift findings closed + #167 verified-already-shipped + verifying-issue skill landed)

A focused-followup release covering the v3.4.8 carry-over backlog. **One new product feature** (patient self-DSAR endpoint for DPDP §15 / GDPR Art. 15 compliance) plus three refinements plus a new skill that's already paying off.

- **Patient self-DSAR endpoint** (`POST /api/wellness/portal/export`): patients can self-export their data via the wellness portal token. Walks the FK chain `Patient → Visit / Prescription / ConsentForm / TreatmentPlan / LoyaltyTransaction / Referral` (every query filters on `patientId: req.patient.id`, never tenantId-only). Audit row: `writeAudit('Patient', 'GDPR_EXPORT_SELF', ..., { actorType: 'patient', patientId })`. Closes the v3.4.8 finding that `/api/gdpr/*` rejects portal tokens at middleware level. 9-test contract spec.
- **Sequence step body sanitization** (carry-over #1): step-level `smsBody` and `conditionJson` on POST /:id/steps + PUT /steps/:id now route through `sanitizeText()` and a new exported `sanitizeJson()` helper (recursive walk over JSON blobs, null-safe, merge-tag-preserving). 10 vitest cases pinning the helper + 4 e2e cases for the route contract.
- **GDPR `/export/contact/:id` role guard tightened** (carry-over #3): now requires `verifyRole(['ADMIN', 'MANAGER'])`, matching sibling `/retention/run` least-privilege default. Spec's RBAC describe block flipped: USER 200 → 403; new MANAGER lane test added. Self-export `/export/me` is unchanged.
- **Orchestrator writes canonical Task case** (carry-over #5): `cron/orchestratorEngine.js` now writes `status:"Pending"` / `priority:"High"` (Title-case canonical per schema:773-774) instead of uppercase `"OPEN"`/`"HIGH"`. Sweep across all 17 cron engines verified — only orchestrator was drifted; ScheduledEmail uses uppercase legitimately (its own schema canonical). 4 new vitest assertions pin the case.
- **#167 verified already-shipped** (no code change): pre-pickup grep found soft-delete + AuditLog + `/restore` already implemented on all 4 routes (Contacts/Deals/Estimates/Tasks); existing specs already have 14-17 audit assertions each. The 4-5 day TODOS estimate was pure phantom-work, caught in 60 seconds by the parent agent before dispatching a 4-agent wave on already-done work.
- **`verifying-issue-before-pickup` skill** added (`.claude/skills/verifying-issue-before-pickup/SKILL.md`) capturing the v3.4.8 wave's learning: **4 of 8 issues picked from TODOS.md across v3.4.8 + v3.4.9 were already done** (50% doc-drift rate). 4-step grep checklist, 4 drift patterns, integration with dispatching/capturing/bumping skills. Cross-reference added in `dispatching-parallel-agent-wave/SKILL.md`. Project skill count: 8 → 9.

Per-push gate is now **~76 specs / ~2,514 API tests + 40 vitest files / 1,115 unit tests = ~3,629 tests on every push** (+28 vs v3.4.8).

See [CHANGELOG.md](CHANGELOG.md#v349--2026-05-04--v348-carry-over-wave-4-drift-findings-closed--167-verified-already-shipped--verifying-issue-skill-landed) for the full v3.4.9 entry.

## What's new in v3.4.8 (May 4 2026 — v3.4.7 follow-up arc: T2.2 + #180 + #398 + #413 + #436 + #443 closed)

A focused-followup release covering the v3.4.7 carry-over plus a 4-agent parallel wave. **No new product features**; this release closes six issues across two days, eliminates the schema-relation drift counter (49 → 0 across batches 1-4), and adds 4 new per-push gate specs + extends 1 existing spec.

- **T2.2 PHI read-audit closed**: 6 staff GET handlers in `routes/wellness.js` gained `writeAudit` (VISIT_LIST_READ, VISIT_CONSUMPTIONS_READ, PRESCRIPTION_LIST_READ, CONSENT_LIST_READ, TREATMENT_PLAN_LIST_READ, TREATMENT_PLAN_READ). 8-test contract spec pins the staff-vs-patient actor split, tenantId scoping, and shape-only details payload (never row contents). 4-5 day estimate compressed to 1 session — the v3.2.1 audit infrastructure was already mature.
- **#180 JWT revocation contract pinned**: implementation was already in v3.2.1 but had ZERO per-push gate coverage. New 10-test `auth-revocation-api.spec.js` covers happy logout (token works pre, 401 post), idempotent upsert, /sessions shape (no userId leak in revokedSessions[]), history reflection, malformed-jti 400 (too short / too long), tenant isolation.
- **#413 schema-relation hygiene COMPLETE — drift 49 → 0**: batch 4 added the final 18 `@relation` declarations on chat/live + dashboards + scheduled-email/booking + survey/template/document + social + voice + marketing/attribution clusters. Every multi-tenant model now has formal `tenant Tenant @relation(onDelete: Cascade)` + matching back-relation. G-24 invariant test will warn at 0 from now on.
- **#436 Tasks queue empty for Owner — two interlocking bugs**: (1) global `stripDangerous` middleware deleted body `userId` (which is `Task.userId` — the assignee, not a tenant pivot) → tasks landed with `userId=null`; (2) Sidebar query hard-coded `?status=PENDING` (uppercase) while enum is Title-case `Pending`. Fix: POST reads `targetUserId`, GET adds `normalizeStatusFilter()` + `?mine=true`. 3-test extension to `tasks-api.spec.js`.
- **#443 GDPR DSAR audit-trail wired**: TODOS framed as "501 stub" but the actual gap was missing AuditLog rows on /export/me and a legacy `action='EXPORT'` label on /export/contact/:id. Both now route through `writeAudit('User'|'Contact', 'GDPR_EXPORT', ...)` with shape-only details. 11-test contract spec (auth gate, cross-tenant 404, audit row contract, RBAC sanity).
- **#398 Sequences XSS** (route was already sanitizing — spec was missing): 8-test pin on `<script>` strip, `javascript:` href strip in ReactFlow node labels, only-HTML-name → 400, PATCH rename, idempotent re-POST, cross-tenant isolation.
- **Service-scrub gap closed** (v3.4.7 release-validation surfaced 3 surviving `_teardown_iso_*` services on demo): both `e2e/global-teardown.js` (hardcoded regex replaced with shared `PAT_REGEX`) and `backend/scripts/scrub-test-data-pollution.js` (added `scrubServices()`) now iterate Service. New 8-test scrub-coverage invariant statically grep-asserts both teardown scripts handle Patient/Contact/Service/Task/Location.

**Doc-vs-reality drift surfaced** by 3 of 4 agents (#180, #398, #443) — when picking from TODOS.md, grep the implementation before estimating. The actual gaps were test-coverage in 2 of 3 cases.

Per-push gate is now **~75 specs / ~2,500 tests + 39 vitest files / 1,101 unit tests = ~3,601 tests on every push** (+48 vs v3.4.7).

See [CHANGELOG.md](CHANGELOG.md#v348--2026-05-04--v347-follow-up-arc-t22--180--398--413--436--443-closed-6-issues--scrub-gap) for the full v3.4.8 entry.
