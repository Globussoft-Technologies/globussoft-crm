...
| **#567** (filed 2026-05-07 from Agent D investigation) | Dashboard.jsx KPIs miss aggregate when won-deals fall outside newest-100 paginated window | Real correctness bug, autonomous-fixable, ~50 LOC frontend. Surfaced inside the #552/553/554 cluster but is orthogonal — happens even when the page renders consistently. Use `/api/deals/stats` for KPI aggregates + `/api/deals?limit=10` for the Recent Deals widget. Vitest pin needed. |
...
| **#534 follow-ups** — pen-test named "4 list endpoints >2s"; indexed Patient + TreatmentPlan; need profiling for the other 2 (likely Visit / Prescription) | ⬜ open — ~30m profile + ~1h fix |
...
| **#534 follow-ups** — pen-test specifically named "4 list endpoints >2s". Indexed Patient + TreatmentPlan; the OTHER two endpoints need profiling against demo to identify (likely Visit list or Prescription list in cold-call window) | ~30m profile + ~1h fix | Perf |
