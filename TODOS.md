...
| **#568** (filed 2026-05-07 from Agent I audit-coverage spec) | Pipeline routes have zero `writeAudit` calls — admin-config writes go un-audited | Compliance gap surfaced by `fef51a6`'s `audit-coverage-api.spec.js`. Pattern reference: `routes/contacts.js:121` (CREATE) + `routes/wellness.js:790` (DELETE). When emission lands, the spec's 2 gap-tracking tests flip from "asserts absence" to `expectAuditShape(...)`. Autonomous-fixable, ~30 LOC backend + spec flip. |
| **#569** (filed 2026-05-07 from Agent I audit-coverage spec) | `/auth/logout` does not emit `writeAudit('User', 'LOGOUT', ...)` — RevokedToken upserts but audit trail stays silent | Compliance gap. LOGIN already audits (paired). Pattern: mirror `routes/auth.js` LOGIN's `writeAudit` call. When emission lands, the audit-coverage-api spec's #180 test flips from soft `console.warn` to hard `expectAuditShape`. Autonomous-fixable, ~10 LOC backend + spec flip. |
