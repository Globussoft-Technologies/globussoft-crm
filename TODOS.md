...
| **#632 follow-up — Staff/Profile aria-label sweep** (filed 2026-05-08 alongside #632 #633 hotfix) | Add `aria-label` to icon-only buttons in `frontend/src/pages/Staff.jsx`, `Profile.jsx`, `Profile2FA.jsx`, `Tasks.jsx`, `LeadScoring.jsx`, `Surveys.jsx`, `wellness/Loyalty.jsx`, `wellness/PatientDetail.jsx` — these were SKIPPED in the #632 sweep because Wave 5 H + Wave 6 K/L/M agents held them concurrently. The pattern is identical to the canonical sweep (commit will be linked here): row-action delete/edit pencils, modal close-X, X-icon dismiss buttons. ~15 min once those agents land. The #632 vitest spec at `frontend/src/__tests__/a11y-table-stability.test.jsx` already pins 21 aria-label sites; the follow-up just extends `ICON_BUTTON_SITES` with the corresponding rows from the held files. Autonomous-fixable. |
