...
- [ ] **#176** `POST /api/contacts/:id/attachments` always 500. Multer config missing or wrong mime handler. Needs file-upload investigation.
...
- [ ] **#180** No JWT revocation. 7-day tokens are not revocable; no logout endpoint, no session listing. Plan: add a `RevokedToken` table keyed by jti, check on `verifyToken`. ~4 hours + schema migration.
- [ ] **#182** SMS queue stuck — 25 messages QUEUED with no provider configured. Wire MSG91 or Twilio (env vars exist) and a drain endpoint.
- [ ] **#184** `/survey/:id` customer-facing route broken: blank content, shows admin sidebar to logged-in users. Needs frontend routing fix + a `/api/surveys/public/:id` endpoint.
- [ ] **#186** No security headers. Missing CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, HSTS, Permissions-Policy. `helmet` is mounted but underconfigured. ~30 min + check no inline scripts break.
- [ ] **#190** Deal stage data migration. Existing rows with stage='Lead' (capitalized) cannot be PUT-updated after the validator was tightened. One-time SQL: `UPDATE Deal SET stage = LOWER(stage) WHERE stage IN ('Lead','Contacted','Proposal','Negotiation','Won','Lost')`.
- [ ] **#191** Login rate limiting. Currently 30 wrong-password attempts in 3.2s all return 403 with no throttling. Add `express-rate-limit` per-IP-per-username on `/auth/login`.
