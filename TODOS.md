...
| **formatMoney callsite-sweep** (filed 2026-05-07 from Agent L commit body) | #286 + #330 root cause: helper exists + tests pin contract, but production bugs originated from CALLSITES bypassing the helper with hardcoded `${amount}` template literals | Agent L's `8fd3283` shipped the helper + 31 vitest cases at `backend/utils/formatMoney.js` + `backend/test/utils/formatMoney.test.js`. The unit test pins the helper's contract; cannot detect bypass callsites. **Audit needed:** `grep -rn '\\$\\$\\{' backend/services/ backend/routes/ frontend/src/` + identify any literal `$` symbols in user-facing render paths. Optionally: add an ESLint custom rule (or comment audit) to prevent regression. ~1-2h sweep + per-callsite fixes. Autonomous-fixable. |
| **datetime callsite-sweep** (filed 2026-05-07 — backlog #23 Path B follow-up) | #244 + #313 + #387 root cause: helper now exists at `backend/lib/datetime.js` + 35 vitest cases pin the contract, but no callsites have been migrated. The helper-only commit closes the regression-coverage backlog item but leaves the production drift behind. | Agent R-23's commit shipped `backend/lib/datetime.js` (`parseDateTimeLocalInTZ` / `formatInTenantTZ` / `toDateTimeLocalInTZ` / `nowInTZ`) over `date-fns-tz`. Three known callsite classes need migration: (a) `routes/wellness.js` `IST_OFFSET_MS = 5.5 * 60 * 60 * 1000` shortcut (~line 165) — should use `parseDateTimeLocalInTZ` so non-IST tenants stop being broken; (b) any naive `new Date(req.body.startsAt)` constructions in visit / appointment / scheduled-email POST handlers — they currently parse datetime-local input as UTC, drifting by tenant offset (#313); (c) `lib/audit.js` ships rows with raw UTC `createdAt` — the audit-viewer render path (`routes/audit_viewer.js` + `frontend/src/pages/AuditLog.jsx`) needs to call `formatInTenantTZ(row.createdAt, viewerTZ)` to satisfy #387's "TZ label" acceptance. Tenant TZ source: `User.timezone` column already exists (default `'UTC'`) — fall back to `'Asia/Kolkata'` for wellness tenants if user TZ is unset. ~2-3h sweep + per-callsite tests. Autonomous-fixable. |
