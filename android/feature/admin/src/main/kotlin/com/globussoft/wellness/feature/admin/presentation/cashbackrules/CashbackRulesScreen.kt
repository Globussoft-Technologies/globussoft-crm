...
fun CashbackRulesScreen(
    viewModel: CashbackRulesViewModel = hiltViewModel(),
    onNavigateBack: () -> Unit = {},
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.effects.collect { effect ->
            when (effect) {
                is CashbackRulesEffect.ShowSnackbar -> scope.launch {
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("Cashback Rules", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold) },
                navigationIcon = {
        ...
        floatingActionButton = {
            ExtendedFloatingActionButton(
                onClick        = { viewModel.onEvent(CashbackRulesEvent.OpenNewSheet) },
        ...
        PullToRefreshBox(
            ...
            onRefresh    = { viewModel.onEvent(CashbackRulesEvent.Refresh) },
            ...
            when {
                ...
                state.error != null && state.rules.isEmpty() ->
                    ErrorState(message = state.error!!, onRetry = { viewModel.onEvent(CashbackRulesEvent.Refresh) }, modifier = Modifier.fillMaxSize())
                state.rules.isEmpty() ->
                    EmptyState(message = "No cashback rules configured.", icon = Icons.Default.Loyalty, actionLabel = "Add Rule", onAction = { viewModel.onEvent(CashbackRulesEvent.OpenNewSheet) }, modifier = Modifier.fillMaxSize())
                else -> LazyColumn(
                    ...
                    items(state.rules, key = { it.id }) { item ->
                        CashbackRuleCard(
                            item     = item,
                            onEdit   = { viewModel.onEvent(CashbackRulesEvent.OpenEditSheet(item)) },
                            onDelete = { viewModel.onEvent(CashbackRulesEvent.RequestDelete(item.id)) },
                        )
                    }
                }
            }
        ...
        if (state.showSheet) {
            CashbackRuleFormSheet(
                isEditing = state.editingItem != null,
                form      = state.form,
                isSaving  = state.isSaving,
                saveError = state.saveError,
                onField   = { field, value -> viewModel.onEvent(CashbackRulesEvent.FieldChanged(field, value)) },
                onToggle  = { viewModel.onEvent(CashbackRulesEvent.ToggleActive(it)) },
                onSave    = { viewModel.onEvent(CashbackRulesEvent.Save) },
                onDismiss = { viewModel.onEvent(CashbackRulesEvent.DismissSheet) },
            )
        }
        ...
        if (state.showDeleteConfirm) {
            ConfirmDialog(
                title         = "Delete Rule",
                message       = "Remove this cashback rule?",
                confirmLabel  = "Delete",
                isDestructive = true,
                onConfirm     = { viewModel.onEvent(CashbackRulesEvent.ConfirmDelete) },
                onDismiss     = { viewModel.onEvent(CashbackRulesEvent.DismissDelete) },
            )
        }
    }
}
...
@Composable
private fun CashbackRuleCard(
    item: CashbackRuleItem,
    onEdit: () -> Unit,
    onDelete: () -> Unit,
) {
    WellnessCard(modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                val meta = buildList {
                    add("Earn ${item.earnPercent}%")
                    if (item.minSpend != null) add("Min ₹${item.minSpend}")
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun CashbackRuleFormSheet(
    isEditing: Boolean,
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            Text(
                text       = if (isEditing) "Edit Cashback Rule" else "New Cashback Rule",
            ...
            Row(
                ...
                OutlinedTextField(
                    ...
                    onValueChange   = { onField("earnPercent", it) },
                    label           = { Text("Earn % *") },
                ...
                OutlinedTextField(
                    value           = form.minSpend,
                    onValueChange   = { onField("minSpend", it) },
