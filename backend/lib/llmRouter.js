...
const TASK_ROUTING = {
  ...
  // AI curriculum-to-itinerary matching (2026-08-24). Given a curriculum PDF's
  // extracted text, pull out discrete learning objectives so they can be
  // embedded and indexed into the curriculum_objectives Qdrant collection
  // (see backend/lib/curriculumRag.js). Same low-cost JSON-shape routing as
  // travel-knowledge-rag — this is a bulk extraction task, not a reasoning
  // task, so Gemini Flash primary keeps ingestion cost down.
  "curriculum-objective-extraction": { primary: "gemini-flash", fallback: "gpt-4o-mini" },
  // AI curriculum-to-itinerary matching, final ranking step (2026-08-24).
  // Given the school's stated curriculum/grade/outcomes, the closest-matching
  // curriculum learning objectives (from curriculum_objectives Qdrant), AND
  // the closest-matching itinerary excerpts (from the existing itinerary
  // Qdrant collection), produce the final ranked destination recommendations
  // with per-destination fit reasoning. Same low-cost routing as the sibling
  // curriculum/RAG tasks.
...
function buildStubText(task, _payload) {
  ...
  switch (task) {
    ...
    case "curriculum-objective-extraction":
      return JSON.stringify({
        objectives: [
          {
            text: "[STUB-CURRICULUM-EXTRACTION] Synthetic learning objective. Real Gemini Flash extraction lands when Q11 keys arrive.",
            subject: "General",
            topicCode: null,
          },
        ],
      });
    case "curriculum-itinerary-match":
      return JSON.stringify({
        recommendations: [
          {
            destination: "[STUB-CURRICULUM-MATCH] Synthetic destination",
            fitScore: 80,
            reasons: [
              { subject: "General", learningOutcome: "Synthetic reason. Real Gemini Flash matching lands when Q11 keys arrive." },
            ],
            driveLink: null,
          },
        ],
    ...
    case "travel-knowledge-rag":
      return JSON.stringify({
        readiness_level: 2,
        readiness_name: "Engagement-Focused & Experience-Driven",
        readinessScore: 7,
...
function buildPrompt(task, payload) {
  ...
  const SYS = {
    ...
    "itinerary-suggest": "You are an expert travel planner pricing a trip for a per-person quote. Given a destination, departure city, number of days, budget tier, and traveller interests/pace, return a realistic day-by-day itinerary as STRICT JSON only — no markdown, no text outside the JSON. Shape: {\"summary\":string,\"days\":[{\"dayNumber\":number,\"items\":[{\"itemType\":string,\"description\":string,\"locationName\":string,\"estimatedCost\":number,\"startTime\":string}]}]}. itemType MUST be one of: flight, transfer, hotel, sightseeing, activity, meals, visa, insurance, other. locationName is a SHORT, REAL, geocodable place name for that item — a specific named beach, fort, hotel-area, landmark, or airport (e.g. \"Calangute Beach, Goa\", \"Fort Aguada, Goa\", \"Indira Gandhi International Airport, Delhi\") — NOT a sentence, NOT the generic destination alone unless the item genuinely has no more specific place (e.g. a free evening); this is the exact string used to plot the item on a map, so vague or invented place names produce a wrong/missing pin. startTime is a 24-hour \"HH:MM\" clock time for when that item begins — ALWAYS include it, order each day's items chronologically by it, and keep the times realistic and non-overlapping (e.g. a morning sightseeing at \"09:30\", lunch at \"13:00\", dinner at \"19:30\"); these times are printed in the customer's itinerary table, so a day with no times renders as a blank column. estimatedCost is the typical PER-PERSON cost in INR (Indian Rupees) — always give a REALISTIC POSITIVE number; use 0 only when genuinely free. CRITICAL FOR FLIGHTS: the context includes a departureCity field (e.g. \"Bangalore\") — use it to describe and price both the Day-1 outbound flight (e.g. \"Flight from Bangalore to Paris\") and the final-day return flight (e.g. \"Return flight Paris to Bangalore\") with a realistic one-way airfare in INR: economy ≈ ₹18,000–₹35,000, mid ≈ ₹35,000–₹65,000, luxury ≈ ₹65,000–₹1,50,000 for international routes; adjust down for short-haul. NEVER set a flight estimatedCost to 0. Each day should also include a hotel, at least one sightseeing, one activity, and one meals item. Keep descriptions short and destination-specific. Return ONLY the JSON object.",
    ...
    "curriculum-objective-extraction":
      "You are an academic curriculum analyst. Given raw extracted text from a board curriculum PDF (e.g. CBSE, ICSE, IB, Cambridge) along with the board and grade band the admin selected when uploading it, identify the discrete LEARNING OBJECTIVES / learning outcomes a student is expected to achieve. Return STRICT JSON only — no markdown, no text outside the JSON. Shape: {\"objectives\": [{\"text\": string, \"subject\": string, \"topicCode\": string|null, \"gradeBand\": string|null}]}. Each `text` should be ONE concise, self-contained learning-outcome statement (e.g. \"Understand plate tectonics and landform formation through field observation\") — not a whole paragraph, not a table of contents entry, not administrative text (grading policy, exam structure, page numbers, chapter numbers alone). `subject` is the school subject the objective belongs to (e.g. \"Geography\", \"History\", \"Biology\") — infer it from section headings in the text; if genuinely ambiguous, use the closest subject heading present in the text. `topicCode` is the board's own strand/topic code if the text names one (e.g. \"CBSE-GEO-9-3\"), otherwise null. `gradeBand` is the specific class/grade range stated beside that objective in the PDF; use the uploaded grade band only when the text does not state a narrower grade. Extract as many distinct objectives as the text actually supports — do not pad with duplicates or invented objectives, and do not include cancellation/policy/admin text. Return ONLY the JSON object.",
    "curriculum-itinerary-match":
      "You are an academic-travel matching advisor. You are given: (1) the enquiring school's stated curriculum board, grade band, subject(s), desired learning outcomes, and diagnosticSignals containing the remaining non-identity diagnostic answers; (2) a set of curriculum learning objectives retrieved from that board's uploaded curriculum documents, each tagged with subject and grade band; (3) a set of itinerary/trip excerpts retrieved from the trip brochure library; and (4) recommendationLimit, the desired maximum number of customer-facing results. Your job is to use all of these signals together to decide which trips best deliver the school's curriculum objectives and practical requirements, then produce a final ranked shortlist. Treat a curriculum objective as applicable only when its grade band includes the school's grade or is unspecified. Return STRICT JSON only — no markdown, no text outside the JSON. Shape: {\"recommendations\": [{\"destination\": string, \"fitScore\": number 1-100, \"reasons\": [{\"subject\": string, \"learningOutcome\": string}], \"driveLink\": string|null}]}. Aim to return recommendationLimit strong destinations, ranked by fitScore descending, each destination appearing once. Never return a fitScore of 0 or less. For each destination give 2-4 `reasons`, each pairing a specific curriculum learning objective with what in that trip's excerpt satisfies it — be concrete (name the site/activity from the excerpt), never generic filler. `driveLink` is the trip's brochure link if present in its excerpt, otherwise null. Only use destination names and facts that appear in the itinerary excerpts — never invent a destination. Never include cancellation, refund, payment, or insurance content in a reason. If fewer than recommendationLimit destinations are genuinely a reasonable curriculum fit, return only the ones that are.",
    "travel-knowledge-rag":
      "You are a travel advisor for the CRM. Given a customer diagnostic profile, a set of brochure excerpts retrieved from a vector database, and recommendationLimit (the desired maximum number of customer-facing results), produce a structured JSON report that helps the customer understand which travel option fits them best. The sub-brand is provided in the context as `subBrand` (e.g., tmc = school trips, rfu = Umrah packages, travelstall = family holidays, visasure = visa services). Return STRICT JSON only — no markdown, no text outside the JSON. Shape: {\"readiness_level\": number 1-4, \"readiness_name\": string, \"summary\": string, \"recommendedTrips\": [{\"name\": string, \"driveLink\": string, \"summary\": string, \"learnings\": string[]}]}. Categorize the school into exactly one readiness level based on the diagnostic answers (curriculum, grade levels, topics, learning goals, budget, group size, timeline): 1 = Curriculum-Aligned & Outcome-Focused (clear curriculum goals and measurable outcomes; premium / IB / Cambridge / top CBSE), 2 = Engagement-Focused & Experience-Driven (values experiential learning but needs curriculum-alignment guidance; mid-tier schools), 3 = Exploration-Ready & Value-Conscious (open to educational travel but needs structured guidance and clear ROI; first-time, budget-conscious, smaller schools), 4 = Custom Support Recommended (complex needs such as multi-curriculum, special requirements, or high parent objections). The readiness_name MUST be exactly one of those four canonical labels. Aim to return recommendationLimit strongly relevant brochure options. It is acceptable to return fewer only when the retrieved catalogue does not contain enough strong matches; never add a weak or unrelated brochure merely to reach the target. For every recommended option, provide a `summary` of exactly 2 concise sentences explaining why it suits the customer. Provide exactly 4 short `learnings` that describe what the traveller will experience, learn, or do on the trip. Never include cancellation, refund, payment terms, insurance disclaimers, or general booking/policy statements in the learnings. Use only the option names and facts present in the excerpts. Include the provided driveLink for each option so it is clickable in the final PDF. Do not invent destinations, prices, or details not in the excerpts.",
    reasoning:
