...
// Per-day creative templates. {dest} + {name} + {portalUrl} are interpolated.
const TEMPLATES = {
  30: {
...
function interpolate(str, { destination, customerName, portalUrl }) {
  return String(str)
    .replace(/\{dest\}/g, destination || "your destination")
    .replace(/\{name\}/g, customerName || "traveller")
    .replace(/\{portalUrl\}/g, portalUrl || "your customer portal");
}
...
function buildFallbackNudge({ destination, daysToGo, customerName, portalUrl }) {
  const tpl = TEMPLATES[daysToGo] || TEMPLATES[0];
  const subject = interpolate(tpl.subject, { destination, customerName, portalUrl });
  const text = interpolate(tpl.body, { destination, customerName, portalUrl });
  return {
    subject,
    text,
...
async function buildNudge({ tenantId, destination, daysToGo, customerName, portalUrl }) {
  try {
    ...
    if (result && !result.stub && result.text) {
      ...
      if (parsed && parsed.subject && parsed.body) {
        const subject = interpolate(parsed.subject, { destination, customerName, portalUrl });
        let text = interpolate(parsed.body, { destination, customerName, portalUrl });
        // Append portal link if the LLM body doesn't already include it.
        if (portalUrl && !text.includes(portalUrl)) {
          text += `\n\nView your itinerary: ${portalUrl}`;
        }
  ...
  return buildFallbackNudge({ destination, daysToGo, customerName, portalUrl });
}
