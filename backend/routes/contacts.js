...
function parseContactTags(tagsJson) {
  ...
  try {
    const parsed =
      typeof tagsJson === "string" ? JSON.parse(tagsJson) : tagsJson;
 ...
 */
function workflowContactPayload(
  contact,
  userId,
  changedFields = [],
  previous = null,
) {
  const callifiedStatus = String(
    contact.callifiedLeadStatus || "",
  ).toLowerCase();
  const isJunk = contact.status === "Junk" || callifiedStatus === "junk";
  const isQualified =
    ["Prospect", "Customer"].includes(contact.status) ||
    callifiedStatus === "qualified";
  ...
  return {
    ...
    externalId,
    metaLeadgenId:
      typeof externalId === "string" && externalId.startsWith("meta:")
        ? externalId.slice(5)
        : null,
  ...
  for (const value of values) {
    ...
    if (CONTACT_TAG_CONTROL_RE.test(tag)) {
      return {
        error: {
          status: 400,
          error: "tags contain invalid control characters",
          code: "INVALID_TAGS",
        },
      };
    }
...
function validateContactInput(body, { isUpdate = false } = {}) {
  ...
  if (body.aiScore !== undefined && body.aiScore !== null) {
    const scoreErr = ensureNumberInRange(body.aiScore, {
      min: 0,
      max: 100,
      field: "aiScore",
      code: "INVALID_AISCORE",
    });
    if (scoreErr) return scoreErr;
  }
  // status — keep open enum but reject obvious junk like "C" (importer #154 already does this).
  if (body.status !== undefined && body.status !== null && body.status !== "") {
    const stErr = ensureEnum(
      body.status,
      ["Lead", "Prospect", "Customer", "Churned", "Junk"],
      { field: "status" },
    );
  ...
  if (
    body.treatmentOfInterest !== undefined &&
    body.treatmentOfInterest !== null &&
    body.treatmentOfInterest !== ""
  ) {
    const tErr = ensureStringLength(body.treatmentOfInterest, {
      max: 191,
      field: "treatmentOfInterest",
    });
    if (tErr) return tErr;
  }
  for (const idField of [
    ...
    if (
      ...
      if (!Number.isInteger(v) || v <= 0) {
        return {
          status: 400,
          error: `${idField} must be a positive integer`,
          code: "INVALID_ID",
        };
      }
    }
...
function canReassignLead(req, contact) {
  ...
  return (
    vertical === "travel" &&
    Number(contact.assignedToId) === Number(req.user.userId)
  );
}
...
const FILTERABLE_FIELDS = {
  ...
  callifiedCampaignId: {
    column: "callifiedCampaignId",
    kind: "id",
    label: "Callified Campaign",
  },
  callifiedLeadStatus: {
    column: "callifiedLeadStatus",
    kind: "text",
    label: "Call Status",
  },
...
function buildFilterClause(fieldDef, operator, values) {
  ...
  if (kind === "range") {
    ...
    return operator === "not_contains"
      ? {
          AND: picked.map((b) => ({
            NOT: { [column]: { gte: b.min, lte: b.max } },
          })),
        }
  ...
  if (operator === "not_contains") {
    return {
      AND: list.map((v) => ({ [column]: { not: { contains: String(v) } } })),
    };
  }
...
function buildCustomFieldClause(defId, operator, values, kind = "text") {
        ...
        OR: [
          { leadCustomFieldValues: { none: { fieldId: defId } } },
          {
            leadCustomFieldValues: {
              some: { fieldId: defId, valueDate: null },
            },
          },
        ],
      };
  ...
  if (operator === "is_not_empty") {
    if (isDate) {
      return {
        leadCustomFieldValues: {
          some: { fieldId: defId, valueDate: { not: null } },
        },
      };
    }
    return {
  ...
  if (operator === "between") {
    ...
    return range
      ? {
          leadCustomFieldValues: { some: { fieldId: defId, valueDate: range } },
        }
      : null;
  }
  const list = (values || []).filter(
    ...
    return {
      NOT: {
        leadCustomFieldValues: {
          some: {
            fieldId: defId,
            OR: list.map((v) => ({ valueText: { contains: String(v) } })),
          },
        },
      },
    };
...
function buildWebFormClause(operator, values) {
  ...
  if (operator === "not_contains") {
    return {
      NOT: {
        webFormSubmissions: { some: { webForm: { name: { in: list } } } },
      },
    };
  }
...
async function attachTravelRelationshipTimeline(contact, tenantId, tenantVertical) {
  ...
  try {
    ...
    const existing = Array.isArray(contact.activities)
      ? contact.activities
      : [];
...
async function attachLeadCustomFields(contact, tenantId) {
  ...
  try {
    const [definitions, values] = await Promise.all([
      ...
      prisma.leadCustomFieldValue.findMany({
        where: { tenantId, contactId: contact.id },
      }),
    ]);
    return {
      ...contact,
      customFields: buildCustomFieldsObject(definitions, values),
    };
  } catch (_e) {
...
async function attachLeadCustomFieldsBatch(contacts, tenantId) {
  ...
  try {
    ...
    for (const v of values) {
      if (!valuesByContact.has(v.contactId))
        valuesByContact.set(v.contactId, []);
      valuesByContact.get(v.contactId).push(v);
    }
    return contacts.map((c) => ({
      ...c,
      customFields: buildCustomFieldsObject(
        definitions,
        valuesByContact.get(c.id) || [],
      ),
    }));
  } catch (_e) {
    return contacts.map((c) => ({ ...c, customFields: {} }));
...
async function writeLeadCustomFieldValues(contactId, tenantId, customFields) {
  ...
  try {
    ...
    console.error(
      "[contacts] writeLeadCustomFieldValues failed (non-fatal):",
      e && e.message,
    );
  }
}
...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.query.assignedToId)
      where.assignedToId = parseInt(req.query.assignedToId);
    ...
    if (req.query.filters) {
      ...
      if (parsed.some((f) => f && isCustomField(f.field))) {
        ...
        tenantCustomFieldKinds = new Map(
          defs.map((d) => [
            d.id,
            CUSTOM_FIELD_TYPE_TO_KIND[d.fieldType] || "text",
          ]),
        );
      }
      ...
      for (const f of parsed) {
        if (
          !f ||
          typeof f !== "object" ||
          !FILTER_OPERATORS.includes(f.operator)
        )
          continue;
        ...
        if (isCustomField(f.field)) {
          ...
          const clause = buildCustomFieldClause(
            defId,
            f.operator,
            rawValues,
            tenantCustomFieldKinds.get(defId) || "text",
          );
        ...
        if (fieldDef.verticals && !fieldDef.verticals.includes(vertical))
          continue;
...
router.get("/filter-fields", async (req, res) => {
  try {
    ...
    fields.splice(
      sourceAt >= 0 ? sourceAt + 1 : fields.length,
      0,
      ...webFormField,
    );
  ...
  try {
    ...
    if (isCustomField(req.params.field)) {
      ...
      if (defId === null)
        return res
          .status(404)
          .json({ error: "Unknown filter field", code: "UNKNOWN_FIELD" });
      const def = await prisma.leadCustomFieldDefinition.findFirst({
      ...
      if (!def)
        return res
          ...
          .json({ error: "Unknown filter field", code: "UNKNOWN_FIELD" });
      if (
        ["dropdown", "radio", "multiselect"].includes(def.fieldType) &&
        def.options
      ) {
        const options = JSON.parse(def.options);
        return res.json({
          values: options.map((o) => ({ value: o, label: o })),
        });
      }
      ...
      const rows = await prisma.leadCustomFieldValue.findMany({
        where: {
          ...
          valueText: { not: null },
          ...(statusScope
            ? { contact: { status: statusScope, deletedAt: null } }
            : {}),
        },
    ...
    if (req.params.field === "callifiedCampaignId") {
      const campaigns = await callifiedClient
    ...
    if (req.params.field === "callifiedLeadStatus") {
      ...
      const values = [];
      for (const row of rows) {
        if (row.callifiedLeadStatus == null || row.callifiedLeadStatus === "")
          continue;
    ...
    if (
      fieldDef.required &&
      (fieldDef.kind === "number" || fieldDef.kind === "date")
    ) {
    ...
    const columnPredicate = notNullFilter
      ? { [fieldDef.column]: notNullFilter }
      : {};
    const rows = await prisma.contact.findMany({
...
// colors so every contact renders the same tag color after a reload.
  try {
    ...
    if (!tag) {
      return res
        .status(400)
        .json({ error: "Tag is required", code: "TAG_REQUIRED" });
    }
    if (CONTACT_TAG_CONTROL_RE.test(tag)) {
      return res.status(400).json({
    ...
    for (const row of rows) {
      ...
      const nextTags = currentTags.filter(
        (current) => current.toLowerCase() !== tagKey,
      );
      if (nextTags.length === currentTags.length) continue;
      await prisma.contact.update({
        where: { id: row.id },
        data: {
          tagsJson: nextTags.length > 0 ? JSON.stringify(nextTags) : null,
        },
      });
      updatedContacts += 1;
    }
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!canAccessLead(req, contact))
      return res.status(404).json({ error: "Contact not found" });
    // #167: 404 soft-deleted rows unless caller opts in.
    if (contact.deletedAt && !includeDeleted)
      return res.status(404).json({ error: "Contact not found" });
    // #464: strip read-restricted fields per the caller's role.
    const filtered = await filterReadFields(
      contact,
    ...
    const withWallet = await attachComputedWalletBalance(
      filtered,
      req.user.tenantId,
    );
    let tenantVertical = "";
    ...
    const withCustomFields = await attachLeadCustomFields(
      withTimeline,
      req.user.tenantId,
    );
...
router.post("/", async (req, res) => {
  try {
    ...
    if (tagsResult.error)
      return res.status(tagsResult.error.status).json(tagsResult.error);
    ...
    if (
      typeof normalised.anniversary === "string" &&
      normalised.anniversary !== ""
    ) {
      normalised.anniversary = new Date(normalised.anniversary);
    }
    if (
      typeof normalised.birthDate === "string" &&
      normalised.birthDate !== ""
    ) {
    ...
    const isGenericLeadAwaitingCall =
      ...
      (!normalised.callifiedLeadStatus ||
        normalised.callifiedLeadStatus === "yet_to_call");
    const shouldSkipInitialAssignee =
      isGenericLeadAwaitingCall ||
      (skipInitialAssignee && normalised.status === "Lead");
    ...
    if (
      normalised.status === "Lead" &&
      normalised.callifiedCampaignId == null
    ) {
      try {
        ...
        console.error(
          "[contacts] auto-campaign rule evaluation failed:",
          e.message,
        );
      }
    }
    ...
    if (!contact) {
      contact = await prisma.contact.create({
        data: { ...normalised, tenantId: req.user.tenantId },
      });
    }
    ...
    try {
      ...
      await emitEvent(
        "contact.created",
        workflowContactPayload(contact, req.user.userId),
        req.user.tenantId,
        req.io,
      );
    } catch (_e) {
    ...
    if (
      contact.status === "Lead" &&
      contact.callifiedCampaignId &&
      contact.phone
    ) {
      try {
        const autoDialEnabled = await getSetting(
          contact.tenantId,
          KEYS.CALLIFIED_AUTO_DIAL_NEW_LEADS_ENABLED,
          {
...
router.put("/bulk-assign", verifyRole(["ADMIN"]), async (req, res) => {
  try {
    ...
    if (nextAssignedToId) {
      ...
      assignableRows = rows.filter(
        (r) => !r.subBrand || canAccessSubBrand(allowed, r.subBrand),
      );
    ...
    if (nextAssignedToId) {
      ...
      for (const row of assignableRows) {
        ...
        try {
          ...
          console.error(
            "[contacts] bulk lead assignment notify failed:",
...
router.put("/bulk-assign-campaign", verifyRole(["ADMIN"]), async (req, res) => {
  try {
    ...
    const ids = contactIds.map((id) => parseInt(id)).filter(Number.isFinite);
    const nextCampaignId = callifiedCampaignId
      ? parseInt(callifiedCampaignId)
      : null;

    if (
      nextCampaignId !== null &&
      (!Number.isInteger(nextCampaignId) || nextCampaignId <= 0)
    ) {
      return res
        .status(400)
        .json({ error: "Invalid campaign ID", code: "INVALID_CAMPAIGN_ID" });
    }
...
router.delete("/bulk-delete", verifyRole(["ADMIN"]), async (req, res) => {
  try {
    ...
    const ids = [
      ...new Set(contactIds.map((id) => parseInt(id)).filter(Number.isFinite)),
    ];
    if (ids.length === 0) {
      return res.status(400).json({
...
router.get("/:id/activities", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(id))
      return res
        .status(400)
        .json({ error: "Invalid contact ID", code: "INVALID_ID" });
    const contact = await prisma.contact.findFirst({
      where: { id, tenantId: req.user.tenantId },
      select: { id: true, status: true, assignedToId: true },
    });
    if (!contact || !canAccessLead(req, contact))
      return res.status(404).json({ error: "Contact not found" });
...
router.post("/:id/activities", async (req, res) => {
  try {
    ...
    try {
      await markFirstResponseIfNeeded({ contactId: contact.id });
    } catch (_e) {
      /* ignore */
    }
...
router.post("/:id/summarize-chat", async (req, res) => {
  try {
    ...
    if (result.skipped === "no-messages") {
      return res.status(409).json({
        error: "No WhatsApp messages found for this contact yet.",
        code: "NO_WHATSAPP_HISTORY",
      });
    }
    res.json(result);
  } catch (e) {
...
router.post("/:id/resummarize-capture", async (req, res) => {
  try {
    ...
    if (result.skipped === "no-description") {
      return res.status(409).json({
        error: "No captured history found for this contact yet.",
        code: "NO_CAPTURE_HISTORY",
      });
    }
    res.json(result);
  } catch (e) {
    ...
    res.status(500).json({
      error: "Failed to consolidate summary",
      code: "RESUMMARIZE_CAPTURE_FAILED",
    });
  }
});
...
const updateContactById = async (req, res) => {
  try {
    ...
    if (req.body.callifiedCampaignId === "")
      req.body.callifiedCampaignId = null;
    ...
    if (tagsResult.hasValue) {
      updateData.tagsJson = tagsResult.tags.length
        ? JSON.stringify(tagsResult.tags)
        : null;
    }
    ...
    if (
      ...
      (Object.keys(updateData).length > 0 ||
        hasCustomFieldUpdates ||
        tagsResult.hasValue)
    ) {
      updateData.updatedAt = new Date();
    }
    if (
      typeof updateData.anniversary === "string" &&
      updateData.anniversary !== ""
    ) {
      updateData.anniversary = new Date(updateData.anniversary);
    }
    if (
      typeof updateData.birthDate === "string" &&
    ...
    try {
      require("../lib/eventBus")
        ...
        .catch((error) =>
          console.error(
            "[contacts] contact.updated workflow failed:",
            error.message,
          ),
        );
    } catch (_e) {}
    ...
    try {
      if (existing.status !== "Customer" && contact.status === "Customer") {
        const tenant = await prisma.tenant.findUnique({
          where: { id: req.user.tenantId },
          select: { vertical: true },
        });
        if (tenant && tenant.vertical === "wellness") {
          ...
          if (!patient && contact.phone) {
            ...
            const last10 = normalizedContactPhone
              ? normalizedContactPhone.slice(-10)
              : "";
            if (last10.length === 10) {
      ...
      console.error(
        "[contacts PUT] wellness Patient backfill failed:",
        e && e.message,
      );
    }
    ...
    for (let i = 0; i < contacts.length; i++) {
      ...
      try {
        ...
        if (!ALLOWED_STATUSES.has(status)) {
          errors.push(
            `Row ${rowNum}: invalid status "${status}" (allowed: ${[...ALLOWED_STATUSES].join(", ")})`,
          );
          continue;
        }
        ...
        errors.push(
          `Row ${rowNum} (${row.email || "no email"}): ${rowErr.message}`,
        );
      }
    }
...
router.put("/:id/assign", async (req, res) => {
  try {
    ...
    if (!canReassignLead(req, existing)) {
      if (
        (req.user?.vertical || "generic") !== "travel" ||
        req.user?.role === "ADMIN"
      ) {
        return res
          .status(403)
          .json({ error: RBAC_DENIED_MESSAGE, code: RBAC_DENIED_CODE });
      }
      return res.status(404).json({ error: "Contact not found" });
    }
    ...
    if (assignedToId && !Number.isInteger(nextAssignedToId)) {
      return res
        .status(400)
        .json({ error: "Invalid staff member", code: "INVALID_ASSIGNEE" });
    }
    const targetUser =
    ...
    if (nextAssignedToId !== null && !targetUser) {
      return res
        .status(404)
        .json({ error: "Staff member not found", code: "ASSIGNEE_NOT_FOUND" });
    }
    if (
      (req.user?.vertical || "generic") === "travel" &&
      req.user?.role !== "ADMIN" &&
    ...
    if (existing.subBrand && nextAssignedToId) {
      ...
      if (!canAccessSubBrand(allowed, existing.subBrand)) {
        return res.status(403).json({
          error:
            "That staff member doesn't have access to this lead's sub-brand",
          code: "SUB_BRAND_ASSIGN_DENIED",
        });
      }
    }
    ...
    if (nextAssignedToId && nextAssignedToId !== existing.assignedToId) {
      try {
        ...
        console.error(
          "[contacts] lead assignment notify failed:",
          notifyErr && notifyErr.message,
        );
      }
    }
...
router.get("/duplicates/find", async (req, res) => {
  try {
    ...
    const annotated = dupes
      .map((g) => ({
        ...g,
        groupKey: computeDuplicateGroupKey(
          g.primary.id,
    ...
    res.status(500).json({
      error:
        "We couldn't check for duplicate contacts right now. Please try again in a moment.",
...
router.post("/merge", async (req, res) => {
  try {
    ...
    if (
      !primaryId ||
      !Array.isArray(secondaryIds) ||
      secondaryIds.length === 0
    ) {
      return res
        .status(400)
        .json({ error: "primaryId and secondaryIds required" });
    }
    const tenantId = req.user.tenantId;
    const userId = req.user?.userId || null;
    const pid = parseInt(primaryId);
    if (!Number.isFinite(pid))
      return res.status(400).json({ error: "Invalid primaryId" });
    ...
    if (secondaries.length === 0)
      return res.status(404).json({ error: "No mergeable secondaries" });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const sec of secondaries) {
        if (!primary.phone && !updates.phone && sec.phone)
          updates.phone = sec.phone;
        if (!primary.company && !updates.company && sec.company)
          updates.company = sec.company;
        if (!primary.title && !updates.title && sec.title)
          updates.title = sec.title;
        if ((sec.aiScore || 0) > (primary.aiScore || 0))
          updates.aiScore = sec.aiScore;
      }
...
router.post("/duplicates/dismiss", async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    const userId = req.user?.userId || null;
    let { groupKey, primaryId, secondaryIds, contactIds, reason } =
      req.body || {};
    ...
    if (Array.isArray(contactIds) && contactIds.length > 0) {
      ...
      ids = [Number(primaryId), ...secondaryIds.map(Number)].filter(
        Number.isFinite,
      );
    }
    ...
    if (ids.length > 0) {
      ...
      if (found.length !== ids.length) {
        return res
          .status(404)
          .json({ error: "One or more contacts not found in tenant" });
      }
    ...
    if (existing) {
      return res.json({
        ...
        idempotent: true,
        groupKey,
        dismissedAt: existing.createdAt,
      });
    }
    ...
    await writeAudit("Contact", "DUPLICATE_DISMISS", null, userId, tenantId, {
      groupKey,
      contactIds: ids,
    });
...
router.get("/:id/attachments", async (req, res) => {
  try {
    ...
    if (!canAccessLead(req, contact))
      return res.status(404).json({ error: "Contact not found" });
    res.json(
      await prisma.contactAttachment.findMany({
        where: { contactId: contact.id, tenantId: req.user.tenantId },
        orderBy: { createdAt: "desc" },
      }),
    );
  } catch (_err) {
...
router.post("/:id/attachments", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(contactId)) {
      return res
        .status(400)
        .json({ error: "Invalid contact id", code: "INVALID_ID", field: "id" });
    }
    const contact = await prisma.contact.findFirst({
    ...
    if (ctype.startsWith("multipart/form-data")) {
      return res.status(400).json({
        error:
          "Multipart upload not supported on this endpoint. POST application/json with {filename, fileUrl}.",
...
router.delete("/attachments/:attachId", async (req, res) => {
  try {
    const existing = await prisma.contactAttachment.findFirst({
      where: { id: parseInt(req.params.attachId), tenantId: req.user.tenantId },
    });
    if (!existing)
