...
function isGenericLandingSite(page) {
  ...
  return Boolean(
    ...
    page && typeof page.templateType === "string" && page.templateType.startsWith(GENERIC_LANDING_SITE_PREFIX)
...
function decoratePublishedPublicPayload(page, parsedContent) {
  ...
  if (parsedContent && typeof parsedContent === "object" && !Array.isArray(parsedContent)) {
    decoratedContent = JSON.parse(JSON.stringify(parsedContent));
...
// Full featured-page payload for external hosts that need the published
// landing page config object (theme/brand/meta/sections) in one public
// response. Keeps the existing lightweight /public/featured contract
// untouched so current consumers do not start downloading heavy content.
router.get("/public/featured-full", async (req, res) => {
  ...
  try {
    ...
    console.error("[LandingPages] public/featured-full error:", err);

    return res.status(500).json({ error: "Failed to resolve full featured page" });
...
// Slug-targeted version of /public/featured-full. External sites can fetch
// the exact published page payload they want without needing an internal ID.
...
router.post("/:id/publish", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Wellness landing sites keep a single published page at a time so the

    // list view stays clear and the public landing-site URL always has one

    // authoritative draft-to-live path. Travel pages continue to use their

    // existing publish/feature workflow untouched.

    if (isGenericLandingSite(existing)) {

      const currentPublishedLandingSite = await prisma.landingPage.findFirst({

        where: {

          tenantId: req.user.tenantId,

          templateType: { startsWith: GENERIC_LANDING_SITE_PREFIX },
      ...
      if (currentPublishedLandingSite) {

        return res.status(409).json({

          error: `Only one published landing site is allowed at a time. Unpublish "${currentPublishedLandingSite.title}" first.`,

          code: "ANOTHER_LANDING_SITE_PUBLISHED",

          currentPublishedLandingSite,
