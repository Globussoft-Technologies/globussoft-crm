...
// /pricing card (currency toggle Aa annual/monthly toggle). Falls back to the
      ...
      try {
        ...
        if (bucket && bucket[per] != null) {
          ...
          if (!Number.isNaN(parsedAmount) && parsedAmount > 0) {
            ...
            // Charge the full annual total (per-month Aa 12).
...
router.get('/:id/invoice.pdf', verifyToken, verifyRole(['ADMIN']), async (req, res) => {
  try {
    ...
    applyRupeeCapableFonts(doc); // rupee glyph fix
