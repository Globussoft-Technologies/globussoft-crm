...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (ALLOWED_FILE_MIMES.has(String(file.mimetype || "").toLowerCase())) return cb(null, true);
...
const FIELD_TYPES = new Set(["text", "textarea", "number", "dropdown", "radio", "date", "url", "checkbox", "multiselect", "file"]);

const CHOICE_TYPES = new Set(["dropdown", "radio", "multiselect"]);

const FILE_FORMATS = new Set(["CSV", "XLSX", "JPG", "JPEG", "PNG", "PDF", "DOCX", "PPTX", "TXT", "WEBP"]);

const DEFAULT_FILE_FORMATS = ["CSV", "XLSX", "JPG", "JPEG", "PNG", "PDF", "DOCX", "PPTX"];

const CONTACT_FIELDS = new Set(["name", "email", "phone", "company", "title", "source", "status"]);
...
function parseOptions(raw) {
  ...
  return parsed.map((item) => textOr(item)).filter(Boolean).slice(0, 50);
...
function parseFileFormats(raw) {
  ...
  const clean = items.map((item) => textOr(item).toUpperCase()).filter((item) => FILE_FORMATS.has(item));
...
function normalizeField(field, index) {
  ...
  const sourceKind = ["contact", "lead_custom", "custom"].includes(field?.sourceKind) ? field.sourceKind : "custom";

  const fieldType = FIELD_TYPES.has(field?.fieldType) ? field.fieldType : "text";
  ...
  return {
    ...
    fileFormats: fieldType === "file" ? parseFileFormats(field?.fileFormats) : [],

    allowMultipleFiles: fieldType === "file" ? Boolean(field?.allowMultipleFiles) : false,
...
function normalizeSettings(raw) {
  ...
  return {
    ...
    submitButtonLabel: textOr(settings.submitButtonLabel, defaultSettings().submitButtonLabel),

    successMessage: textOr(settings.successMessage, defaultSettings().successMessage),
...
function coerceCustomFieldValue(def, raw) {
  ...
  if (def.fieldType === "checkbox") {
    ...
    return { valueBool: raw === true || raw === "true" || raw === "1" || raw === "on" };
...
async function writeLeadCustomFieldValues(contactId, tenantId, customFieldValues) {
...
async function listWithCounts(tenantId) {
  ...
  if (ids.length) {
    ...
    counts = Object.fromEntries(grouped.map((row) => [row.webFormId, row._count._all]));
...
router.get("/public/:slug", async (req, res) => {
  ...
  try {
    ...
    const form = await prisma.webForm.findFirst({ where: { slug, isActive: true } });
  ...
  try {
    ...
    for (const field of fields) {
      ...
      if (field.fieldType === "file") {
        ...
        const fieldFiles = files.filter((file) => file.fieldname === field.sourceKey);

        if (field.required && fieldFiles.length === 0) missing.push(field.label);
      ...
      const raw = field.hidden ? field.defaultValue : readBodyValue(body, field.sourceKey);

      payload[field.sourceKey] = raw == null ? null : raw;

      if (field.required && !isTruthyValue(field.fieldType, raw)) missing.push(field.label);

      if (!CONTACT_FIELDS.has(field.sourceKey) && field.sourceKind === "lead_custom") {
      ...
      if (field.sourceKind === "contact") {
        ...
        if (field.sourceKey === "name") contactData.name = textOr(raw, contactData.name);

        else if (field.sourceKey === "email") contactData.email = textOr(raw);

        else if (field.sourceKey === "phone") contactData.phone = textOr(raw);

        else if (field.sourceKey === "company") contactData.company = textOr(raw);

        else if (field.sourceKey === "title") contactData.title = textOr(raw);

        else if (field.sourceKey === "source") contactData.source = textOr(raw, contactData.source);

        else if (field.sourceKey === "status") contactData.status = textOr(raw, contactData.status);
          ...
          if (Number.isInteger(assignedToId) && assignedToId > 0) contactData.assignedToId = assignedToId;

        } else if (field.sourceKey === "industry") contactData.industry = textOr(raw) || null;

        else if (field.sourceKey === "companySize") contactData.companySize = textOr(raw) || null;

        else if (field.sourceKey === "linkedin") contactData.linkedin = textOr(raw) || null;

        else if (field.sourceKey === "website") contactData.website = textOr(raw) || null;

        else if (field.sourceKey === "firstTouchSource") contactData.firstTouchSource = textOr(raw) || null;

        else if (field.sourceKey === "lastTouchSource") contactData.lastTouchSource = textOr(raw) || null;

        else if (field.sourceKey === "subBrand") contactData.subBrand = textOr(raw) || null;

        else if (field.sourceKey === "treatmentOfInterest") contactData.treatmentOfInterest = textOr(raw) || null;

        else if (field.sourceKey === "stateCode") contactData.stateCode = textOr(raw) || null;

        else if (field.sourceKey === "billingStateCode") contactData.billingStateCode = textOr(raw) || null;

        else if (field.sourceKey === "gst") contactData.gst = textOr(raw) || null;

        else if (field.sourceKey === "birthDate") {

          const birthDate = new Date(raw);

          if (!Number.isNaN(birthDate.getTime())) contactData.birthDate = birthDate;

        } else if (field.sourceKey === "anniversary") {

          const anniversary = new Date(raw);

          if (!Number.isNaN(anniversary.getTime())) contactData.anniversary = anniversary;
    ...
    contactData.name = textOr(contactData.name, contactData.email || contactData.phone || form.name || "Web form lead");
    ...
    if (Object.keys(customFieldValues).length) {
      ...
      await writeLeadCustomFieldValues(contact.id, form.tenantId, customFieldValues);
...
router.post("/", verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json(shapeForm(form, 0, `${req.protocol}://${req.get("host")}`));
...
router.get("/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const form = await prisma.webForm.findFirst({ where: { id, tenantId: req.user.tenantId } });
...
router.put("/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const existing = await prisma.webForm.findFirst({ where: { id, tenantId: req.user.tenantId } });

    if (!existing) return res.status(404).json({ error: "Form not found", code: "FORM_NOT_FOUND" });
    ...
    if (body.description !== undefined) data.description = textOr(body.description);

    if (body.isActive !== undefined) data.isActive = Boolean(body.isActive);

    if (body.fields !== undefined) data.fieldsJson = JSON.stringify(normalizeFields(body.fields));

    if (body.style !== undefined) data.styleJson = JSON.stringify(normalizeStyle(body.style));

    if (body.settings !== undefined) data.settingsJson = JSON.stringify(normalizeSettings(body.settings));



    const updated = await prisma.webForm.update({ where: { id: existing.id }, data });

    res.json(shapeForm(updated, existing.submissionCount || 0, `${req.protocol}://${req.get("host")}`));
