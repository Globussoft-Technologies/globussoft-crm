...
async function sendVisitPaymentLink({ visit, tenantId, baseUrl, amount }) {
  ...
  const name = contact.name || patient.name || "there";
  const amountText = Number(visit.amountCharged || 0) > 0 ? ` for INR ${Number(visit.amountCharged).toLocaleString("en-IN")}` : "";
  const text =
    `Hi ${name},\n\n` +
    `Your wellness payment link is ready${amountText}.\n` +
    `Open it here:\n${paymentLink.url}\n\n` +
    `Thank you.`;
  const html =
    `<p>Hi ${name},</p>` +
    `<p>Your wellness payment link is ready${amountText}.</p>` +
    `<p><a href="${paymentLink.url}" target="_blank" rel="noopener noreferrer">Open payment link</a></p>` +
    `<p>Thank you.</p>`;
  ...
  if (contact.email) {
    const emailResult = await sendEmail({
      to: contact.email,
      subject: `Your wellness payment link${amountText}`,
      text,
      html,
    }).catch(() => ({ sent: false }));
    emailStatus = emailResult && emailResult.sent ? "SENT" : "SKIPPED";
    if (emailResult && emailResult.sent) channels.push("email");
  }

  if (contact.phone) {
    const waResult = await waWebClient.sendBestEffort({
      tenantId,
      subBrand: null,
    ...
    whatsappStatus = (waResult && waResult.status) || (waResult && waResult.sent ? "SENT" : "FAILED");
    if (waResult && waResult.sent === true) channels.push("whatsapp");
  }
  ...
  if (channels.length > 0) {
    await createPatientNotification({
      ...
      message: `Your wellness payment link${amountText} is ready. Please check email or WhatsApp.`,
      type: "payment",
      link: paymentLink.url,
    }).catch((err) => {
      console.error("[wellness] patient payment-link notification failed:", err.message);
    });
  }
  ...
  return {
    ...
    channel: channels.length ? channels.join("+") : "none",
  };
}
...
// 00:0005:30 IST visits land on the previous day.
//
...
router.post("/patients/tags", phiWriteGate, async (req, res) => {
  try {
    ...
    if (rawName.length > 60) {
      return res.status(400).json({ error: "name must be 60 chars or fewer" });
    }
    const color =
...
// #108: a phone is optional, but if supplied must contain 1015 digits after
...
function validatePatientInput(body, { isUpdate = false } = {}) {
  ...
  if (!isValidPhoneOrEmpty(body.phone)) {
    return {
      status: 400,
      error: "phone must contain 10 to 15 digits",
      code: "INVALID_PHONE",
    };
  }
...
// Generate/regenerate a hosted payment link and best-effort deliver it to the
// patient over email and WhatsApp. Returns the delivery channels used so the UI
// can toast a concise summary.
router.post("/visits/:id/payment-link/send", phiWriteGate, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    const visit = await prisma.visit.findFirst({
      where: { id, tenantId: req.user.tenantId },
    ...
    console.error("[wellness] visit payment-link send error:", err.message);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Race-safe transition: only flip pendingapproved. `updateMany` with a
  ...
  async (req, res) => {
    try {
      ...
      const [
        ...
        prisma.patient.count({
          where: buildPatientListWhere({
            user: req.user,
            query: locationId ? { locationId: String(locationId) } : {},
          }),
        }),
      ...
      // Rule-based, no ML. Score 0100 per upcoming booked visit; aggregate
...
// upload can't be loaded). The 400121 "-pdf" variant is PDF-safe; the
// original globussoft-logo.png is 21,6186,558 (~567 MB decoded) and would
...
//    COMPLETED Sale.total   REFUNDED Sale.total   (POS, any tender)
//   +  SUCCESS Payment.amount                        (online gateway)
//   +  non-cancelled Subscription.amount             (recurring plans)
...
// stays at the route layer because the CUSTOMER cohort's userpatient
