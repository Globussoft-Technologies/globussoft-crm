// Appointment service Ã¢â‚¬â€ centralized business logic for patient-facing
...
// patient-resolution + envelope-shape adapters only Ã¢â‚¬â€ they MUST NOT add
// validation here.
//
// PATIENT DETECTION (Phase 1 Ã¢â‚¬â€ no dedicated PATIENT role)
//
...
//   Ã¢â‚¬Â¢ { type: 'user', id }    Ã¢â‚¬â€ staff or CUSTOMER session
//   Ã¢â‚¬Â¢ { type: 'patient', id } Ã¢â‚¬â€ phone+OTP portal session
//
...
// Visit statuses considered "active" Ã¢â‚¬â€ these block slot conflicts and
...
// Audit helpers Ã¢â‚¬â€ patient-actor audits set actorType=patient + patientId.
...
// computed server-side so the UI doesn't re-derive them Ã¢â‚¬â€ single source
...
// Slot-conflict probe Ã¢â‚¬â€ returns the colliding visit (if any) for the
...
// (the service does NOT auto-create patients Ã¢â‚¬â€ callers must pass an
...
async function cancelAppointment({ tenantId, patientId, visitId, actor }) {
  ...
  if (visit.status === 'cancelled') {
    // Idempotent Ã¢â‚¬â€ already cancelled, return current state without audit churn.
...
// arrived with "No preference Ã¢â‚¬â€ admin will assign". The admin picks a
...
// PUT /visits/:id instead of this endpoint Ã¢â‚¬â€ that path already exists
// and has its own audit trail. This endpoint is specifically for the
// pending Ã¢â€ â€™ assigned transition.
async function assignDoctor({ tenantId, visitId, doctorId, actor }) {
  ...
  if (visit.doctorId) {
    // Prevent silent overwrite of an existing doctor Ã¢â‚¬â€ reassignment goes
...
// an array of Visit rows (with relations) Ã¢â‚¬â€ the route layer maps each
...
async function listPatientAppointments({ tenantId, patientId, bucket = 'upcoming' }) {
  ...
  if (bucket === 'upcoming') {
    ...
    // must surface somewhere Ã¢â‚¬â€ otherwise it falls out of the patient's
