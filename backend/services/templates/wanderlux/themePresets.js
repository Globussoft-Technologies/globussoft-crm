...
function sanitizeWanderluxThemeOverrides(overrides = {}) {
  const source = (overrides && typeof overrides === 'object') ? overrides : {};
  const clean = {};

  for (const key of THEME_COLOR_FIELDS) {
    const value = normalizeThemeColor(source[key], null);
    if (value) clean[key] = value;
  }

  for (const key of ['serifFont', 'sansFont', 'pattern']) {
    if (typeof source[key] === 'string' && source[key].trim()) {
      clean[key] = source[key].trim();
    }
  }
...
function mergeWanderluxThemeOverrides(baseTheme = {}, overrides = {}) {
  return {
    ...cloneTheme(baseTheme),
    ...sanitizeWanderluxThemeOverrides(overrides),
  };
}
...
function buildPresetChoice(preset) {
  return {
    id: preset.id,
    label: preset.label,
...
function getStaticThemeChoice(themeId) {
  ...
  return found ? buildPresetChoice(found) : null;
}

function listWanderluxThemePresets(subBrand) {
  return [
    buildBrandDefaultChoice(subBrand),
    ...STATIC_THEME_PRESETS.map(buildPresetChoice),
  ];
}

function resolveWanderluxThemePreset(input = {}) {
  ...
  if (destinationMatch) return buildPresetChoice(destinationMatch);

  return buildBrandDefaultChoice(input.subBrand);
}

module.exports = {
  WANDERLUX_THEME_PRESETS: STATIC_THEME_PRESETS,
  THEME_COLOR_FIELDS,
  listWanderluxThemePresets,
  sanitizeWanderluxThemeOverrides,
  mergeWanderluxThemeOverrides,
  resolveWanderluxThemePreset,
  getWanderluxThemePreset(themeId, input = {}) {
    return resolveWanderluxThemePreset({ themeId, ...input });
  },
};
