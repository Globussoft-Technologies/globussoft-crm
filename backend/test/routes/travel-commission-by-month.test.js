// @ts-check
/**
 * PRD_TRAVEL_B2B_AGENT_PORTAL #905 slice 19 — tenant-wide commission
 * monthly rollup.
 *
 * Pins GET /api/travel/commission-profiles/by-month which aggregates
 ...
 * Deal × Contact join by Deal.createdAt's UTC YYYY-MM. One row per
 * month present, with profileCount / dealCount / totalSale /
 * totalCommission aggregated across every contributing profile.
 *
 * Mirrors slice 16 (/quotes/by-month) + slice 29 (/invoices/by-month)
 * for shape + orderBy semantics, and slice 15 (/:id/summary/by-month)
 * for the per-profile computeCommission pattern lifted to tenant scope.
 *
 * What's pinned
 * -------------
 *   - GET /api/travel/commission-profiles/by-month
 *       1. INVALID_MONTH_FORMAT for from=2026-13
 *       2. happy path: 2 profiles × 4 deals → correct per-month rollup
 *          (UTC YYYY-MM bucketing, sum-up across profiles)
 *       3. orderBy=totalCommission:desc sorts months by commission DESC
 *       4. from/to single-month window filters out other months
 ...
 *       7. pagination via limit + offset (totalMonths reflects full
 *          filtered population, not page window)
 *       8. no token → 401
 *
 * Test pattern mirrors travel-commission-stats.test.js — patch prisma
...
// March 2026:
//   - Deal 101 → profile 31 (flat 5%): 100000 → commission 5000
//   - Deal 102 → profile 31: 50000 → commission 2500
//   - Deal 103 → profile 32 (tiered): 80000 → 80000 * 0.04 = 3200
// April 2026:
//   - Deal 104 → profile 32 (tiered): 200000 → 100000*0.04 + 100000*0.06 = 10000
//
// Per-month expected:
//   2026-03: profileCount=2, dealCount=3, totalSale=230000, totalCommission=10700
//   2026-04: profileCount=1, dealCount=1, totalSale=200000, totalCommission=10000
const dealsHappy = [
  {
    id: 101,
    amount: 100000,
...
describe('GET /api/travel/commission-profiles/by-month (slice 19)', () => {
  test('INVALID_MONTH_FORMAT for from=2026-13', async () => {
    ...
    const res = await request(makeApp())
      .get('/api/travel/commission-profiles/by-month?from=2026-13')
  ...
  test('happy path: 2 profiles × 4 deals → correct per-month rollup', async () => {
    prisma.travelCommissionProfile.findMany.mockResolvedValue([
    ...
    // Default sort is month:asc, so 2026-03 comes first.
    const march = res.body.months[0];
    expect(march.month).toBe('2026-03');
    expect(march.profileCount).toBe(2);
    expect(march.dealCount).toBe(3);
    expect(march.totalSale).toBe(230000);
    expect(march.totalCommission).toBe(10700);

    const april = res.body.months[1];
    expect(april.month).toBe('2026-04');
    expect(april.profileCount).toBe(1);
    expect(april.dealCount).toBe(1);
    expect(april.totalSale).toBe(200000);
    expect(april.totalCommission).toBe(10000);

    // Grand totals: distinct profiles across all months = 2.
  ...
  test('orderBy=totalCommission:desc sorts months by commission DESC', async () => {
    prisma.travelCommissionProfile.findMany.mockResolvedValue([
      profileFlatTmc, profileTieredRfu,
    ]);
    prisma.travelCommissionProfile.count.mockResolvedValue(2);
    ...
    const res = await request(makeApp())
      .get('/api/travel/commission-profiles/by-month?orderBy=totalCommission:desc')
      .set('Authorization', `Bearer ${tokenFor('ADMIN')}`);

    expect(res.status).toBe(200);
    // March has 10700, April has 10000 → March first under desc sort.
    expect(res.body.months[0].month).toBe('2026-03');
    expect(res.body.months[0].totalCommission).toBe(10700);
    expect(res.body.months[1].month).toBe('2026-04');
    expect(res.body.months[1].totalCommission).toBe(10000);
  });

  test('from/to single-month window: only that month appears', async () => {
    prisma.travelCommissionProfile.findMany.mockResolvedValue([
      profileFlatTmc, profileTieredRfu,
    ]);
    prisma.travelCommissionProfile.count.mockResolvedValue(2);
    ...
    const res = await request(makeApp())
      .get('/api/travel/commission-profiles/by-month?from=2026-04&to=2026-04')
      .set('Authorization', `Bearer ${tokenFor('ADMIN')}`);
  ...
  test('malformed profileJson on one profile → contributes 0; other still correct', async () => {
    ...
    expect(res.status).toBe(200);
    // March: broken profile's 2 deals (100000 + 50000) contribute 0 commission
    //        + tiered profile's 1 deal (80000) contributes 3200.
    //        totalSale still 230000 (broken profile's deals counted), commission only 3200.
    const march = res.body.months.find((m) => m.month === '2026-03');
    expect(march.dealCount).toBe(3);
    expect(march.totalSale).toBe(230000);
    expect(march.totalCommission).toBe(3200);

    // April: only tiered profile's deal → unchanged at 10000.
    const april = res.body.months.find((m) => m.month === '2026-04');
    expect(april.totalCommission).toBe(10000);
  });
  ...
  test('MANAGER subBrandAccess=["rfu"] → only rfu (and tenant-wide) profiles in rollup', async () => {
    ...
    // March 3200 + April 10000 = 13200 across the two months.
    expect(res.body.grandTotalCommission).toBe(13200);
  });

  test('pagination: limit=1&offset=1 returns the second month only', async () => {
    prisma.travelCommissionProfile.findMany.mockResolvedValue([
      profileFlatTmc, profileTieredRfu,
    ]);
    prisma.travelCommissionProfile.count.mockResolvedValue(2);
    ...
    const res = await request(makeApp())
      .get('/api/travel/commission-profiles/by-month?limit=1&offset=1')
      .set('Authorization', `Bearer ${tokenFor('ADMIN')}`);

    expect(res.status).toBe(200);
    // totalMonths is the full filtered population, NOT the page slice.
    expect(res.body.totalMonths).toBe(2);
    expect(res.body.months).toHaveLength(1);
    // Default sort month:asc → offset=1 gives the second month (April).
    expect(res.body.months[0].month).toBe('2026-04');
