// @ts-check
/**
 * PRD_TRAVEL_RFU §3 — GET /api/travel/rfu-profiles/by-year
 * tenant-wide RFU pilgrim profile annual rollup.
 *
 * Completes the /rfu-profiles/stats + /by-month + /by-quarter + /by-year
 * triplet — same UTC bucketing template applied at year granularity
 * (YYYY). Mirrors /itineraries/by-year + /suppliers/by-year — same
 ...
 *   - Happy path: 3 profiles across 2 years → 2 year rows,
 *     correct counts, default year:asc ordering
 ...
 *
 * Test pattern mirrors travel-rfu-profiles-by-quarter.test.js — patch
...
// Spread of profiles across 2025 + 2026.
//   2025: 2 profiles
//   2026: 1 profile
const baseRows = [
  { createdAt: new Date('2025-04-10T08:00:00Z') },
  { createdAt: new Date('2025-11-17T10:30:00Z') },
...
describe('GET /api/travel/rfu-profiles/by-year', () => {
  test('401 when no Authorization header (verifyToken gate)', async () => {
    const res = await request(makeApp()).get('/api/travel/rfu-profiles/by-year');
  ...
  test('400 INVALID_YEAR_FORMAT on bad ?from (e.g. YYYY-MM)', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?from=2026-01')
  ...
  test('400 INVALID_YEAR_FORMAT on bad ?to (3-digit form)', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?to=202')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
  ...
  test('happy path: 3 profiles across 2 years → 2 year rows, correct counts', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
    ...
    expect(res.body.rows[0]).toMatchObject({ year: '2025', count: 2 });
    expect(res.body.rows[1]).toMatchObject({ year: '2026', count: 1 });
  ...
  test('?orderBy=count:desc flips the ordering (busier year first)', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?orderBy=count:desc')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
  ...
  test('?from=2026&to=2026 narrows the bucket array to a single year', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?from=2026&to=2026')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
  ...
  test('defensive: null createdAt → "unknown" bucket (kept when no from/to set)', async () => {
    ...
    expect(res.status).toBe(200);
    // 1 row in 2025 + 2 in "unknown" → 2 buckets total.
    expect(res.body.total).toBe(2);
  ...
  test('defensive: "unknown" bucket EXCLUDED when ?from / ?to is set', async () => {
    ...
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?from=2020')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
  ...
  test('pagination: ?limit=1&offset=1 slices AFTER aggregation (returns 2nd of 2 year buckets)', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?limit=1&offset=1')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
  ...
  test('unknown ?orderBy token degrades silently to year:asc default', async () => {
    const res = await request(makeApp())
      .get('/api/travel/rfu-profiles/by-year?orderBy=bogus:desc')
      .set('Authorization', `Bearer ${tokenFor('USER')}`);
