...
> **Status update 2026-05-03 (v3.4.3):** **17 gate specs shipped** — G-1..G-16 + G-24 + G-25 (~+580 API tests + 103 vitest unit tests; per-push gate is now **50 specs / ~1,665 API tests + 30 vitest files / 803 unit tests = ~2,468 per-push**). **Four admin-gated cron-trigger endpoints + one for marketing campaigns + three for admin backup** added across the engine batch. **Six compliance items closed** (#408 audit role guard, #409 integrations toggle, #410 recurring-invoice VOID/VOIDED, #411 retention no-op AuditLog, plus the new `confirmDestructive` guard + per-deletion AuditLog in GDPR retention). **Bonus infra fix**: `vitest.config.js` `cron/` `deps.inline` gap was silently blocking ALL cron-engine unit tests — the engine-fixes agent added it as a side effect. **Outstanding contract-drift findings worth filing as separate issues**: Campaign `global._campaignSchedules` in-memory map (silent data loss on restart); 49 models with `tenantId` but no formal `tenant Tenant @relation` (cascade leak on `Tenant.delete()`); `MarketplaceLead.@@unique([provider, externalLeadId])` doesn't include `tenantId`. **Next pickup recommendations (G-17 → G-24 still open):** **G-20 tenant-isolation-api spec** (2-3 days) is the highest-severity multi-day pickup; then **G-17/G-18/G-19** wellness.js route split (1 day each — best after G-20 since the isolation contract should be locked down before structural changes); **G-21 frontend vitest+RTL** (3-5 days), **G-22 msw/nock integration tier**, **G-23 migration-safety check** still open.
