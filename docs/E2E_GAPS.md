...
> **Status update 2026-05-04 (v3.4.7):** Only **G-21 frontend vitest+RTL** remains open from this priority backlog. Every other G-XX row from G-1 through G-25 is now ✅ shipped. v3.4.7 added 4 regression-guard test files (`security-token-storage`, `scrubResponse`, `sensitive-field-leak-api`, `tenant-header-ignored-api`, `test-data-patterns`) outside the G-XX backlog — they pin the contract for the QA P0/P1 closures (#426 / #343 / #405 / #427 / #428). Per-push gate is now **~71 specs / ~2,460 API tests + 39 vitest files / 1,093 unit tests = ~3,553 per-push** (+~46% since the v3.4.3 baseline below). Schema-invariants drift counter (G-24's pinned metric): 49 → 39 → 29 → **19** across batches 1+2+3 of #413 + the PlaybookProgress audit. **Next pickup**: G-21 frontend vitest+RTL is the only remaining G-row (3-5 days, multi-day project — needs library/runner-config decisions; not parallel-agent dispatchable). Otherwise the long tail moves to bug-driven work via TODOS.md (#180 JWT revocation, #167 hard-DELETE audit, T2.2 audit middleware build-out for Patient/Visit/Rx/Consent).
>
