...
| **T25** | Deprecate T11's `CataloguePromotionPanel` sub-panel in DiagnosticBuilder.jsx | `frontend/src/pages/travel/DiagnosticBuilder.jsx` (lines 1078-1180) + `frontend/src/__tests__/DiagnosticBuilder.test.jsx` | ⬜ TODO | Flagged by T16 agent: with the dedicated `TmcCatalogueAdmin.jsx` page shipped at `/travel/tmc/catalogue`, the sub-panel inside the EngineWeights tab is now redundant. Replace lines 1078-1180 with a one-line "Open TMC Catalogue Admin →" link to `/travel/tmc/catalogue`. Update the existing DiagnosticBuilder.test.jsx Promote-to-active tests to either delete or pin the new link-only surface. Depends on T16 ✅. **Parallel-safe** with T26 + T27. Est. ~15 min. |
| **T26** | Sidebar discoverability — add TMC Catalogue admin nav entry | `frontend/src/components/Sidebar.jsx` | ⬜ TODO | Flagged by T16 agent: `/travel/tmc/catalogue` has no nav entry; only reachable via URL typing. Travel-vertical sidebar should grow a Travel → TMC → "Catalogue Admin" entry under the existing Travel section. ADMIN+MANAGER visibility (matches the page's auth). Depends on T16 ✅. **Parallel-safe** with T25 + T27. Est. ~10-20 min. |
