...
| **T38** | Update package-lock.json — `magicast@0.3.5` lockfile drift | `backend/package-lock.json` | ✅ DONE 2026-06-08 — `<SHA>` | Flagged by T34 agent: `npm ci` blocks locally on missing `magicast@0.3.5` in lockfile vs. installed deps (likely from a transitive-dep version drift after a recent `npm install`). CI's `npm ci --no-audit --no-fund` works in its container with its own node_modules lifetime. **Fix:** run `cd backend && npm install` once to regenerate the lockfile, commit ONLY `package-lock.json`. **Docs-only friction smell** — not a functional bug, just lets fresh-clone devs use the canonical `npm ci` rather than the looser `npm install`. Depends on T28 ✅. **Parallel-safe** with T37. Est. ~5 min. |
