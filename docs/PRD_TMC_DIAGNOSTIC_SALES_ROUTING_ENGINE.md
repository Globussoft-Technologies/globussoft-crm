...
| **T24** | Fix T8 submit-tmc vitest's missing `prisma.contact.findMany` mock (2 timeouts on main) | `backend/test/routes/travel-diagnostics-readiness-report.test.js` | ⬜ TODO | Flagged by T14 agent during their slice work: `test/routes/travel-diagnostics-readiness-report.test.js` has 2 vitest cases (`happy path → 201` + `suspect lead`) that 5s-timeout on main HEAD. Root cause: T8's `submit-tmc` route added a `prisma.contact.findMany` call for the `priorSubmissionsLast24h` count (rescoped in the T12 fix-up `ad40689e`), but T8's test only mocked `findUnique` + `findFirst` + `create`. The missing mock makes the `findMany` call hang. **Fix:** add `prisma.contact.findMany.mockResolvedValue([])` to the affected test cases' setup. Depends on T8 ✅. **Parallel-safe** with anything (test-only edit). Est. ~30 min. |
