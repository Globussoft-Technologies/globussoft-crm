...
| **T27** | Backend test suite stabilization sweep — 31 pre-existing red `backend/test/routes/*.test.js` files (SCOPING AUDIT slice) | `docs/gaps/backend-test-routes-red-audit.md` (NEW) + `docs/gaps/backend-test-routes-red-raw-2026-06-08.txt` (NEW raw output) | ✅ DONE 2026-06-08 | Scoping audit shipped at [`docs/gaps/backend-test-routes-red-audit.md`](../docs/gaps/backend-test-routes-red-audit.md). **Headline:** of T24's reported 31 reds, **20 are a local-only `npm install` lag** (missing `@aws-sdk/client-s3` — declared in `package.json` since `eba590b8` 2026-05-20, but `node_modules/@aws-sdk/` was empty on the dev box; CI's `npm ci` makes them green there — confirmed against `164305af` + `3ea648f7` + `6a034ebb` recent successful deploys). **Real CI-red surface: 11 files**, all sharing the T24 missing-mock pattern (route grew a Prisma call after the test was written; mock surface didn't catch up; falls through to demo MySQL and 5s-timeouts). Classified into 5 sub-classes (B1 `userRole.findMany` × 7, B2 `tenant.findUnique` × 1, B3 `webhook.findMany` × 1, B4 `travelPaymentSchedule.findFirst`/`findMany` × 1, B5 mixed × 1). Vitest run: `Test Files 31 failed | 338 passed (369)` / `Tests 24 failed | 6316 passed (6340)` / 62.33s wall-clock. Total est. CI sweep cost ~4-5h serial or ~2.5h via 5 parallel agents. Sub-slices T28..T34 appended below. |
| **T28** | Class A — local-env note (docs only; no code) | `README.md` (Getting Started section) | ✅ DONE 2026-06-08 — `621c5896` | Add a one-liner under "Development Setup" reminding fresh-clone devs to run `cd backend && npm install` after pulling — `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` were added in `eba590b8` (2026-05-20). On CI the `npm ci` step covers this; on local dev boxes pulling AFTER a deps-add commit, `npm install` is mandatory before `npx vitest run`. NOT a real test sweep — 20 of T24's "31 reds" disappear with one `npm install`. Est. ~10 min. **Parallel-safe** with T29-T33. |
| **T29** | Class B1 batch — add `prisma.userRole.findMany` mock to 7 test files (POS + drugs) | `backend/test/routes/drugs.test.js` + `backend/test/routes/pos-cashLedger.test.js` + `backend/test/routes/pos-sale-context.test.js` + `backend/test/routes/pos-sale-finalize.test.js` + `backend/test/routes/pos-sales-by-month.test.js` + `backend/test/routes/pos-sales-stats.test.js` + `backend/test/routes/pos-void-refund.test.js` | ✅ DONE 2026-06-08 — `cc7a6dfb` | Recipe applied per the slice spec: added `prisma.userRole = { findMany: vi.fn() }` to each file's singleton-patch mock surface (these 7 files use direct singleton patching, not `vi.mock`) + `prisma.userRole.findMany.mockReset().mockResolvedValue([])` in each file's `beforeEach`. **Audit-spec recipe was incomplete — needed +1 mock per file:** the 6 POS files use `verifyWellnessRole(['admin','manager'], { anyOfPermissions: [...] })`, which on the deny path for non-allowed wellnessRoles calls `getUserPermissions` → `loadUserPermissions`. Empty `userRoles` → `permSet.size === 0` → enters `maybeSelfHealAdminPermissions` (middleware/requirePermission.js:227 → :88-94) which queries `prisma.user.findUnique` — unmocked → falls through to demo MySQL → 5s timeout. Added `prisma.user = prisma.user || {}; prisma.user.findUnique = vi.fn().mockResolvedValue(null)` to ALL 7 files so the self-heal early-exits at the "user not found" return (line 94). drugs.test.js shares the same hazard (writeGate uses anyOfPermissions). **Pre-fix:** 9 timing-out tests across 7 files. **Post-fix:** all 7 files pass, 120 tests in 941ms. No source-code changes. **Recipe-completeness deviation flagged for follow-up:** the original audit at `docs/gaps/backend-test-routes-red-audit.md:78-83` should be updated to include the `prisma.user.findUnique` stub for any test that uses singleton-patching + targets a route with `anyOfPermissions`. **Parallel-safe** with T28 + T30 + T31 + T32 + T33 (file-disjoint). |
| **T30** | Class B2 — add `prisma.tenant.findUnique` mock to `staff.test.js` | `backend/test/routes/staff.test.js` | ✅ DONE 2026-06-08 | Added `prisma.tenant = { findUnique: vi.fn() }` to the file-level singleton-patch mock surface (mirrors the existing `prisma.user` / `prisma.userRole` / `prisma.auditLog` shapes — this file uses direct singleton patching, not `vi.mock`). Added a `beforeEach` in the wellnessRole-enum-validation describe block resolving the lookup to `{ id: 1, vertical: 'generic' }` — generic-branch satisfies both cases identically (`LEGACY_WELLNESS_ROLES` includes "cashier" per DD-5.1, and the error message lists "cashier" for the rejection case) AND avoids needing a second mock surface (`prisma.wellnessRoleType.findFirst`) that the wellness catalog branch would otherwise require for `isCatalogedKey`. **Pre-fix:** 2 timeouts (`accepts "cashier"` + `rejects garbage`) at 5s each — route's `getCallerVertical()` reached the demo MySQL connection string and hung. **Post-fix:** all 24 tests pass in 886ms. No source-code changes. **Parallel-safe** with T28 + T29 + T31 + T32 + T33. |
| **T31** | Class B3 — add `prisma.webhook.findMany` mock + triage dedup-409 assertion mismatch in `contacts.test.js` | `backend/test/routes/contacts.test.js` | ✅ DONE 2026-06-08 — `7d879ec1` | Two-part: (a) Added `webhook: { findMany: vi.fn() }` to mock surface + `prisma.webhook.findMany.mockReset().mockResolvedValue([])` in `beforeEach`. Empty array → `lib/webhookDelivery.js:52` deliverWebhooks short-circuits (no subscribers) so contact.created / contact.updated emits don't fall through to real Prisma. **Pre-fix:** 3 timing-out tests (happy create / happy update / ADMIN delete) at 15.89s wall-clock with `Can't reach database server at 163.227.174.141:3306`. **Post-fix:** all 19 tests pass in 0.86s. (b) Part B triage: the dedup-409 test PASSES on current main as-is. `routes/contacts.js` (lines 295-320) returns 409 BEFORE any audit/emit calls in the dedup branch — and `beforeEach` already resets `writeAuditMock` and `emitEventMock`. The "AssertionError: expected vi.fn() to not be called at all, but actually been called 1 times" cited in the slice description appears to have been a stale snapshot, OR was tied to the timeout-induced state leak (when tests hang on real-DB call, mock state can spill between tests). With Part A's mock added, the dedup test passes cleanly. **Verdict: no Part B action needed.** No source-code changes (route untouched per slice's hard constraint). |
| **T32** | Class B4 — add `travelPaymentSchedule` mock surfaces to `travel-invoice-issue.test.js` | `backend/test/routes/travel-invoice-issue.test.js` | ✅ DONE 2026-06-08 — `<SHA-to-fill>` | Recipe applied: added `prisma.travelPaymentSchedule = { findFirst: vi.fn(), findMany: vi.fn(), create: vi.fn(), createMany: vi.fn() }` to the singleton-patch mock surface (this file uses direct singleton patching, not `vi.mock`) + `prisma.travelPaymentSchedule.findFirst.mockReset().mockResolvedValue({ id: 1 })` in `beforeEach` so the auto-create block (`routes/travel_invoices.js:3908`) short-circuits at the "schedule already exists" branch. **Recipe note for follow-up:** the slice's recipe listed `findFirst` + `findMany` + `create`, but the route actually uses `findFirst` + `createMany` (line 3922) — added `createMany: vi.fn()` for completeness even though the short-circuit makes it unreachable in these 6 tests. **T29-style preemptive mocks NOT needed:** grepped `routes/travel_invoices.js` for `requirePermission` / `userRole.findMany` / `user.findUnique` — zero hits, so the self-heal trap from T29's discovery doesn't apply here. **Pre-fix:** 6 timing-out tests at 5s each (happy TMC / happy RFU / happy travel_stall / sequential 0001→0002 / audit row carry / cross-sub-brand independence) → 30.14s wall-clock. **Post-fix:** all 12 tests pass in 75ms (884ms total incl. transform). No source-code changes. |
| **T33** | Class B5 — fix mixed-class `search.test.js` (needs B1 + B2 mocks) | `backend/test/routes/search.test.js` | ✅ DONE 2026-06-08 — `eec71695` | Recipe applied: added `prisma.tenant = { findUnique: vi.fn() }` + `prisma.patient = { findMany: vi.fn() }` to the singleton-patch mock surface (this file uses direct singleton patching, not `vi.mock`) + `prisma.tenant.findUnique.mockReset().mockResolvedValue({ id: 1, vertical: 'generic' })` + `prisma.patient.findMany.mockReset().mockResolvedValue([])` in `beforeEach`. Generic-vertical default → `canSearchPatients()` returns false → `patient.findMany` is never invoked, but the mock is in place for future wellness-tenant test cases. **Deviation note:** `routes/search.js` does **NOT** mount `requirePermission` (only `verifyToken`) — so B1 mocks (`userRole.findMany` / `user.findUnique`) are NOT actually needed here. The slice spec's "needs B1 + B2 mocks" framing was incorrect for this file; only B2 + the conditional `patient.findMany` were missing. **Pre-fix:** 4-5 tests failing (happy path / tenant-scoped / take-cap / handler error / empty-query call-count bleed-over from prior timeouts) at 15.84s wall-clock with `prisma.tenant.findUnique` hanging on real-DB fall-through to demo MySQL. **Post-fix:** all 8 tests pass in 48ms (844ms total incl. transform). No source-code changes. |
| **T34** | T27 sweep verification — re-run `npx vitest run test/routes/` after T28..T33 land; pin 0 reds | (no files changed — verification only; close out T27 in CHANGELOG) | ✅ DONE 2026-06-08 — `2cd3c15d` | Re-ran `cd backend && npm install && npx vitest run test/routes/` (note: `npm ci` blocked locally on `magicast` lockfile drift — `npm install` covers same surface; CI's `npm ci --no-audit --no-fund` step continues to work on its own). **Verification result:** `Test Files 5 failed | 359 passed | 5 skipped (369)` / `Tests 9 failed | 6492 passed | 75 skipped (6576)` / 42.93s wall-clock. **All 11 T28-T33-targeted files (drugs + 6 POS + staff + contacts + travel-invoice-issue + search) pass 100% green** — the 5 sub-classes (B1 × 7 / B2 × 1 / B3 × 1 / B4 × 1 / B5 × 1) are resolved. The 5 residual failing files (`auth-cookie-set.test.js`, `auth-profile-picture.test.js`, `auth.test.js`, `consent-templates.test.js`, `wellness-patients-xlsx.test.js`) were originally classified as **Class A only** (missing AWS-SDK) by the T27 audit at `docs/gaps/backend-test-routes-red-audit.md:45-68`. After `npm install` fixes Class A, a secondary Class-B-style layer of missing Prisma mocks surfaced (5s timeouts on real-Prisma fall-through to demo MySQL `163.227.174.141:3306`). **These are out-of-scope-for-T27** — tracked as a follow-up T37 (audit-spec residual sweep — re-classify the 5 misclassified Class-A-only files); T35's test-harness guard will catch this residual class structurally rather than per-file. CHANGELOG bumped to `v3.9.4` documenting the full T14-T36 follow-up arc. Depends on T28+T29+T30+T31+T32+T33+T36 ✅. |
| **T35** | Test-harness guard against real-DB fall-through | `backend/test/setup.js` (or similar global test setup) + `backend/test/lib/db-fallthrough-guard.test.js` | ✅ DONE 2026-06-08 — `06c1f8e3` | Implemented Option (a) per the slice spec. Created `backend/test/setup.js` fresh (no prior setup file existed) exporting `dbFallthroughGuard()` which self-runs on module import. Guard gates on `process.env.VITEST === 'true'` (vitest sets this automatically — more reliable than `NODE_ENV === 'test'` since vitest doesn't set NODE_ENV by default), refuses to start when `DATABASE_URL` resolves to a non-local host, and honors `ALLOW_REMOTE_DB_IN_TESTS=1` as the escape hatch for the rare legit integration case. Local-host detection covers `localhost` / `127.0.0.1` / `::1` (with WHATWG `[::1]` bracket-stripping) / `*.local` / docker-compose `mysql` + `db` service names. Wired in via single-line `setupFiles: ['./test/setup.js']` addition to `backend/vitest.config.js`. Sentinel test at `backend/test/lib/db-fallthrough-guard.test.js` pins 9 contract cases (remote throws, 5 local-shape allows, escape-hatch allows, non-VITEST no-op, empty-URL no-op, unparseable-URL no-op). **Mid-flight fix:** initial implementation missed bracketed IPv6 (`u.hostname === '[::1]'` not `'::1'`); added a one-line `bareHost = host.replace(/^\[/, '').replace(/\]$/, '')` normalization. **Live verification (proof the guard is loud):** set `DATABASE_URL=mysql://admin:password123@163.227.174.141:3306/gbscrm` (which is current `backend/.env`) and ran `npx vitest run` → suite refused to start with the diagnostic error. Set `ALLOW_REMOTE_DB_IN_TESTS=1` → suite ran, 9/9 sentinel tests pass. **Spot-check of `test/routes/` with `DATABASE_URL=mysql://root@localhost:3306/gbscrm`:** guard did NOT false-fire; suite ran to completion with 361/369 files pass + 5 skipped + 3 fail (`auth-cookie-set.test.js`, `auth.test.js`, `consent-templates.test.js` — pre-existing failures unrelated to T35, flagged for T34 sweep verification). No source-code changes outside the three intended files. **Parallel coordination:** T34 was running concurrently; if T34's runner has a remote `DATABASE_URL`, the new guard will surface it loudly — recovery is `ALLOW_REMOTE_DB_IN_TESTS=1` for that local re-run while the dev sets up a local stack. |
| **T36** | Fix Class B1 recipe in backend-test-routes-red-audit.md (missing `prisma.user.findUnique` stub) | `docs/gaps/backend-test-routes-red-audit.md` | ✅ DONE 2026-06-08 — `2797ad06` | Flagged by T29 agent: the canonical Class B1 fix recipe at `docs/gaps/backend-test-routes-red-audit.md:78-83` is **incomplete**. T29 discovered that mocking only `prisma.userRole.findMany = vi.fn().mockResolvedValue([])` is insufficient — empty array triggers `requirePermission.js`'s `maybeSelfHealAdminPermissions` path which queries unmocked `prisma.user.findUnique` and falls through Prisma singleton to demo MySQL (`163.227.174.141:3306`) → 5s timeout. Full Class B1 fix requires BOTH `userRole.findMany` AND `user.findUnique` stubbed. The doc's claim "falls back to legacy `req.user.role` enum check" is wrong — it goes through the self-heal path first. Update the recipe with the corrected fix + a one-line note explaining the self-heal trap. **Docs-only.** Depends on T29 ✅. **Parallel-safe** with T28 + T32 + T33 + T34 + T35. Est. ~10 min. |
| **T37** | Audit-spec residual sweep — re-classify 5 hidden-Class-B files post-`npm install` | `docs/gaps/backend-test-routes-red-audit.md` + 5 backend test files (`backend/test/routes/auth.test.js` + `auth-cookie-set.test.js` + `auth-profile-picture.test.js` + `consent-templates.test.js` + `wellness-patients-xlsx.test.js`) | ✅ DONE 2026-06-08 — `d617b0d9` | All 5 files were a single new subclass — **Class B6 (RBAC self-heal seam)** — not a mix of B1/B2/B3 as the slice spec speculated. Root cause: `routes/auth.js`'s login self-heal block + signup/register's unconditional `provisionRbacForFreshTenant` + `lib/roleResolution.js`'s resolvePrimaryRole + `middleware/wellnessRole.js`'s permission fallback ALL transit one or more of `prisma.userRole.{count,findUnique,findFirst,findMany,create}` / `prisma.role.{findFirst,create}` / `prisma.rolePermission.{findFirst,create}` / `prisma.roleWidget.create` / `prisma.wellnessRoleType.findMany`. Without these stubs the real Prisma client tries demo MySQL (`163.227.174.141:3306`); the errors ARE caught as non-fatal warnings but each socket retry blocks for the full 5s test timeout. **Fix shipped:** per-file singleton-patch mock additions following a uniform template (documented in `docs/gaps/backend-test-routes-red-audit.md` under new **Class B6** section); also added `mockReset()` discipline in each `beforeEach`. **Per-file subclass classification:** all 5 files = `A + B6` (audit doc updated with per-file fixed-SHA annotations). **No source-code changes** — only test mock surface additions. **Pre-fix:** 10 failures (5 files, 5s timeouts each). **Post-fix:** 0 failures (`Test Files 5 passed (5) / Tests 72 passed | 2 skipped (74) / 1.81s`). Depends on T28..T34 ✅. **Parallel-safe** with T38 (file-disjoint — T38 = `package-lock.json`). |
| **T38** | Update package-lock.json — `magicast@0.3.5` lockfile drift | `backend/package-lock.json` | ✅ DONE 2026-06-08 — `f115a16d` | Flagged by T34 agent: `npm ci` blocks locally on missing `magicast@0.3.5` in lockfile vs. installed deps (likely from a transitive-dep version drift after a recent `npm install`). CI's `npm ci --no-audit --no-fund` works in its container with its own node_modules lifetime. **Fix:** run `cd backend && npm install` once to regenerate the lockfile, commit ONLY `package-lock.json`. **Docs-only friction smell** — not a functional bug, just lets fresh-clone devs use the canonical `npm ci` rather than the looser `npm install`. Depends on T28 ✅. **Parallel-safe** with T37. Est. ~5 min. |
| **T39** | Strengthen T35 guard — Option B prisma surface guard (catch local-DB fallthrough too) | `backend/test/setup.js` (extend) + `backend/test/lib/db-fallthrough-guard.test.js` (extend) | ✅ DONE 2026-06-08 — `6fcce3a9` | Implemented Option B per the slice spec. Extended `backend/test/setup.js` with `prismaSurfaceGuard()` which wraps `@prisma/client`'s `PrismaClient` class so every `new PrismaClient()` returns a Proxy that throws on any unmocked `prisma.<model>.<method>()` call. Tests with complete `vi.mock('../../lib/prisma', ...)` factories or full singleton-patch surfaces are transparent to the guard — the wrap only fires when a route's call falls through to the real client. **Proxy design:** caches per-model proxies (so the `prisma.foo = prisma.foo \|\| {}` idiom that singleton-patch tests use stays referentially stable), honors `set` traps (so `prisma.user.findUnique = vi.fn()` patches override the thrower), no-ops lifecycle methods ($connect / $disconnect / $on / $use), preserves `$extends` chainability by returning the proxy itself, throws on $transaction + raw-query surfaces, skips Symbol property access + promise-like introspection (then/catch/finally) so the proxy doesn't break iteration or async-return paths. **Escape hatch:** `PRISMA_ALLOW_REAL_CALLS=1` skips the wrap entirely — parallel to T35's `ALLOW_REMOTE_DB_IN_TESTS`. Integration tests in `backend/test/integration/` that intentionally exercise real Prisma should set both env vars. Sentinel test file extended with 14 new T39 cases (export shape, wrap marker flag, original constructor preserved, model.method() throws with diagnostic, two different model surfaces named correctly, lifecycle $connect + $disconnect resolve, $on + $use sync no-op, $extends returns proxy for chaining, $transaction throws, raw queries throw, Symbol property access doesn't throw, then/catch/finally return undefined not throwers, error message points at PRISMA_ALLOW_REAL_CALLS escape hatch, error message points at vi.mock pattern). **Total: 23 sentinel cases pass (9 T35 + 14 T39).** **Broader test/routes/ sweep:** 364 files pass / 5 skipped / 0 fail / 6501 tests pass / 75 skipped — full pre-T39 baseline preserved (the surface guard is fully transparent to mocks that cover their surfaces). **Full backend vitest sweep:** 540 files pass / 1 fail / 5 skipped — the single failure is `test/lib/prisma.test.js > $extends query hooks (mocked engine)` (15 of 21 tests skip after a failed beforeAll; 6 other tests in the file pass). This test pins the REAL Prisma $extends machinery by tapping `prisma.$parent._engine` to swap the engine.request implementation — it's structurally an integration test for Prisma's own internals, not a mockable unit. **Flagged as T40 follow-up** (option: either prepend `PRISMA_ALLOW_REAL_CALLS=1` via a vitest env-override at the top of that file, or move the file to `backend/test/integration/` and run it under a separate config that sets the escape hatch). No source-code changes outside the three allowed files. Depends on T35 ✅. |
| **T40** | Excuse `backend/test/lib/prisma.test.js` from the T39 surface guard | `backend/test/lib/prisma.test.js` → moved to `backend/test/integration/prisma-extends.test.js` + new `backend/vitest.integration.config.js` + new `backend/test/integration-setup.js` + `backend/vitest.config.js` (exclude) + `backend/package.json` (test:integration script) | ✅ DONE 2026-06-08 — `4a622fcd` | **Implemented Option B (clean separation) per the slice spec's recommendation** — the file is the first of what's likely several future integration-shaped specs (`backend/test/integration/` already hosts `stripe-webhook.test.js` + `route-mount-audit.test.js`, both of which keep using the singleton-monkey-patch pattern under the main config and were NOT touched). Steps: (1) moved `backend/test/lib/prisma.test.js` to `backend/test/integration/prisma-extends.test.js` (content unchanged; added a top-of-file note explaining the location); (2) added `backend/vitest.integration.config.js` — `include: ['test/integration/prisma-extends.test.js']`, `setupFiles: ['./test/integration-setup.js', './test/setup.js']` (integration-setup runs FIRST and writes `PRISMA_ALLOW_REAL_CALLS=1` + `ALLOW_REMOTE_DB_IN_TESTS=1` so when setup.js then fires both `dbFallthroughGuard()` and `prismaSurfaceGuard()` early-return — the real `@prisma/client` `PrismaClient` reaches the test, so `prisma.$parent._engine` is tappable); (3) added `backend/test/integration-setup.js` — minimal env-write file (named `*-setup.js` not `*.test.js` so the main `include: ['test/**/*.test.js']` glob doesn't pick it up); (4) extended `backend/vitest.config.js`'s exclude list to add `test/integration/prisma-extends.test.js` so the file does NOT run under the main config (the other integration files stay in main config — they use singleton-patch and don't need the escape hatch); (5) added `"test:integration": "vitest run --config vitest.integration.config.js"` to `backend/package.json`. **Validation:** `npm test` → `Test Files 540 passed \| 5 skipped (545)` / `Tests 11280 passed \| 75 skipped` / 0 failures (previously had 1 failed file from `prisma.test.js`'s broken `beforeAll`). `npm run test:integration` → `Test Files 1 passed (1)` / `Tests 21 passed (21)` — full coverage of the $extends machinery + singleton contract + production-branch + module shape suites restored. **No content changes** to the moved file beyond the top-of-file location-rationale comment; the 21 tests are the same 21 that pre-T39 used to pass. **Follow-up to consider (out of scope here):** wire `npm run test:integration` into `.github/workflows/deploy.yml`'s `unit_tests` step as a second invocation so CI also runs the integration spec (currently the main `npm test` skips it, so the CI gate runs only the unit-tier — same as today's main suite). Depends on T39 ✅. |
| **T41** | Wire `npm run test:integration` into CI's `unit_tests` job | `.github/workflows/deploy.yml` (extend `unit_tests` step to run `npm run test:integration` after `npm test`) | ✅ DONE 2026-06-08 — `86f005e6` | Flagged by T40 agent: T40 moved `prisma.test.js` to `backend/test/integration/prisma-extends.test.js` under a dedicated vitest config with the T35/T39 escape hatches set. The main `npm test` now correctly excludes integration specs, but `.github/workflows/deploy.yml`'s `unit_tests` job runs only `npm test` — so the 21 integration tests (including 15 `$extends` query-hook assertions that pin the wellness-PII encrypt/decrypt walker without booting MySQL) are no longer running in CI. **Fix:** add a second step (or extend the existing step) in the `unit_tests` job to run `cd backend && npm run test:integration` after `npm test`. Both must pass for the gate to be GREEN. Depends on T40 ✅. **Parallel-safe** with nothing else (no other ⬜ TODO rows). Est. ~10 min. Pattern A applied (separate step for failure-attribution clarity); local integration run 21/21 passing. |
