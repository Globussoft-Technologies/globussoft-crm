...
| **T31** | Class B3 — add `prisma.webhook.findMany` mock + triage dedup-409 assertion mismatch in `contacts.test.js` | `backend/test/routes/contacts.test.js` | ⬜ TODO | Two-part: (a) Add `webhook: { findMany: vi.fn() }` to mock surface + `prisma.webhook.findMany.mockResolvedValue([])` in `beforeEach`. Successful POST `/api/contacts` emits `contact.created` via eventBus → `webhookDelivery.deliverWebhooks` (`backend/lib/webhookDelivery.js:52`) calls `prisma.webhook.findMany`. Fixes 3 of 4 failing tests (happy create / happy update / ADMIN delete). (b) The `dedup hit → 409 DUPLICATE_CONTACT` test fails with `AssertionError: expected "vi.fn()" to not be called at all, but actually been called 1 times` — separately triage whether the route now writes an audit row on dedup-blocked creates (test must update) OR if a sibling test is leaking emit-spy state into this test (reset hygiene gap). Read current `routes/contacts.js` dedup branch to decide. Est. ~30 min total. **Parallel-safe** with T28 + T29 + T30 + T32 + T33. |
