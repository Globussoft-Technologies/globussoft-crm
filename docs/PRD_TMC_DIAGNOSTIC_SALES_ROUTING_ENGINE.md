...
| **T36** | Fix Class B1 recipe in backend-test-routes-red-audit.md (missing `prisma.user.findUnique` stub) | `docs/gaps/backend-test-routes-red-audit.md` | ⬜ TODO | Flagged by T29 agent: the canonical Class B1 fix recipe at `docs/gaps/backend-test-routes-red-audit.md:78-83` is **incomplete**. T29 discovered that mocking only `prisma.userRole.findMany = vi.fn().mockResolvedValue([])` is insufficient — empty array triggers `requirePermission.js`'s `maybeSelfHealAdminPermissions` path which queries unmocked `prisma.user.findUnique` and falls through Prisma singleton to demo MySQL (`163.227.174.141:3306`) → 5s timeout. Full Class B1 fix requires BOTH `userRole.findMany` AND `user.findUnique` stubbed. The doc's claim "falls back to legacy `req.user.role` enum check" is wrong — it goes through the self-heal path first. Update the recipe with the corrected fix + a one-line note explaining the self-heal trap. **Docs-only.** Depends on T29 ✅. **Parallel-safe** with T28 + T32 + T33 + T34 + T35. Est. ~10 min. |
