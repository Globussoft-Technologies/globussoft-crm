...
| **T14** | Public readiness-report JSON endpoint — `GET /api/travel/diagnostics/public/readiness-report/:slug` | `backend/routes/travel_diagnostics.js` + `backend/test/routes/travel-diagnostics-public-readiness-json.test.js` | ⬜ TODO | Flagged by T10 agent: `frontend/src/pages/public/TmcReadinessReport.jsx` fetches this endpoint to render the §3.5 10-section template. Today the page degrades to "Report being generated, please try again in a moment" because the route doesn't exist. The route mirrors the PDF endpoint's data pipeline (engine output + JobA narrative + standing facts + board hook + runway display) but returns JSON instead of streaming a PDF. **Public access via slug** (no auth) — same `openPaths` regex pattern as the PDF endpoint (`/travel/diagnostics/<slug>/readiness-report.json`). ≥8 vitest cases (happy path / 404 / cross-tenant isolation / Layer 3 fallback path / standing-facts injection / board hook branches / honest-numbers literal). Depends on T8 ✅. **Parallel-safe** with T15 + T16 + T17. |
| **T15** | EngineWeights CRUD endpoint — `GET/PUT /api/travel/engine-weights` | `backend/routes/travel_engine_weights.js` + `backend/server.js` (mount) + `backend/test/routes/travel-engine-weights.test.js` | ⬜ TODO | Flagged by T11 agent: the EngineWeights tab in `DiagnosticBuilder.jsx` PUTs to this endpoint but it doesn't exist; UI gracefully falls back to PRD §3.3.3 defaults on 404. GET returns the tenant's current row (or defaults if no row yet); PUT upserts with auto-version-bump when any weight changes. ADMIN+MANAGER gate. ≥12 vitest cases (GET default / GET row / PUT happy / PUT validation: negative weight, threshold out of range / version auto-bump / role gate / cross-tenant isolation / idempotent re-PUT). Depends on T1 ✅. **Parallel-safe** with T14 + T16 + T17. |
| **T16** | Dedicated `TmcCatalogueAdmin.jsx` admin page — list + Promote-to-active + create/edit modal | `frontend/src/pages/travel/TmcCatalogueAdmin.jsx` + `frontend/src/__tests__/TmcCatalogueAdmin.test.jsx` + `frontend/src/App.jsx` (route registration) | ⬜ TODO | Flagged by T11 agent: currently Promote-to-active is a sub-panel inside the EngineWeights tab. Dedicated admin page at `/travel/tmc/catalogue` shows all `TmcTripCatalogue` rows (active + archived tabs), exposes create + edit modal, prominent Promote-to-active button on archived rows, ADMIN-only for promote. Calls T5's catalogue CRUD endpoints. Mobile-responsive grid. ≥10 vitest cases. Depends on T5 ✅. **Parallel-safe** with T14 + T15 + T17. |
| **T17** | Number-word check in `tmcReportGuard` — Layer 2 spelled-out-numbers detection | `backend/lib/tmcReportGuard.js` + `backend/test/lib/tmcReportGuard.test.js` | ⬜ TODO | Flagged by T7 agent: current Layer 2 number-check is integers only (3+ digits). PRD §3.7.1 check 2 names both digit AND word forms ("five hundred", "two thousand five hundred", etc.). Extend with a word-to-digit canonicaliser then check against the honest-numbers whitelist. ≥8 new vitest cases extending the existing 49 (one per common spelled form + whitelisted "three hundred and five" passes / non-whitelisted spelled forms reject / mixed digit+word "200 (two hundred)" rejects). Depends on T7 ✅. **Parallel-safe** with T14 + T15 + T16. |
