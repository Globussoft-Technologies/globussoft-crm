...
| **T34** | T27 sweep verification — re-run `npx vitest run test/routes/` after T28..T33 land; pin 0 reds | (no files changed — verification only; close out T27 in CHANGELOG) | ⬜ TODO | Once T28..T33 are all ✅, run `cd backend && npm ci && npx vitest run test/routes/` and confirm `Test Files 0 failed | 369 passed`. Then bump CHANGELOG.md with the T27 sweep entry (5 sub-classes resolved, 11 CI-red files green, +1 local-env doc note for fresh clones). Verification slice — no parallel concerns since it's the closing tick. Est. ~10 min. Depends on T28+T29+T30+T31+T32+T33 all ✅. |
| **T35** | Test-harness guard against real-DB fall-through | `backend/test/setup.js` (or similar global test setup) + `backend/test/lib/db-fallthrough-guard.test.js` | ⬜ TODO | Flagged by T27 agent: tests in `backend/test/routes/` routinely fall through to the demo MySQL (`163.227.174.141:3306`) when their Prisma mocks miss a surface — the route's call hits the real `new PrismaClient()` because vitest's mock didn't cover that field. Symptom: tests pass locally (against demo data) but the test is structurally a real-DB integration test in mock's clothing — this is the root cause of T24's class of bugs. **Fix:** add a global test-harness guard that detects `DATABASE_URL` being a remote host during `cd backend && npx vitest run` and either (a) throws on test-setup if `process.env.NODE_ENV === 'test'` AND `DATABASE_URL` is non-localhost, OR (b) wraps the Prisma client's `_baseDmmf` to fail on any non-mocked surface call (more invasive). Option (a) is the minimum. Add a sentinel test that asserts the guard fires when `DATABASE_URL=mysql://remote-host/...` is set. **Cross-cutting hygiene fix** — affects all 369 `backend/test/routes/*.test.js` files structurally, but the guard is a one-file global addition. Depends on T28..T33 ✅ (so the cleanup sweep doesn't false-fire under the new guard). Est. ~1.5h. |
