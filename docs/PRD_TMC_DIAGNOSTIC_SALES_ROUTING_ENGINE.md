...
| **T29** | Class B1 batch — add `prisma.userRole.findMany` mock to 7 test files (POS + drugs) | `backend/test/routes/drugs.test.js` + `backend/test/routes/pos-cashLedger.test.js` + `backend/test/routes/pos-sale-context.test.js` + `backend/test/routes/pos-sale-finalize.test.js` + `backend/test/routes/pos-sales-by-month.test.js` + `backend/test/routes/pos-sales-stats.test.js` + `backend/test/routes/pos-void-refund.test.js` | ⬜ TODO | Each file's `vi.mock('../../lib/prisma', ...)` block needs `userRole: { findMany: vi.fn() }` added to the surface + `prisma.userRole.findMany.mockReset().mockResolvedValue([])` in `beforeEach`. Empty array → `requirePermission` middleware (`backend/middleware/requirePermission.js:178`) falls back to the legacy `req.user.role` enum check that all existing tests already stub. Affects 9 currently-timing-out tests (1 each in 6 files; 3 in `pos-void-refund.test.js`). Est. ~20 min/file × 7 = ~2.5h. Single-agent slice since the recipe is identical across files. **Parallel-safe** with T28 + T30 + T31 + T32 + T33 (file-disjoint). |
