...
| **T40** | Excuse `backend/test/lib/prisma.test.js` from the T39 surface guard | `backend/test/lib/prisma.test.js` (set `PRISMA_ALLOW_REAL_CALLS=1` at file head OR move under `backend/test/integration/`) | ⬜ TODO | Flagged by T39 agent: `test/lib/prisma.test.js > $extends query hooks (mocked engine)` describe taps `prisma.$parent._engine` to swap the engine.request implementation — it's structurally an integration test for Prisma's $extends machinery, not a mockable unit. 15 of 21 tests skip after a failed beforeAll under T39's surface guard. **Fix options:** (a) prepend `process.env.PRISMA_ALLOW_REAL_CALLS = '1'` before the require/import of @prisma/client at the top of the file (env-override approach — keeps the file in place); (b) move the file to `backend/test/integration/` and run it under a separate vitest config that sets the escape hatch globally (cleaner separation). Pick (a) for minimum churn; pick (b) if the file is the first of several future integration-shaped specs. Depends on T39 ✅. Est. ~15 min. |
