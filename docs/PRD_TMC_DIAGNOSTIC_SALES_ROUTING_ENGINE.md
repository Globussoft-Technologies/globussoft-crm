...
| **T28** | Class A — local-env note (docs only; no code) | `TODOS.md` OR `README.md` (fresh-clone dev guidance) | ⬜ TODO | Add a one-liner under "Development Setup" reminding fresh-clone devs to run `cd backend && npm install` after pulling — `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` were added in `eba590b8` (2026-05-20). On CI the `npm ci` step covers this; on local dev boxes pulling AFTER a deps-add commit, `npm install` is mandatory before `npx vitest run`. NOT a real test sweep — 20 of T24's "31 reds" disappear with one `npm install`. Est. ~10 min. **Parallel-safe** with T29-T33. |
