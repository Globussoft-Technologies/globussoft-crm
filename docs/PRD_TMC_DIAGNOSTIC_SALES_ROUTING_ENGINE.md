...
| **T32** | Class B4 — add `travelPaymentSchedule` mock surfaces to `travel-invoice-issue.test.js` | `backend/test/routes/travel-invoice-issue.test.js` | ⬜ TODO | Add `travelPaymentSchedule: { findFirst: vi.fn(), findMany: vi.fn(), create: vi.fn() }` to the prisma mock surface. In `beforeEach`, set `prisma.travelPaymentSchedule.findFirst.mockResolvedValue({ id: 1 })` so the "auto-create payment schedule if not present" block in `backend/routes/travel_invoices.js:3908` short-circuits — matches all 6 failing tests' intent which assert on FY-counter invoice-num formatting, NOT on schedule creation. Affects 6 timing-out tests (happy TMC / happy RFU / happy travel_stall / sequential 0001→0002 / audit row carry / cross-sub-brand independence). Est. ~45 min. **Parallel-safe** with T28 + T29 + T30 + T31 + T33. |
