...
| **T37** | Audit-spec residual sweep — re-classify 5 hidden-Class-B files post-`npm install` | `docs/gaps/backend-test-routes-red-audit.md` + 5 backend test files (`backend/test/routes/auth.test.js` + `auth-cookie-set.test.js` + `auth-profile-picture.test.js` + `consent-templates.test.js` + `wellness-patients-xlsx.test.js`) | ⬜ TODO | Flagged by T34 agent: after T28's `npm install` cleared the Class A AWS-SDK lag, 5 files that were classified as Class A only turned out to have a HIDDEN Class B layer underneath (missing prisma mocks). T34's verification surfaced 9 residual failing tests across these 5 files. **Fix:** (a) re-classify the 5 files in `docs/gaps/backend-test-routes-red-audit.md` from "Class A only" to "Class A + Class B (specify subclass)"; (b) apply the appropriate mock-fix recipe per file (likely a mix of B1 self-heal, B2 tenant.findUnique, B3 webhook.findMany — TBD per audit). Depends on T28..T34 ✅. **Parallel-safe** with T38. Est. ~1-2h. |
