...
| **T30** | Class B2 — add `prisma.tenant.findUnique` mock to `staff.test.js` | `backend/test/routes/staff.test.js` | ⬜ TODO | Add `tenant: { findUnique: vi.fn() }` to the prisma mock surface + `prisma.tenant.findUnique.mockResolvedValue({ id: 1, vertical: 'wellness' })` in the wellnessRole-enum-validation describe `beforeEach`. The route's `resolveVertical(req)` helper (`backend/routes/staff.js:49`) reads `tenant.findUnique` to decide if `wellnessRole` is gate-relevant. Affects 2 timing-out tests (`accepts "cashier"` + `rejects garbage`). Est. ~20 min. **Parallel-safe** with T28 + T29 + T31 + T32 + T33. |
