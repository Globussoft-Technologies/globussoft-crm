...
| **T39** | Strengthen T35 guard — Option B prisma surface guard (catch local-DB fallthrough too) | `backend/test/setup.js` (extend) + `backend/test/lib/db-fallthrough-guard.test.js` (extend) | ⬜ TODO | Flagged by T37 agent: T35's current guard catches only the REMOTE-DB fallthrough case (DATABASE_URL points off-localhost). It does NOT catch the LOCAL-DB-with-incomplete-mocks case — where dev's DATABASE_URL is set to localhost AND a route hits an unmocked Prisma surface, the test silently calls real Prisma against local data. That's the Class B6 trap T37 just resolved across 5 files (auth + consent + wellness-xlsx). **Option B** (per T35's original prompt) wraps `PrismaClient`'s proxy to throw on any non-mocked surface call regardless of DATABASE_URL host. More invasive but structurally closes the class. **Open design Q:** how to allow integration tests (in `backend/test/integration/`) to opt out — likely a new env var (`PRISMA_ALLOW_REAL_CALLS=1`) parallel to T35's `ALLOW_REMOTE_DB_IN_TESTS`. Depends on T35 ✅. **Parallel-safe** with nothing currently (no other ⬜ TODO rows after this). Est. ~3-4h. |
