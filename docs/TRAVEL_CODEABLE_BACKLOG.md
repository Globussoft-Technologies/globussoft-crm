...
| **C4** | Pipeline mobile touch drag + a11y + virtualization | `frontend/src/pages/Pipeline.jsx` (extend) + `frontend/src/__tests__/Pipeline.test.jsx` (extend) | ✅ DONE 2026-06-09 — `88e1f1fe` | ~3d | Per [PRD_TRAVEL_PIPELINE_KANBAN](PRD_TRAVEL_PIPELINE_KANBAN.md) FR-3.16/17/18. Shipped: native touch-event listeners for mobile drag (no `@dnd-kit/core` dep added — separate slice if upgraded), keyboard a11y (Tab focus, arrow nav, Space to enter move mode, Esc to cancel, aria-live announcer), in-component scroll-position-based windowed slice virtualization with threshold=100 (no `react-window` / `@tanstack/react-virtual` dep added — same rationale), `prefers-reduced-motion` honoured. |
