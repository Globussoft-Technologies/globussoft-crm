...
| **C2** | Passport OCR stub-mode client + upload route + verification queue UI | `backend/services/passportOcrClient.js` (NEW) + `backend/routes/travel_passport.js` (NEW) + `frontend/src/pages/travel/PassportVerificationQueue.jsx` (NEW) + tests | ✅ DONE 2026-06-09 — `82499cbb` | ~3d | Per [PRD_PASSPORT_OCR](PRD_PASSPORT_OCR.md) §5.4 "½-day clean stub drop is viable". Stub returns canned extraction result; verification queue lets ADMIN approve/reject. Schema cols already on `TripParticipant` (passportNumber/Expiry/DocId). **Parallel-safe with C1 + C3 + C5 + C6 + C8 + C9.** |
| **C3** | Pipeline sub-brand URL-param persistence | `frontend/src/pages/Pipeline.jsx` (extend) + `frontend/src/__tests__/Pipeline.test.jsx` (extend) | ✅ DONE 2026-06-09 — `5f088f26` | ~0.5d | Per [PRD_TRAVEL_PIPELINE_KANBAN](PRD_TRAVEL_PIPELINE_KANBAN.md) FR-3.15. Filter chip already live; this adds `?subBrand=tmc,rfu` URL sync via `useSearchParams`. **Same-file with C4** — dispatch C3 first, C4 after C3 lands. |
| **C4** | Pipeline mobile touch drag + a11y + virtualization | `frontend/src/pages/Pipeline.jsx` (extend) + `frontend/src/__tests__/Pipeline.test.jsx` (extend) | ✅ DONE 2026-06-09 — `pending-sha` | ~3d | Per [PRD_TRAVEL_PIPELINE_KANBAN](PRD_TRAVEL_PIPELINE_KANBAN.md) FR-3.16/17/18. Shipped: native touch-event listeners for mobile drag (no `@dnd-kit/core` dep added — separate slice if upgraded), keyboard a11y (Tab focus, arrow nav, Space to enter move mode, Esc to cancel, aria-live announcer), in-component scroll-position-based windowed slice virtualization with threshold=100 (no `react-window` / `@tanstack/react-virtual` dep added — same rationale), `prefers-reduced-motion` honoured. |
...
| **C6** | TMC Curriculum CSV import/export + coverage report endpoint | `backend/routes/travel_curriculum.js` (extend with `POST /import.csv` + `GET /export.csv` + `GET /coverage`) + `backend/lib/curriculumCsvParser.js` (NEW) + tests + sample CSV fixtures under `backend/test/fixtures/curriculum/` | ✅ DONE 2026-06-09 — `0cdab912` | ~3d | Per [PRD_TMC_CURRICULUM_MAPPING](PRD_TMC_CURRICULUM_MAPPING.md) FR-2/4/8. Import = upsert by `(board, subject, gradeBand)` composite key; export = round-trip same shape; coverage = which board×grade×outcome combos have ≥1 mapping. **Parallel-safe with C1 + C2 + C3 + C5 + C8 + C9.** |
| **C7** | TMC Curriculum engine integration — top-N curriculum-fit recs on TMC submit | `backend/routes/travel_diagnostics.js` (extend `submit-tmc`) + `backend/lib/tmcDiagnosticEngine.js` (extend with `curriculumFitJson` output) + `backend/prisma/schema.prisma` (add `curriculumFitJson` nullable column on `TravelDiagnostic`) + tests + migration check | ✅ DONE 2026-06-09 — `29918b83` | ~1d | Per [PRD_TMC_CURRICULUM_MAPPING](PRD_TMC_CURRICULUM_MAPPING.md) FR-5. Schema add is purely additive nullable → no bless marker needed. **Depends on C6** (needs curriculum data populated to test fit). C6 ✅ DONE 2026-06-09 — `0cdab912`. |
| **C8** | Billing reminder crons + Aged Receivable + Aged Payable + TCS Form-27EQ | `backend/cron/paymentScheduleReminderEngine.js` (NEW, T-7/T-3/T-1 milestone fires) + `backend/routes/travel_invoices.js` (extend with `GET /aged-receivable-report` + `GET /aged-payable-report` + `GET /tcs/27eq`) + tests | ✅ DONE 2026-06-09 — `3ac2a258` | ~6d | Per [PRD_TRAVEL_BILLING](PRD_TRAVEL_BILLING.md). Cron fires T-7/T-3/T-1 customer reminders (SMS+email via injectable notify callback; WA leg stub pending Q9 Wati creds); per-row `remindersSentCount` cap=3 + `lastReminderSentAt` bump on each send. Aged endpoints land at `/aged-receivable-report` + `/aged-payable-report` (named-bucket spec — `Current (0-30 days)`, `31-60 days`, `61-90 days`, `Over 90 days`) to keep the existing slice-23 `/aged-receivable` + slice-8 `/payables/aging` shapes untouched. TCS 27EQ = quarterly CSV per Sec 206C(1G); FY parsing rejects mismatched YY (e.g. `2025-27`); buyer PAN derived from GSTIN chars 3..12. Schema: `TravelPaymentSchedule.remindersSentCount` + `lastReminderSentAt` additive nullable. 20 vitest cases (cron) + 17 (routes). **Parallel-safe with C1 + C2 + C3 + C5 + C6 + C9.** |
| **C9** | Quote Builder customer-accept landing + share JWT + snapshot history + expiry cron | `backend/routes/travel_quotes_public.js` (NEW) + `frontend/src/pages/public/QuoteAcceptLanding.jsx` (NEW) + `backend/cron/quoteExpirySweep.js` (NEW) + `backend/lib/quoteShareToken.js` (NEW JWT helper) + `backend/prisma/schema.prisma` (add `TravelQuoteSnapshot` model, additive) + tests | ✅ DONE 2026-06-09 — `bb41c51a` (schema + server.js wires absorbed in `3ac2a258`) | ~5d | Per [PRD_TRAVEL_QUOTE_BUILDER](PRD_TRAVEL_QUOTE_BUILDER.md). Customer clicks share link → JWT validates + decrypts quote slug → landing page renders read-only quote + Accept/Reject/Counter buttons. Snapshot history = immutable QuoteSnapshot rows on every status change. Cron sweeps `validUntil < now AND status='draft'` daily. **Parallel-safe with C1 + C2 + C3 + C5 + C6 + C8.** |
