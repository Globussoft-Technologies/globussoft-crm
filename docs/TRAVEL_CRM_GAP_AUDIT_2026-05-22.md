...
- **SHIPPED:** **74** (~95%) — unchanged numerator (the 4 newly shipped items since the prior refresh were already counted as SHIPPED §7 page rows that round, except `Itinerary LLM draft` which was a §4.3 GAP — net effect: §4.3 LLM-draft moved GAP-AUTONOMOUS → SHIPPED, so SHIPPED is up +1 from 73 → 74 and GAP-AUTONOMOUS at the §4 layer is down -1; §7 page-row ships are tracked separately and don't move the §4 denominator)
...
- **GAP-AUTONOMOUS:** **0** (0%) **in §4 PRD requirements** — exhausted at the PRD-requirement layer. §7 page rows still have viable autonomous picks (LlmSpendDashboard.jsx, ItineraryBuilder explicit `/new` route). Phase 1.5 form-vs-call persistence + customer-duplicate UI modal remain as engineering-quality picks not tied to a §4 row
...
1. **`LlmSpendDashboard.jsx` admin observability page** (PRD §4.9 row + R7). New page under `frontend/src/pages/admin/LlmSpendDashboard.jsx` consuming `GET /api/admin/llm-spend?days=N` (commit `f5c9518`). Renders the `{ totals, byDay, byTask, byModel }` envelope as 4 widgets (line chart of daily totals, bar by task, bar by model, summary cards). ADMIN-only sidebar link. ~½ day, pure frontend, zero cred deps. **Why next:** the backend daily-summary endpoint ships without an admin surface; visibility into R7 LLM cost observability stays internal until this lands. Closes the only remaining unstruck pick from the prior refresh menu, AND closes the last §4.9 (Reports/Dashboards) gap. First-choice high-confidence pick.
...
| `LlmSpendDashboard.jsx` (admin observability surface for `/api/admin/llm-spend`) | NOT SHIPPED — GAP-AUTONOMOUS | Single-page admin chart consuming the daily summary; pairs with R7 observability — **pick #1 below** |
...
| `DuplicateContactModal.jsx` (front-end modal for 409 DUPLICATE_CONTACT envelope) | NOT SHIPPED — GAP-AUTONOMOUS | Backend ships at `routes/contacts.js:263-278`; modal needs to intercept the 409 + render merge/keep-both choice — **pick #3 below** |
...
1. **`LlmSpendDashboard.jsx` admin observability page** (PRD §4.9 row + R7). New page under `frontend/src/pages/admin/LlmSpendDashboard.jsx` consuming `GET /api/admin/llm-spend?days=N` (commit `f5c9518`). Renders the `{ totals, byDay, byTask, byModel }` envelope as 4 widgets (line chart of daily totals, bar by task, bar by model, summary cards). ADMIN-only sidebar link. ~½ day, pure frontend, zero cred deps. **Why next:** closes the only unstruck pick from the prior refresh, AND closes the last §4.9 (Reports/Dashboards) gap. Cleanly closes R7's observability loop.
...
4. **Rooming XLSX export endpoint** (§4.5 row, Phase 1.5). New `GET /api/travel/trips/:tripId/rooming.xlsx` endpoint in `routes/travel_trip_billing.js` that produces an XLSX of the trip's `RoomingAssignment` rows (already-CRUDed via `routes/travel_trip_billing.js:65-200`) using the in-tree `xlsx` lib. Columns: room # / room type / participants (comma-sep names) / capacity / occupancy. ADMIN/MANAGER role + sub-brand access check. Wire a "Download Rooming XLSX" button in `TripDetail.jsx`. ~3-4 hrs. **Why fourth:** small endpoint, no cred deps, lifts §4.5 Rooming row from PARTIAL → SHIPPED. Lower priority than picks #1-#3 because the in-CRM table view already covers the daily workflow.
