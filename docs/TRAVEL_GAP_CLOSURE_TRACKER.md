...
| ID | Slice | Files | Effort | Status |
|---|---|---|---|---|
| G047 | `Itinerary.clonedFromTemplateId` lineage column + UI surface (FR-3.1.e) | schema + route + UI | 1d | ⬜ |
| G048 | Template versioning: `version`, `isLatest`, `archivedAt` on `ItineraryTemplate` (FR-3.5.a, FR-3.5.b) | schema (additive) + route | 1.5d | ⬜ |
| G049 | Template metrics: `acceptedCount`, `avgFinalPrice`, `lastUsedAt` (FR-3.1.h) | schema + on-quote-accept hook | 1d | ⬜ |
| G050 | "Save current itinerary as template" action (FR-3.1.f) | route + UI button | 1d | ⬜ |
| G051 | `ItineraryItem.draftedByAi: boolean` provenance (FR-3.4.h) | schema + suggest endpoint + UI badge | 0.5d | ⬜ |
