...
| **G-21** | Frontend vitest + RTL setup + first 5 component tests | 3-5 days | Med — 80 pages + 11 components have zero isolated tests | ✅ shipped (verified 2026-05-07 in cron wave-13: `frontend/vitest.config.js` + setup + 14 component test files in `frontend/src/__tests__/` (CPQBuilder, CommandPalette, Dashboard, DealModal, EmailSignatureEditor, LanguageSwitcher, Layout, Leads, NotificationBell, OwnerDashboard, PatientDetail, Presence, Services, TelecallerQueue) far exceeding the "first 5" minimum, 203 vitest tests passing, `frontend_unit_tests` CI gate live in `deploy.yml`. Ongoing component coverage continues incrementally as part of feature work — not a gate-blocking item.) |
...
> **Status update 2026-05-09 (v3.5.0):** **All G-1..G-25 rows shipped** as of the v3.4.11 status update below; nothing new maps to a G-x row this release. v3.5.0 is a **greenfield feature release** — 4 entirely new product surfaces (POS / Attendance + Leave / WhatsApp 2-way / Booking-widget completion) added 11 new Prisma models, 4 new route files, **3 new API gate specs** (`pos-api.spec.js` 38 tests, `attendance-api.spec.js` 25 tests, `leave-api.spec.js` 28 tests; +91 API tests), **+93 backend vitest cases** via Wave 3's coverage-extension agent (eventBus.js 38% → 83% lines / +51 cases, plus landingPageRenderer + slaBreachEngine extensions), and **+76 backend vitest cases** via Wave 5 Agent XX's three new cron-engine vitests (leadSla / lowStock / scheduledEmail — each lifted from 0% to 90%+ lines). Per-push gate is now **~82 specs / ~2,651 API tests + 49 vitest files / ~1,365 unit tests + 6 frontend vitest / ~35 tests = ~4,051 per-push** (+~9% since v3.4.14, +~64% since the v3.4.3 baseline). **Carry-over to v3.5.1**: frontend RTL component tests for the 4 new feature pages (PointOfSale / Attendance / Leave / WhatsAppThreads). The 4 frontend pages shipped without RTL tests because the parallel-wave agents prioritised the API gate spec; frontend vitest stays at 6 files / ~35 tests until those land.
>
> **Earlier status update 2026-05-05 (v3.4.11):** **G-21 frontend vitest+RTL** still the only open G-XX row. v3.4.11 added **+8 API tests** via the new `e2e/tests/report-schedules-api.spec.js` (8 tests: 6 sanitization regression + 2 auth-gate) — the only NEW spec wired into the per-push gate this arc. The 4-route #398/#447 sanitization audit identified by `68e6c5b` is now fully closed: helper promoted to `backend/lib/sanitizeJson.js` (`097ef5a`), adopted at lead_routing / ab_tests / marketing / report_schedules with matched regression coverage in each route's `*-api.spec.js` (4 spec extensions + 1 new dedicated spec). Per-push gate is now **~77 specs / ~2,522 API tests + 42 vitest files / ~1,184 unit tests = ~3,706 per-push** (+~50% since the v3.4.3 baseline below). **Carry-over to v3.4.12**: the v3.4.10/v3.4.11 git tags both pending user push (release-validation `e2e-full.yml` fires on tag); #445 Nginx config gap fully documented; the 9 landing-page UI issues are frontend-shaped and need a coordinated builder pickup.
>
