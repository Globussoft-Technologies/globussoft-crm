...
- **`approvals.js:38`** — `prisma.user.findMany({ where: { id: { in: userIds } } })`. **RESOLVED in S36 (commit TBD)** — GENUINE LEAK. The `userIds` come from approval requests of the current tenant, but the Prisma schema doesn't enforce a `(User.tenantId == ApprovalRequest.tenantId)` constraint, so a stale or maliciously-crafted cross-tenant id in `requestedBy` / `approvedBy` would leak User PII (name + email + role). Fix landed: `hydrateUsers(requests, tenantId)` now passes the caller's `tenantId` into the user lookup `where: { id: { in: userIds }, tenantId }`. All 9 existing callsites already passed `tenantId` to the helper; the function signature was the only change.
- **`auth.js:109`** — `prisma.tenant.findMany({ where: { isActive: true } })` on the PUBLIC `/api/auth/public/tenants` route. **RESOLVED in S36 (commit TBD)** — FALSE POSITIVE. The route has NO `verifyToken` (it's the pre-registration tenant dropdown), so there is no `req.user.tenantId` to scope by; the cross-tenant response is by design. The S2 audit doc's "login + signup flows query the User table by email" framing was wrong about WHICH Prisma model this line is — the call lists Tenants, not Users. Suppressed with `// eslint-disable-next-line gbscrm/tenant-scope-finder-heuristic` + rationale.
- **`auth.js:353`** — `prisma.tenant.findMany({ where: { isActive: true } })` on the PUBLIC `/api/auth/customer/tenants` route. **RESOLVED in S36 (commit TBD)** — FALSE POSITIVE. Same shape as `auth.js:109` (the audit doc said "auth.js:350" — the actual ESLint-flagged line is 353 post-comment-block). Returns minimal display fields only (id, name, vertical) — no plan, owner, billing, or branding metadata exposed. Suppressed with inline rationale.
- **`dashboards.js:335` + `:379`** — `prisma.deal.findMany` (`chart-revenue-trend`) + `prisma.task.findMany` (`table-overdue-tasks`). **RESOLVED in S36 (commit TBD)** — FALSE POSITIVE per the spread-pattern false-positive class (audit doc §"Heuristic gaps the ESLint rule won't catch" point 1). Both calls spread `...tenantWhere` (defined on L284 as `{ tenantId }` via the `resolveWidget(type, tenantId)` helper), which injects `tenantId` at runtime — but the AST selector can't see through the spread. Suppressed with inline rationale.
- **`reports.js:200-203`** — `prisma.deal.findMany` + `prisma.task.findMany` + `prisma.callLog.findMany` + `prisma.emailMessage.findMany` on `/api/reports/agent/:userId`. **RESOLVED in S36 (commit TBD)** — FALSE POSITIVE per the spread-pattern class. All four calls spread `...baseWhere` where `baseWhere = buildWhere(req, startDate, endDate)` (the helper on L26-34 inlines `tenantId: req.user.tenantId` into the returned where object). The audit doc said "L200 + L201" but ESLint actually flagged L200-203 (4 lines, not 2) — corrected during S36. Suppressed with inline rationales on each of the 4 calls. **Defense-in-depth note:** the route ALSO does a tenant-scoped `prisma.user.findFirst({ where: { id: userId, tenantId } })` on L195 BEFORE the aggregations, so a cross-tenant `:userId` 404s before the spread calls run.

All five of the audit-flagged callsites are now closed. S36 also added 5 per-route cross-tenant probes to [`cross-tenant-coverage-audit.spec.js`](../../e2e/tests/cross-tenant-coverage-audit.spec.js) — `S36 approvals POST + cross-tenant list probe`, `S36 auth.js:109 /auth/public/tenants minimal-fields shape pin`, `S36 auth.js:353 /auth/customer/tenants minimal-fields shape pin`, `S36 dashboards widget data resolver`, `S36 reports.js:200-203 /reports/agent/:userId cross-tenant 404`.

**ESLint count delta (4 modified route files):** 8 warnings → 0 warnings. Whole-routes-tree count drops correspondingly; the remaining ~64 warnings on the broader surface are the catalogued false-positive patterns enumerated above.
