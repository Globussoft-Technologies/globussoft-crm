...
describe('DiagnosticDetail  advisor brief UI (PRD 4.1 + 4.2)', () => {
  ...
  it('surfaces the SAMPLE pill when the envelope marks stub=true', async () => {
    ...
    const pills = screen.getAllByText('SAMPLE');
  ...
  it('renders the talking-points meta line with model + date, no raw stub flag, when envelope is non-stub', async () => {
    fetchApiMock.mockImplementation(makeFetchImpl(DIAGNOSTIC_WITH_BRIEF));
    renderPage();
    await screen.findByRole('heading', { name: /Diagnostic #42/i });
    await screen.findByTestId('brief-body-advisor_brief');

    // Meta line shows the generating model — no raw "stub: yes/no" text
    // (that's advisor-facing UI, not an internal debug flag).
    expect(screen.getByText('Generated by', { exact: false })).toBeTruthy();
    expect(screen.getByText(TALKING_POINTS_ENVELOPE.model)).toBeTruthy();
    expect(screen.queryByText(/stub:/i)).toBeNull();
    // SAMPLE pill is NOT visible (real envelope, stub=false).
  ...
  it('comparison.stub=true surfaces the SAMPLE pill in the result row', async () => {
    ...
    await screen.findByTestId('comparison-result');
    // SAMPLE pill renders inside the comparison-result block.
    const pills = screen.getAllByText('SAMPLE');
    expect(pills.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('T11: Engine output is COLLAPSED when humanPick is null + user is ADMIN', async () => {
    ...
    expect(screen.getByText(/AI recommendation hidden/i)).toBeTruthy();
  ...
  it('T11: Collapse button toggles engine output back to hidden after reveal', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Hide AI recommendation/i }));
...
describe('DiagnosticDetail  chosen itinerary interests', () => {
  it('renders nothing when the diagnostic has no chosenInterests', async () => {
    fetchApiMock.mockImplementation(makeFetchImpl({ ...DIAGNOSTIC_NO_BRIEF, chosenInterests: null }));
    renderPage();
    await screen.findByRole('heading', { name: /Diagnostic #42/i });
    expect(screen.queryByText(/chosen itinerary interests/i)).not.toBeInTheDocument();
  });

  it('renders the interests + brochure links + submitted timestamp when present', async () => {
    fetchApiMock.mockImplementation(makeFetchImpl({
      ...DIAGNOSTIC_NO_BRIEF,
      chosenInterests: {
        interests: [
          { name: 'Hampi Heritage Trail', driveLink: 'https://drive.example/hampi' },
          { name: 'Coorg Nature Camp', driveLink: '' },
        ],
    ...
    expect(await screen.findByText(/chosen itinerary interests/i)).toBeInTheDocument();
    ...
    const link = screen.getByRole('link', { name: /View brochure/i });
    expect(link).toHaveAttribute('href', 'https://drive.example/hampi');
    // Only Hampi has a driveLink — Coorg gets no brochure link.
    expect(screen.getAllByRole('link', { name: /View brochure/i })).toHaveLength(1);
  });
});
