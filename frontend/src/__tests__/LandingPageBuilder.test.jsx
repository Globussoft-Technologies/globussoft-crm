...
function defaultFetch(url, opts) {
  ...
  if (url === "/api/landing-pages/43" && method === "GET")
    return Promise.resolve(samplePagePublished);
...
describe("<LandingPageBuilder /> — page surface", () => {
  ...
  it("renders the empty-canvas hint when the page has zero components", async () => {
    ...
    expect(
      screen.getByText(/Click components on the left to add them/i),
    ).toBeInTheDocument();
  });
  ...
  it("clicking a palette button appends a component of that type to the canvas", async () => {
    ...
    expect(beforeStrong.parentElement.textContent).toMatch(
      /Components:\s*0\s*$/,
    );
    ...
    expect(afterStrong.parentElement.textContent).toMatch(
      /Components:\s*1\s*$/,
    );
  });
  ...
  it("renders the right-rail property-editor placeholder when nothing is selected", async () => {
    ...
    expect(
      screen.getByText(
        /Click a component on the canvas to edit its properties\./i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("after adding a component, Undo becomes enabled and clicking it reverts the addition", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Enter your text content here\./i),
      ).toBeInTheDocument(),
    );
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/Enter your text content here\./i),
  ...
  it("Save fires PUT /api/landing-pages/:id with title + JSON-stringified components", async () => {
    ...
    await waitFor(() => {
      const call = fetchApiMock.mock.calls.find(
        ([url, opts]) =>
          url.startsWith("/api/landing-pages/42") && opts?.method === "PUT",
      );
  ...
  it("typing an invalid slug normalizes it (uppercase → lowercase, spaces → hyphens)", async () => {
    ...
    renderBuilder();
    await waitFor(() =>
      expect(screen.getByLabelText("Page URL slug")).toBeInTheDocument(),
    );
  ...
  it('PUBLISHED page renders the "Open live" link to /trips; DRAFT does not', async () => {
    ...
    expect(
      screen.queryByRole("link", { name: /Open live/i }),
    ).not.toBeInTheDocument();
    unmount();
  ...
  it("selecting a block shows the in-canvas Move Up / Move Down / Trash row controls", async () => {
    ...
    const headingPaletteBtn = screen
      .getAllByText("Heading")[0]
      .closest("button");
    ...
    fetchApiMock.mockImplementation((url, opts) => {
      ...
      if (url.startsWith("/api/landing-pages/") && method === "PUT") {
        return Promise.reject(
          Object.assign(new Error("Bang"), { status: 500 }),
        );
      }
      return Promise.resolve([]);
    });
...
describe("<LandingPageBuilder /> — TMC trip picker combobox", () => {
  const TMC_TRIPS = [
    {
      id: 10,
      tripCode: "TMC-AND-2026-G7",
      destination: "Andaman",
      status: "confirmed",
    },
    {
      id: 11,
      tripCode: "TMC-GOA-2026-G8",
      destination: "Goa",
      status: "in-trip",
    },
  ];

  function tripsEnabledFetch(url, opts) {
    const method = (opts && opts.method) || "GET";
    if (url === "/api/landing-pages/42" && method === "GET")
      return Promise.resolve(samplePageDraft);
    if (url === "/api/lead-routing" && method === "GET")
  ...
  beforeEach(() => {
    ...
    confirmMock.mockReset();
    confirmMock.mockImplementation(() => Promise.resolve(true));
    fetchApiMock.mockImplementation(tripsEnabledFetch);
  });

  it('renders the picker button with "Not linked to a trip" label when no trip is linked', async () => {
    renderBuilder();
    await waitFor(() =>
      expect(screen.getByLabelText("Page title")).toBeInTheDocument(),
    );
  ...
  it("clicking the picker button opens the dropdown with a search input and all trips", async () => {
    ...
    // Dropdown not yet open.
    expect(screen.queryByLabelText(/Search trips/i)).not.toBeInTheDocument();

    await user.click(screen.getByLabelText(/Link landing page to TMC trip/i));

    // Dropdown opens with search input + both trip entries.
    expect(screen.getByLabelText(/Search trips/i)).toBeInTheDocument();
    expect(screen.getByText("TMC-AND-2026-G7")).toBeInTheDocument();
  ...
  it("selecting a trip fires PUT with { tripId } and closes the dropdown", async () => {
    ...
    // Click the trip code span for the Andaman trip.
    await user.click(screen.getByText("TMC-AND-2026-G7"));
    ...
    await waitFor(() => {
      ...
      expect(putCall).toBeTruthy();
      const body = JSON.parse(putCall[1].body);
      expect(body.tripId).toBe(10);
    });

    expect(notifySuccess).toHaveBeenCalledWith(
      expect.stringMatching(/Linked to TMC-AND-2026-G7/i),
    );
  });

  it('selecting "Not linked to a trip" fires PUT with { tripId: null }', async () => {
    const user = userEvent.setup();
    // Start with a page that already has tripId: 10 linked.
    fetchApiMock.mockImplementation((url, opts) => {
      const method = (opts && opts.method) || "GET";
      if (url === "/api/landing-pages/42" && method === "GET")
        return Promise.resolve({ ...samplePageDraft, tripId: 10 });
      if (url === "/api/lead-routing" && method === "GET")
        return Promise.resolve(sampleRules);
      if (url.startsWith("/api/travel/trips") && method === "GET")
        return Promise.resolve({ trips: TMC_TRIPS });
    ...
    // Wait for linkingTripId to sync from page.tripId=10 so the button label shows the
    // trip code, not "Not linked to a trip". If we click before this sync, both the
    // button label and the list item say "Not linked to a trip" and getByText finds
    // duplicates.
    await waitFor(() =>
      expect(
        screen.getByText(/TMC-AND-2026-G7 \(Andaman\)/),
      ).toBeInTheDocument(),
    );

    await user.click(screen.getByLabelText(/Link landing page to TMC trip/i));

    // Scope the click to the open dropdown list so we never accidentally hit the
    // button label (which also contains the trip text while the picker is open).
    const listbox = screen.getByRole("listbox");
    await user.click(within(listbox).getByText(/Not linked to a trip/i));
    ...
    await waitFor(() => {
      ...
      expect(putCall).toBeTruthy();
      expect(JSON.parse(putCall[1].body).tripId).toBeNull();
    });
