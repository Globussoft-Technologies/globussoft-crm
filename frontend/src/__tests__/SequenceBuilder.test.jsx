...
// Flyer pages were removed from the Travel UI. Keep this guard so the
// sequence builder does not re-introduce the flyer-template network call.
describe('<SequenceBuilder /> - flyer pages removed', () => {
  beforeEach(() => {
    fetchApiMock.mockReset();
    fetchApiMock.mockImplementation(defaultFetchMock);
    notifyError.mockReset();
  ...
  it('does not fetch flyer templates on initial mount', async () => {
    renderBuilder();
    await waitFor(() => {
      expect(screen.getByText('Welcome Drip')).toBeInTheDocument();
    });
