...
import {
  ...
  Shield,
  ShieldCheck,
  // #917 slice 5 Ã¢â‚¬â€ CSP Violations admin nav entry icon.
  ShieldAlert,
  ScrollText,
  GitBranch,
  TrendingUp,
  ...
  QrCode,
  Loader2,
  // Wave 11 Agent HH Ã¢â‚¬â€ Inventory backbone admin entries
  Layers,
  Truck,
  ArrowDownToLine,
  Recycle,
  ...
  Activity,
  // #898 Ã¢â‚¬â€ Campaigns sidebar surfacing (Email / SMS / Push)
  Megaphone,
  // Travel CRM vertical (Day 1 scaffolding Ã¢â‚¬â€ Phase 1 per docs/TRAVEL_CRM_PRD.md Ã‚Â§7)
  Compass,
  ClipboardCheck,
  Map as MapIcon,
  Luggage,
  // PRD Ã‚Â§7 Ã¢â‚¬â€ FlightQuoteAgent nav entry icon (Flight quick-quote, the in-CRM
  // fallback for the Chrome flight plugin). Sits in the Quotes cluster.
  Plane,
  Key,
  // Phase 3 Visa Sure scaffolding (cluster B3) Ã¢â‚¬â€ admin-only sidebar group
  Stamp,
  BadgeCheck,
  // Phase 1 TMC curriculum-mappings admin (tick #181) Ã¢â‚¬â€ consumes
  // /api/travel-curriculum CRUD shipped tick #180 (commit 6d5919a8).
  GraduationCap,
  // Per-sub-brand BrandKit admin entry Ã¢â‚¬â€ consumes /api/brand-kits CRUD
  // (backend commit e4783e0).
  Palette,
  // RateHawk hotel-search admin entry Ã¢â‚¬â€ consumes /api/ratehawk (backend
  // commit be67789, tick #103).
  Hotel,
  // Booking.com / Expedia hotel-search admin entry Ã¢â‚¬â€ consumes
  // /api/booking-expedia (backend commit bb33cbe, tick #105). 4th and
  // FINAL cap-consumer UI in the wrapper-route series.
  BedDouble,
  // Arc 2 Travel Gap #907 slice 5/N Ã¢â‚¬â€ SightseeingMaster nav entry icon.
  ...
  Camera,
  // Arc 2 Travel Gap #907 slice 8/N Ã¢â‚¬â€ ItineraryTemplates nav entry icon.
  // Reusable itinerary template scaffolds Ã¢â‚¬â€ placed adjacent to Sightseeing
  // Master because both are #907 admin pages.
  LayoutTemplate,
  // S49 (TRAVEL_BIG_SCOPE_BACKLOG) Ã¢â‚¬â€ QuoteTemplates nav entry icon. Stack
  // of templates motif; sibling to FileText (Quotes). ADMIN+MANAGER gated.
  FileStack,
  // S55 (TRAVEL_BIG_SCOPE_BACKLOG) Ã¢â‚¬â€ CancellationPolicies nav entry icon.
  // Ban-circle motif matches cancellation / refund semantics. ADMIN+MANAGER
  // gated to mirror the backend POST/PATCH RBAC posture.
  Ban,
  // TravelSubBrandSwitcher Ã¢â‚¬â€ caret on the custom (non-native) sub-brand
...
// by the fallback-resolved effective brand for the active sub-brand Ã¢â‚¬â€ never
...
const Sidebar = ({
  ...
  // requiredPermission stacks on top Ã¢â‚¬â€ only hides an entry once permissions
  ...
  // hamburger Ã¢â‚¬â€ that side is handled in Layout.jsx via toggleRef.
  useEffect(() => {
    if (!mobileOpen || !isMobileViewport) return undefined;
    const aside = asideRef.current;
    if (!aside) return undefined;

    // Initial focus Ã¢â‚¬â€ defer one frame so the slide-in transition has started
  ...
  // against the four sidebar count endpoints when the dashboard was idle Ã¢â‚¬â€
  ...
  //   3. useEffect body runs again Ã¢â‚¬â€ fires refreshCounts() AND opens a
  ...
  //
  // Two-part fix:
  //   Ã¢â‚¬Â¢ refreshCounts moves into a ref so the function identity stays
  //     stable across renders (the body still closes over the latest
  //     in-memory state).
  //   Ã¢â‚¬Â¢ useEffect depends only on `user?.id` Ã¢â‚¬â€ a primitive that's stable
  ...
  // Stable wrapper for socket / interval handlers Ã¢â‚¬â€ they call through the
  ...
  // 50+ NavLink re-evaluations. Debounce horizon is 300ms Ã¢â‚¬â€ batches
  ...
  useEffect(() => {
    if (!user) return;
    refreshCounts();
    // 60s safety-net polling Ã¢â‚¬â€ covers cases where the socket can't connect
    // (nginx without /socket.io proxy) or events are missed during reconnects.
    const intervalId = setInterval(refreshCounts, 60000);

    // Live socket bumps Ã¢â‚¬â€ using the same single-namespace io('/') pattern as
    ...
    // Generic invalidation event Ã¢â‚¬â€ any module can emit and we re-fetch.
    ...
    // to force the badge to re-fetch Ã¢â‚¬â€ the existing socket event above only
    ...
    // Depend only on user?.id Ã¢â‚¬â€ a primitive that ONLY changes on real
  ...
  // Disabled to improve sidebar performance Ã¢â‚¬â€ 60s safety interval covers stale reads.
  ...
  useEffect(() => {
    ...
    // match what the chip shows. Multi-brand users are untouched Ã¢â‚¬â€ they
  ...
  // Stable identity for the sub-brand switcher's onChange Ã¢â‚¬â€ without this,
  ...
  //
  // The switcher itself is a custom (non-native) dropdown Ã¢â‚¬â€ see
  // TravelSubBrandSwitcher below Ã¢â‚¬â€ specifically so this async round-trip
    ...
    (next) => {
      ...
      subBrandDebounceRef.current = setTimeout(async () => {
        ...
        try {
          ...
          // unchanged Ã¢â‚¬â€ the controlled switcher snaps back to the prior
          // activeSubBrand.
        }
      }, 250);
    },
  ...
  // logo when one exists, else the tenant-wide default Ã¢â‚¬â€ never both shown
  ...
  // Inline style applied to section labels Ã¢â‚¬â€ for travel, uses the
  ...
  // a DIFFERENT component type and unmounts + remounts the entire subtree Ã¢â‚¬â€
  ...
  // dominant source of sidebar lag. Fix: ref-backed impls + useMemo([], Ã¢â‚¬Â¦)
  // so the component identity React sees is stable, while the closure-
  // captured state (isAdmin, location, hasPermission, Ã¢â‚¬Â¦) stays live.
  ...
  // Section Ã¢â‚¬â€ wraps a group label + its child Links so the label only renders
  ...
  sectionImplRef.current = ({ label, children }) => {
    ...
    const hasVisibleChild = Children.toArray(children).some((child) => {
      ...
      // visible Ã¢â‚¬â€ Section is a label gate, not a child filter.
  ...
  // Accessible pages Ã¢â‚¬â€ fetched from /api/pages/me (the server's
  ...
  //
  // Re-fetch triggers:
  //   Ã¢â‚¬Â¢ on mount (initial state)
  //   Ã¢â‚¬Â¢ when the user's permission list changes (handles same-user perm
  //     edits Ã¢â‚¬â€ usePermissions' module-level cache is invalidated by
  //     RolesAdmin after every PUT, which propagates a new `permissions`
  //     array here and re-fires this effect)
  //   Ã¢â‚¬Â¢ on the `sidebar:pages-changed` window event Ã¢â‚¬â€ cross-component
  ...
  // re-fetching on every render Ã¢â‚¬â€ only when the actual permission set
  ...
  // SSO-authenticated AdsGPT launcher Ã¢â‚¬â€ does the same token + Redis-key
  ...
  // Ref-backed impl + stable useMemo identity Ã¢â‚¬â€ same perf rationale as Link.
  ...
  // SSO-authenticated Callified launcher Ã¢â‚¬â€ generates a signed JWT and opens
  ...
  // T2.1: when the drawer is open at <900px, the sidebar IS a modal dialog Ã¢â‚¬â€
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Wellness sidebar Ã¢â‚¬â€ slim, clinic-focused Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Wellness sidebar Ã¢â‚¬â€ which catalog categories render here and in what
...
const WELLNESS_CATEGORY_ORDER = [
  ...
  // (`inventory`) Ã¢â‚¬â€ split into two sections only for sidebar grouping.
  "Products",
  "Inventory Admin",
  // User holds personal-user surfaces (Notification Settings) Ã¢â‚¬â€ only
...
// Categories rendered without a section header Ã¢â‚¬â€ they appear at the top of
...
function renderWellnessNav({
  ...
  // Sidebar is rendered ENTIRELY from `accessiblePages` Ã¢â‚¬â€ the per-user
  ...
  //
  // No hardcoded adminOnly / managerOnly / wellnessRoles gating here Ã¢â‚¬â€ the
  // backend has already filtered the list by permission. The only role-
  // sensitive UI bits left are:
  //   Ã¢â‚¬Â¢ hideForAdminTier Ã¢â‚¬â€ catalog-level UX flag to hide clinical-day-to-
  //     day surfaces from users who already see the Admin category.
  //   Ã¢â‚¬Â¢ AdsGPT / Callified Ã¢â‚¬â€ external integrations, kept visible only for
  ...
  return (
    <>
      {/* Core + Manager render at the top with no section header Ã¢â‚¬â€ they're
      ...
      {/* External integrations: admin/manager only. Doctors, nurses,
          ...
          (call-centre console) in their day-to-day nav Ã¢â‚¬â€ they land on the
      ...
      {/* Remaining categories Ã¢â‚¬â€ order driven by WELLNESS_CATEGORY_ORDER so
          ...
          manager Ã¢â‚¬â€ they manage their own notification preferences via the
      ...
      {/* Admin Ã¢â‚¬â€ rendered LAST so management surfaces sit at the bottom of
          the sidebar, below day-to-day operational entries (Leads & Revenue,
          Finance, etc.). Catalog-driven entries pulled from /api/pages/me Ã¢â‚¬â€
      ...
      {(() => {
        ...
        // settingsPage is undefined and nothing renders for it Ã¢â‚¬â€ the
        ...
        return (
          ...
          >
            ...
            {/* Settings Ã¢â‚¬â€ pinned LAST in the sidebar per UX requirement.
      ...
      {/* Notification Settings is rendered via the page-catalog "User"
          ...
          for non-admin/non-manager users Ã¢â‚¬â€ the catalog version uses the
...
// it needs its own open/close + keyboard-nav state Ã¢â‚¬â€ hooks require an
...
// same reflow can't cause it to blank Ã¢â‚¬â€ closing the popup on selection is
...
function TravelSubBrandSwitcher({ activeSubBrand, visibleSubBrands, onChange }) {
  ...
  useEffect(() => {
    ...
    // Tab press Ã¢â‚¬â€ matches native <select> "opens focused" behavior.
    ...
    <div ref={containerRef} style={{ position: "relative", flex: 1 }}>
      ...
      {open && (
        <ul
          ...
          // index.css rules (light/dark) Ã¢â‚¬â€ var(--surface-color) is
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Travel sidebar Ã¢â‚¬â€ Day 1 scaffolding Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
//
// Slim placeholder nav for the travel vertical. Phase 1 (docs/TRAVEL_CRM_PRD.md
// Ã‚Â§7) will fill out the full surface: Diagnostics, Itineraries, Trips (per
// sub-brand: TMC trips / RFU pilgrims), Visa Applications, Suppliers,
// Microsites. For Day 1, only Dashboard is wired Ã¢â‚¬â€ everything else is
...
// access to Ã¢â€°Â¥2 sub-brands Ã¢â‚¬â€ a single-sub-brand user has no choice to make.
...
function renderTravelSubBrandHeader({
  ...
  // switch between, so we don't render the dropdown Ã¢â‚¬â€ but we DO surface a
...
function renderTravelNav({
  ...
  // requiredPermission Ã¢â‚¬â€ role-string gates would defeat the
  ...
  //   1. ACCESS Ã¢â‚¬â€ the user must be entitled to that sub-brand. Full-access
  ...
  //   2. SWITCHER Ã¢â‚¬â€ when a specific sub-brand is active, narrow further to
  ...
  // Itineraries, Cost Master, Reports, Ã¢â‚¬Â¦) and always render Ã¢â‚¬â€ their pages
  ...
  // Travel sidebar Ã¢â‚¬â€ fully permission-driven. Every link below carries a
  ...
  // to ANY role makes the POI Approvals link appear for that role Ã¢â‚¬â€
  ...
  //
  // Section dividers are wrapped in <Section label="..."> Ã¢â‚¬â€ the
  ...
  return (
    <>
      ...
      {/* "Leads" lives in the Sales-pipeline section below (next to Contacts +
          Pipeline). The duplicate top-level Leads link was removed Ã¢â‚¬â€ it pointed
      ...
      {/* Brochure Engine Ã¢â‚¬â€ agentic-orchcrm integration. AI orchestration
      ...
      {/* Destination Landing Pages Ã¢â‚¬â€ travel-only surface backed by the
      ...
      {/* Visa Sure sub-brand cluster Ã¢â‚¬â€ inBrand() is a tenant feature
      ...
      {/* Gmail Ã¢â‚¬â€ personal per-user mailbox connection (each staff member links
      ...
      {/* The generic /reports link was removed here Ã¢â‚¬â€ travel uses its own
      ...
      {/* Admin + Platform Ã¢â‚¬â€ both blocks unwrapped from the legacy
      ...
      {/* Notification Settings is a personal end-user surface Ã¢â‚¬â€ hidden
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Generic sidebar (preserved unchanged) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function renderGenericNav({
  ...
  return (
    <>
      {/* Core Ã¢â‚¬â€ Home is the role-aware widget dashboard for non-admins.
      ...
      />
      {/* #474: label was "Calendar" pointing at /calendar-sync Ã¢â‚¬â€ the integration
      ...
      {/* Admin section. Opens for the legacy `ADMIN` role-string AND for any
          custom role granted `roles.read` via RBAC Ã¢â‚¬â€ without the latter,
          ...
          the Roles entry under this divider Ã¢â‚¬â€ which is the correct UX. */}
      {(isAdmin || (permissionsReady && hasPermission("roles", "read"))) && (
        ...
        >
          ...
          <Link
            ...
            adminOnly
          />
          {/* #917 slice 5 Ã¢â‚¬â€ CSP Violations admin (consumes GET /api/csp/violations
          ...
          <Link
            ...
            adminOnly
          />
          {/* S128 Ã¢â‚¬â€ Embed allowlist admin (Tenant.embedAllowlistJson editor).
          ...
          <Link
            ...
            adminOnly
          />
          {/* PRD_STATUS_PAGE Ã¢â‚¬â€ Platform status admin (declare incidents, post updates). */}
          <Link
            to="/admin/status"
            icon={Activity}
            label="Status"
            adminOnly
          />
          {/* PRD Gap Ã‚Â§1.5 / Ã‚Â§1.6 Ã¢â‚¬â€ Commission profiles + revenue goals admin pages. */}
          <Link
      ...
      {/* User Notification Settings Ã¢â‚¬â€ only for regular users, not admin/manager */}
      {!isAdmin && !isManager && (
        <div
          style={{
            paddingTop: "0.75rem",
...
// surface (#4b5563 on #161821 Ã¢â€°Ë† 3.4:1) and the wellness cream surface
// (#5C5046 on #FFFFFF Ã¢â€°Ë† 6.5:1 Ã¢â‚¬â€ passes AA but reads as muted-grey-on-cream
...
const navStyle = {
  ...
  // change Ã¢â‚¬â€ including layout-affecting ones Ã¢â‚¬â€ and fires on every hover state
  ...
  // compositor in "live layer" mode for nav-links Ã¢â‚¬â€ drop it.)
...
// #392: live counter badge Ã¢â‚¬â€ shown on /leads, /tasks, /tickets, /inbox.
const badgeStyle = {
  fontSize: "0.7rem",
  fontWeight: 700,
