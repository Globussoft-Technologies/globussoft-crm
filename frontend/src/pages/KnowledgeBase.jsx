...
export default function KnowledgeBase() {
  ...
  const [selectedStatus, setSelectedStatus] = useState(null); // null | 'published' | 'draft'
  ...
  const filteredArticles = articles.filter((a) => {
    const categoryMatch =
      !selectedCategoryId || a.categoryId === selectedCategoryId;
    const statusMatch =
      !selectedStatus ||
      (selectedStatus === "published" ? a.isPublished : !a.isPublished);
  ...
  const deleteArticle = async (id) => {
    if (!(await notify.confirm("Delete this article? This cannot be undone.")))
      return;
    try {
      await fetchApi(`/api/knowledge-base/articles/${id}`, {
        method: "DELETE",
      });
      loadAll();
    } catch (err) {
  ...
  const deleteCategory = async (id) => {
    if (
      !(await notify.confirm(
        "Delete this category? Articles in it will be uncategorized.",
      ))
    )
      return;
    try {
      await fetchApi(`/api/knowledge-base/categories/${id}`, {
        method: "DELETE",
      });
...
<<<<<<< knowledgeBase/fixes
  ...
  const tenantData = tenant || getTenant();
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text-secondary)", marginTop: "0.25rem" }}>
            ...
            Manage help articles and categories. Published articles are exposed
      ...
      >
        <button
          onClick={() =>
            setSelectedStatus(
              selectedStatus === "published" ? null : "published",
            )
          }
          style={{
            padding: "0.4rem 1rem",
      ...
      >
        ...
        >
          Published articles are automatically accessible to your customers in
          the help portal. Click the <strong>View</strong> button on any
          published article to see it live.
        </p>
      </div>
            ...
            >
              ...
              >
                ...
                >
                  {editing === "new" ? (
                    <>
                      <Plus size={18} color="var(--accent-color)" /> New Article
                    </>
                  ) : (
                    <>
                      <Edit size={18} color="var(--accent-color)" /> Edit
                      Article
                    </>
                  )}
                </h3>
                            ...
                            <td style={{ padding: "0.85rem 0.5rem" }}>
                              ...
                              >
                                ...
                                <button
                                  ...
                                  style={{
                                    background: "none",
                                    border: `1px solid ${a.isPublished ? "rgba(245,158,11,0.4)" : "rgba(16,185,129,0.4)"}`,
                                    color: a.isPublished
                                      ? "#f59e0b"
                                      : "#10b981",
                                    cursor:
                                    ...
                                    padding: "0.35rem 0.65rem",
                                    borderRadius: "6px",
                                    opacity: publishingId === a.id ? 0.6 : 1,
                                  }}
                                >
                                  <Globe size={12} />
                                  {publishingId === a.id
