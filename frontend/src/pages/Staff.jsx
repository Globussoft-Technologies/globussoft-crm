...
function parseSubBrandAccessValue(value) {
  if (value === null || value === undefined || value === "") return [];
  try {
    const parsed = Array.isArray(value) ? value : JSON.parse(value);
    if (!Array.isArray(parsed)) return [];
    return parsed.filter((id) => SUB_BRAND_IDS.includes(id));
  } catch {
    return [];
  }
}

// Travel-only sub-brand access picker for Add/Edit Staff modals.
// Empty selection means full access (all travel sub-brands).
function SubBrandAccessPicker({ value, onChange }) {
  const scope = Array.isArray(value) ? value : [];
  const labels = scope.map((id) => SUB_BRAND_LABEL[id] || id);

  const toggle = (id) => {
    const next = scope.includes(id)
      ? scope.filter((item) => item !== id)
      : [...scope, id];
    onChange(next);
  };
  ...
  return (
    ...
    >
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: "999px",
                border: `1px solid ${active ? "rgba(99, 123, 255, 0.55)" : "var(--border-color)"}`,
                background: active ? "rgba(99, 123, 255, 0.16)" : "transparent",
      ...
      <div>
        Access limited to:{" "}
        {labels.length ? labels.join(", ") : "all sub-brands"}.
      </div>
    </div>
  );
}
    ...
    <div style={{ marginTop: "0.35rem" }}>
      ...
      >
        {value.length === 0
          ...
          : `Must check in within range of: ${locations
...
function displayRole(member) {
  ...
  if (member.wellnessRole) {
    return (
      member.wellnessRole.charAt(0).toUpperCase() + member.wellnessRole.slice(1)
    );
  }
  return member.role;
}
...
function RoleBadge({ member }) {
  ...
  const key = String(
    member.primaryRole?.key || member.role || "",
  ).toUpperCase();
  ...
  return (
    <span
      style={{
        ...
        border: isDarkMode
          ? "1px solid rgba(245,241,232,0.14)"
...
export default function Staff() {
  ...
  const openEdit = (member) => {
    const rbacRoleId = member.primaryRole?.id
      ? String(member.primaryRole.id)
      : "";
    setEditing({
      id: member.id,
      name: member.name || "",
      ...
      commissionProfileId:
        member.commissionProfileId == null
          ? ""
          : String(member.commissionProfileId),
      ...
      subBrandAccess: parseSubBrandAccessValue(member.subBrandAccess),
      // Travel-only: pre-fill the sub-brand picker from the member's current scope.
    ...
    if (isWellness) {
      fetchApi(`/api/wellness/location-assignments/${member.id}`)
        .then((res) => {
          const ids = Array.isArray(res?.locations)
            ? res.locations.map((l) => l.id)
            : [];
          setEditing((prev) =>
            prev && prev.id === member.id
              ? { ...prev, locationIds: ids }
              : prev,
          );
        })
        .catch(() => {
  ...
  const saveLocations = async () => {
    ...
    try {
      ...
      if (!err.status)
        notify.error(err.message || "Failed to update clinic assignments.");
    } finally {
  ...
  const saveEdit = async () => {
    if (!editing) return;
    if (!editing.rbacRoleId) {
      notify.error("Please choose a role.");
      return;
    }
    ...
    const rbacRole = availableRoles.find(
      (r) => String(r.id) === String(editing.rbacRoleId),
    );
      ...
      await fetchApi(`/api/staff/${editing.id}`, {
        ...
        body: JSON.stringify({
          ...
          subBrandAccess: isTravel
            ? editing.subBrandAccess && editing.subBrandAccess.length
              ? editing.subBrandAccess
              : null
            : undefined,
          // PRD Gap ?1.5 ? number or null. '' becomes null (clear assignment).
          commissionProfileId:
            editing.commissionProfileId === ""
              ? null
              : Number(editing.commissionProfileId),
  ...
  const openCreate = () => {
    const defaultTravelSubBrand = isTravel
    ...
    setCreating({
      ...
      wellnessRoleTouched: false,
      subBrandAccess:
        isTravel && defaultTravelSubBrand ? [defaultTravelSubBrand] : [],
    });
  };
  ...
  const saveCreate = async () => {
    ...
    if (!name) {
      notify.error("Please enter the staff member’s name.");
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    const rbacRole = availableRoles.find(
      (r) => String(r.id) === String(creating.rbacRoleId),
    );
    const accessTier = deriveAccessTier(rbacRole);
    const derivedWellnessRole = deriveWellnessRole(rbacRole, wellnessRoleTypes);
    ...
    try {
      const created = await fetchApi("/api/staff", {
        ...
        body: JSON.stringify({
          ...
          // Travel-only: empty array means unrestricted / all sub-brands.
          subBrandAccess: isTravel
            ? creating.subBrandAccess && creating.subBrandAccess.length
              ? creating.subBrandAccess
              : null
            : undefined,
  ...
  const toggleActive = async (member) => {
    ...
    try {
      ...
      notify.success(
        `${target} ${goingInactive ? "deactivated" : "reactivated"}.`,
      );
      loadStaff();
    } catch (err) {
  ...
  const sendResetLink = async () => {
    ...
    const target =
      resettingPassword.name || resettingPassword.email || "this user";
    try {
      await fetchApi(`/api/staff/${resettingPassword.id}/reset-password`, {
        method: "POST",
        body: JSON.stringify({}),
      });
  ...
  const resendInvite = async (member) => {
    ...
    if (
      ...
      return;
    try {
      await fetchApi(`/api/staff/${member.id}/resend-invite`, {
        method: "POST",
        body: JSON.stringify({}),
      });
  ...
  const deriveDefaultWellnessRole = (rbacRoleId) => {
    const rbacRole = availableRoles.find(
      (r) => String(r.id) === String(rbacRoleId),
    );
  ...
  const handleCreateRoleChange = (v) => {
    setCreating((prev) => ({
      ...prev,
      rbacRoleId: v,
      wellnessRole: prev.wellnessRoleTouched
        ? prev.wellnessRole
        : deriveDefaultWellnessRole(v),
    }));
  };
  ...
  const filteredStaff = staff.filter((s) => {
    ...
    if (wellnessRoleFilter && s.wellnessRole !== wellnessRoleFilter)
      return false;
    if (normalizedQuery) {
  ...
  const handleStaffTableScroll = (event) => {
    ...
    if (scrollHeight - scrollTop - clientHeight < 80) {
      setVisibleStaffCount((count) =>
        Math.min(count + STAFF_PAGE_SIZE, filteredStaff.length),
      );
    }
  };
  ...
  const activeFilterCount =
    (statusFilter !== "all" ? 1 : 0) + (wellnessRoleFilter ? 1 : 0);
  const hasActiveFilters =
    Boolean(filter) || activeFilterCount > 0 || Boolean(normalizedQuery);
  const clearFilters = () => {
    setSearchQuery("");
    setFilter(null);
    setStatusFilter("all");
  ...
  return (
    ...
    >
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "999px",
              background:
                filter === "USER" ? "var(--subtle-bg-3)" : "var(--subtle-bg-4)",
              color:
                filter === "USER"
                  ? "var(--text-primary)"
                  : "var(--text-secondary)",
          ...
          <button
            ...
            style={{
              padding: "0.4rem 1rem",
              borderRadius: "999px",
              background: !filter ? "var(--subtle-bg-3)" : "var(--subtle-bg-4)",
                ...
                >
                  ...
                  {!member.available && member.leave && (
                    ...
                    >
                      ...
                      <div>
                        {new Date(member.leave.startDate).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "numeric" },
                        )}
                        {" – "}
                        {new Date(member.leave.endDate).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "numeric" },
                        )}
                      </div>
      ...
      <div
        className="card"
        style={{ padding: "2rem", minWidth: 0, overflow: "hidden" }}
      >
        <div
          style={{
            display: "flex",
                    ...
                    {isWellness && wellnessRoleTypes.length > 0 && (
                      ...
                      >
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setWellnessRoleFilter(e.target.value)
                          }
                  ...
                  >
                    ...
                    >
                      ...
                      {member.deactivatedAt && (
                        <div
                          data-testid="staff-inactive-badge"
                          style={{
                      ...
                      {canManageStaff || canViewPermissions ? (
                        ...
                        >
                          ...
                          {canViewPermissions && (
                            <button
                              onClick={() =>
                                navigate(`/staff/${member.id}/permissions`)
                              }
      ...
      {creating && (
        ...
        >
          ...
          >
            ...
            >
              The new member will be added to your organization and can sign in
              ...
              {isWellness && wellnessRoleTypes.length > 0 && (
                ...
                >
                  ...
                  <span style={{ fontWeight: 400 }}>
                    (controls booking dropdown visibility)
                  </span>
                  <select
                    className="input-field"
                    ...
                    onChange={(e) =>
                      setCreating({
                        ...creating,
                        wellnessRole: e.target.value,
                        wellnessRoleTouched: true,
                      })
                    }
              ...
              {isTravel && (
                ...
                >
                  ...
                  <SubBrandAccessPicker
                    ...
                    onChange={(next) =>
                      setCreating({ ...creating, subBrandAccess: next })
                    }
                  />
                </label>
              )}
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "6px",
                  padding: "0.4rem 0.9rem",
                  cursor: savingCreate ? "wait" : "pointer",
                  fontSize: "0.85rem",
                  fontWeight: 600,
                }}
              >
      ...
      {resettingPassword && (
        ...
        >
          ...
          >
            ...
            >
              Send a password-reset link to {resettingPassword.email}? The link
              is valid for 1 hour.
            </p>
            <label
              style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}
            >
              Manual password{" "}
              <span style={{ fontWeight: 400 }}>
                (optional, set a new password immediately)
              </span>
            ...
            >
              ...
              {isTravel && (
                ...
                >
                  ...
                  <SubBrandAccessPicker
                    ...
                    onChange={(next) =>
                      setEditing({ ...editing, subBrandAccess: next })
                    }
                  />
                </label>
              )}
              ...
              {isWellness && clinicLocations.length > 0 && (
                ...
                >
                  Clinic locations{" "}
                  <span style={{ fontWeight: 400 }}>
                    (geofenced check-in — leave empty for no location
                    restriction)
                  </span>
