...
function dayItemVisual(itemType) {
  return (
    DAY_ITEM_VISUAL[String(itemType || "").toLowerCase()] || {
      Icon: Briefcase,
      color: "#64748b",
    }
  );
...
async function geocodeCity(cityName) {
  ...
  try {
    ...
    /* Nominatim unreachable — silently skip */
  }
  return null;
}
...
export default function Itineraries() {
  ...
  useEffect(() => {
    ...
    if (!selectedItinerary.destination) {
      setMapItems(raw);
      return;
    }
  ...
  const submitSuggest = async (e) => {
    ...
    try {
      ...
      notify.error(friendlyAiError(err?.body?.error || err?.message || ""));
    } finally {
  ...
  const createFromSuggestion = async () => {
    ...
    const effectiveSubBrand =
      materialiseSubBrand || defaultSubBrandFor(user, activeSubBrand);
    ...
    try {
      ...
      const res = await fetchApi("/api/travel/itineraries/from-suggestion", {
        method: "POST",
        body: JSON.stringify(body),
      });
      const itemsCreated =
        res && typeof res.itemsCreated === "number"
          ? res.itemsCreated
      ...
      notify.error(
        err?.body?.error || err?.message || "Failed to materialise itinerary",
      );
    } finally {
  ...
  const submitCreate = async (e) => {
    ...
    try {
      ...
      notify.success(
        selectedTemplateId
          ? "Itinerary created from template"
          : "Itinerary created",
      );
      setCreating(false);
      setPage(1);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={td}>
                        ...
                        >
                          <span
                            className={`travel-itin-status-pill travel-itin-status-pill--${statusVariant}`}
                          >
                            {statusLabel}
                          </span>
                          {!it.cancellationStatus &&
      ...
      {creating && (
        ...
        >
          <form
            ...
            className="card travel-itin-drawer"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        ...
                        {templates
                          ...
                          .map((t) => {
                            ...
                            const price = t.basePriceMinor
                              ? `${t.currency || "INR"} ${(t.basePriceMinor / 100).toLocaleString()}`
                              : null;
                            return (
                              <button
                                key={t.id}
                                ...
                                onClick={() => {
                                  if (isSelected) {
                                    ...
                                    setForm((prev) => ({
                                      ...prev,
                                      destination:
                                        t.destinationName || prev.destination,
                                ...
                                style={{
                                  ...
                                  cursor: "pointer",
                                  border: isSelected
                                    ? "1px solid var(--accent-color, #C89A4E)"
                                    : "1px solid var(--border-light, rgba(255,255,255,0.08))",
                                  background: isSelected
                                    ? "rgba(200,154,78,0.12)"
              ...
              <label style={fieldLabel}>
                ...
                {lockedBrand ? (
                  ...
                  <select
                    value={form.subBrand}
                    onChange={(e) =>
                      setForm({ ...form, subBrand: e.target.value })
                    }
                    style={inputStyle}
                  >
                    {myBrands.map((b) => (
              ...
              >
                ...
                WhatsApp / inbound leads who haven&apos;t taken the diagnostic
                yet. Running the diagnostic first is still recommended for
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span
                  style={{
                    ...
                    flexShrink: 0,
                    background:
                      "linear-gradient(135deg, var(--primary-color, var(--accent-color)) 0%, #6366f1 100%)",
                    color: "#fff",
                ...
                <div>
                  ...
                  >
                    AI builds a day-by-day outline — review it, then create the
                    itinerary.
                  </p>
                </div>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                <label style={fieldLabel}>
                  ...
                  <input
                    ...
                    style={inputStyle}
                    aria-invalid={
                      suggestFieldErrors.durationDays ? "true" : "false"
                    }
                    aria-describedby={
              ...
              <label style={fieldLabel}>
                ...
                <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>
                  ...
                  customer can adjust dates when they accept.
                </span>
              </label>
              <label style={fieldLabel}>
                Interests (optional)
              ...
              <label style={fieldLabel}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSuggestForm({ ...suggestForm, pace: e.target.value })
                  }
                  style={modalSelectStyle}
              ...
              >
                ...
                {suggestionEstTotal(suggestResult.suggestion) > 0 && (
                  ...
                  >
                    Estimated total:{" "}
                    {fmtMoney(
                      suggestionEstTotal(suggestResult.suggestion),
                      "INR",
                    )}{" "}
                    / person
                ...
                {Array.isArray(suggestResult.suggestion.days) &&
                suggestResult.suggestion.days.length > 0 ? (
                  <div
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      ...
                      return (
                        ...
                        >
                          <div style={dayCardHead}>
                            ...
                            {dayEstTotal(day) > 0 && (
                              <span style={dayHeadTotal}>
                                {fmtMoney(dayEstTotal(day), "INR")}
                              </span>
                            )}
                          </div>
                          {Array.isArray(day.items) && day.items.length > 0 && (
                            <div>
                              {day.items.map((item, i) => {
                                const { Icon, color } = dayItemVisual(
                                  item.itemType,
                                );
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                >
                  <strong>{confirmDelete.destination}</strong> will be
...
const dayCardHead = {
  ...
  padding: "10px 12px",
  background:
    "linear-gradient(135deg, rgba(37,99,235,0.12), rgba(99,102,241,0.05))",
};
const dayBadge = {
  display: "inline-flex",
  alignItems: "center",
