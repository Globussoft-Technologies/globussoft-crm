...
const PALETTE_META = {
  ...
  accentHex: {
    label: "Accent",
    hint: "Placeholder fill for empty image / logo blocks",
  },
...
export default function MarketingFlyerStudio() {
  ...
  useEffect(() => {
    stateRef.current = { layout, palette, assets };
  }, [layout, palette, assets]);
  ...
  const snap = useCallback(
    (v) => (snapToGrid ? Math.round(v / GRID) * GRID : Math.round(v)),
    [snapToGrid],
  );
  ...
  const setPaletteKey = useCallback(
    (k, newVal) => {
      ...
      if (
        k !== "bgHex" &&
        oldVal &&
        oldVal.toLowerCase() !== String(newVal).toLowerCase()
      ) {
        setLayout((prev) =>
          prev.map((b) =>
  ...
  const handleSuggestCopy = useCallback(
    async (e) => {
      e?.preventDefault?.();
      ...
      try {
        ...
        const data = await fetchApi(
          "/api/travel/flyer-templates/suggest-copy",
          {
            method: "POST",
        ...
        setLayout((prev) => {
          ...
          const startY = isFresh
            ? 24
            : Math.max(0, ...prev.map((b) => (b.y || 0) + (b.height || 0))) +
              24;
          const newBlocks = [
            {
              type: "text",
  ...
  const handleSuggestImage = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (activeSubBrand) body.subBrand = activeSubBrand;
        const data = await fetchApi(
          "/api/travel/flyer-templates/suggest-image",
          {
            method: "POST",
            body: JSON.stringify(body),
          },
  ...
  const onCanvasMouseMove = useCallback(
    (e) => {
      ...
      if (r) {
        ...
        setLayout((prev) =>
          prev.map((b, i) =>
            i === r.idx ? { ...b, width: nw, height: nh } : b,
          ),
        );
        return;
      }
      ...
      d.moved = true;
      setLayout((prev) =>
        prev.map((b, i) => (i === d.idx ? { ...b, x: nx, y: ny } : b)),
      );
    },
    [snap],
  );
  const onCanvasMouseUp = useCallback(() => {
    ...
    if (resizeRef.current && resizeRef.current.moved)
      pushHistory(resizeRef.current.snapshot);
  ...
  const loadTemplate = useCallback(
    async (templateId) => {
      ...
      try {
        const data = await fetchApi(
          `/api/travel/flyer-templates/${templateId}`,
        );
        const parsedPalette = parseJsonField(
          data?.paletteJson,
          DEFAULT_PALETTE,
        );
        ...
        setAssets(
          parsedAssets && typeof parsedAssets === "object"
            ? parsedAssets
            : DEFAULT_ASSETS,
        );
        setSelectedIdx(null);
    ...
    <div style={pageWrap} data-testid="marketing-flyer-studio">
      <header style={headerWrap}>
        <div>
          ...
          <p style={subtitleStyle}>
            Build branded flyers for TMC / RFU / Travel Stall / Visa Sure. Add
            blocks to the canvas, set the palette, then save as a reusable
            template.
          </p>
        </div>
      ...
      {!loading && loadedTemplate && (
        ...
        >
          ...
          {loadedTemplate.subBrand
            ? ` — ${loadedTemplate.subBrand.toUpperCase()}`
            : ""}
        </div>
      )}
      ...
      <div data-testid="flyer-editor" style={editorWrap}>
        <div style={toolbarStyle}>
          ...
          <button
            ...
            style={toolBtn}
            data-testid="flyer-add-image"
          >
            + Image
          </button>
          <button
            type="button"
            onClick={() => addBlock("price")}
            style={toolBtn}
            data-testid="flyer-add-price"
          >
            + Price
          </button>
          <button
          ...
          <button
            ...
            style={{
              ...
              background: snapToGrid
                ...
                : undefined,
              color: snapToGrid ? "#fff" : undefined,
            }}
        ...
        >
                  ...
                  {SRC_TYPES.has(b.type) ? (
                    b.src ? (
                      ...
                      <span style={{ fontSize: 11, opacity: 0.5 }}>
                        {b.type === "logo" ? "Logo" : "Image"} — set URL →
                      </span>
                    )
                  ) : (
                    b.content || ""
          ...
          <div style={propsPanel} data-testid="flyer-properties">
            {!selected ? (
              ...
              >
                Select a block to edit it — or use <strong>+ Text</strong> /{" "}
                <strong>+ Image</strong> to add one.
              </p>
            ) : (
              <div
              ...
              >
                  ...
                  <>
                    <label style={propLabel}>
                      ...
                      <input
                        ...
                        onFocus={commit}
                        onChange={(e) =>
                          updateBlock(selectedIdx, { src: e.target.value })
                        }
                        style={propInput}
                  ...
                  <>
                    ...
                    {selected.type === "text" ? (
                      <label style={propLabel}>
                        ...
                        <input
                          ...
                          onFocus={commit}
                          onChange={(e) =>
                            updateBlock(selectedIdx, { color: e.target.value })
                          }
                    ...
                    <label style={propLabel}>
                      ...
                      <select
                        ...
                        onFocus={commit}
                        onChange={(e) =>
                          updateBlock(selectedIdx, { font: e.target.value })
                        }
                        style={propInput}
                      >
                        {FONT_OPTIONS.map((o) => (
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        ...
                        onClick={() => {
                          commit();
                          updateBlock(selectedIdx, { bold: !selected.bold });
                        }}
                        style={{
                          ...styleToggleBtn,
                          fontWeight: 700,
                          ...(selected.bold ? styleToggleActive : {}),
                        }}
                      >
                        B
                      </button>
                      <button
                        ...
                        style={{
                          ...styleToggleBtn,
                          fontStyle: "italic",
                          ...(selected.italic ? styleToggleActive : {}),
                        }}
                      >
                        I
                      </button>
                      <button
                        ...
                        style={{
                          ...
                          ...(selected.underline ? styleToggleActive : {}),
                        }}
                      >
                        U
                      </button>
                      ...
                      {ALIGN_OPTIONS.map((a) => (
                        <button
                          ...
                          onClick={() => {
                            commit();
                            updateBlock(selectedIdx, { align: a.value });
                          }}
                          style={{
                            ...styleToggleBtn,
                ...
                <button
                  ...
                  style={deleteBtn}
                  data-testid="flyer-delete-block"
                >
                  Delete block
                </button>
              </div>
      ...
      {showCopyModal && (
        ...
        >
          <form onSubmit={handleSuggestCopy} style={modalDialog}>
            <h2 id="ai-copy-heading" style={modalHeading}>
      ...
      {showImageModal && (
        ...
        >
          <form onSubmit={handleSuggestImage} style={modalDialog}>
            <h2 id="ai-image-heading" style={modalHeading}>
              Generate AI flyer image
            </h2>
            <p style={modalHint}>
