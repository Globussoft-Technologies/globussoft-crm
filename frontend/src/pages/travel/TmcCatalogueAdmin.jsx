...
export default function TmcCatalogueAdmin() {
  ...
  const handleEdit = (row) => {
    ...
    setForm({
      ...
      durationNights:
        row.durationNights != null ? String(row.durationNights) : "",
      ...
      indicativePricePerStudent:
        row.indicativePricePerStudent != null
          ? String(row.indicativePricePerStudent)
          : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        notify.info(
          "Created. Per the human-verify gate, the entry lands in Archived. Use Promote to active when reviewed.",
        );
      }
      resetForm();
      ...
      notify.error(
        err?.body?.error || err?.message || "Failed to save catalogue entry",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (row) => {
    ...
    try {
      await fetchApi(`/api/travel-tmc-catalogue/${row.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        ...
        {canWrite && !showForm && (
          <button
            ...
            onClick={handleAdd}
            style={primaryBtn}
            aria-label="Add catalogue entry"
          >
      ...
      {showForm && (
        ...
        >
          ...
          >
            ...
            <Field label="minGradeBand *">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, minGradeBand: e.target.value })
                }
                placeholder="grade-6"
            ...
            <Field label="maxGradeBand *">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, maxGradeBand: e.target.value })
                }
            ...
            <Field label="boardsSupported (comma-separated)">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, boardsSupportedJson: e.target.value })
                }
            ...
            <Field label="minGroupSize *">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, minGroupSize: e.target.value })
                }
                placeholder="20"
            ...
            <Field label="indicativePricePerStudent">
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    indicativePricePerStudent: e.target.value,
                  })
                }
                placeholder="35000"
          ...
          >
            <Field label="primaryOutcomes (comma-separated)">
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, primaryOutcomesJson: e.target.value })
                }
            ...
            <Field label="skillsDeveloped (comma-separated)">
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, skillsDevelopedJson: e.target.value })
                }
            ...
            <Field label="subjectsTouched (comma-separated)">
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, subjectsTouchedJson: e.target.value })
                }
          ...
          <div style={{ marginTop: 12 }}>
            <Field label="reportSkillBlurb *">
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, reportSkillBlurb: e.target.value })
                }
          ...
          <div style={{ marginTop: 12 }}>
            <Field label="summaryForBrief *">
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, summaryForBrief: e.target.value })
                }
          ...
          {!editingId && (
            ...
            >
              Per PRD §3.2, every catalogue row lands in{" "}
              <strong>Archived</strong>. An ADMIN must Promote-to-active before
              the engine matches against it.
            </p>
          )}
      ...
      {loading ? (
        ...
        <div
          role="alert"
          style={{ ...emptyStyle, color: "var(--danger-color)" }}
        >
          {loadError}
        </div>
      ) : rows.length === 0 ? (
          ...
          {rows.map((row) => (
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "var(--text-primary)" }}>
                Grades <strong>{row.minGradeBand}</strong>–
                <strong>{row.maxGradeBand}</strong>
              ...
              {row.indicativePricePerStudent != null && (
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  ~₹{Number(row.indicativePricePerStudent).toLocaleString()}
                  /student
                </div>
              )}
              <div
              ...
              >
                {tab === STATUS_ARCHIVED && isAdmin && (
                  <button
                    ...
                    style={
                      promotingId === row.id ? primaryBtnDisabled : primaryBtn
                    }
...
function TmcConfigPanel({ notify, isAdmin }) {
  ...
  const saveBookingLink = async (e) => {
    ...
    try {
      ...
      notify.error(
        err?.body?.error || err?.message || "Failed to save booking link",
      );
    } finally {
