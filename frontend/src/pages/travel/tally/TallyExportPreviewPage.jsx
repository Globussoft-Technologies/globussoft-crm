...
export default function TallyExportPreviewPage() {
  ...
  const downloadTripMastersXml = () => {
    ...
    downloadXml(buildTallyMastersXml({ companyName: master.companyName, voucherRows: exportData.voucherRows }), `${exportData.fileName}-masters.xml`);
    setMastersDownloaded(true);
  };
  const downloadTripXml = () => {
    const exportData = buildTripExport();
    if (!exportData || !mastersDownloaded) return;
    downloadXml(buildTallyXml({ companyName: master.companyName, voucherRows: exportData.voucherRows }), `${exportData.fileName}.xml`);
  };
  const downloadTripEducationalXml = () => {
    const exportData = buildTripExport();
    if (!exportData || !mastersDownloaded) return;
    downloadXml(buildTallyXml({ companyName: master.companyName, voucherRows: exportData.voucherRows, educationalMode: true }), `${exportData.fileName}-educational.xml`);
  };
  ...
  return <main style={page}>
    ...
    <section style={card}>
      <div style={header}><div><span style={eyebrow}>Tally Export Preview</span><h1 style={{ margin: "5px 0 0" }}>{summary.label}</h1><p style={muted}>Complete trip accounting details prepared for XML export.</p><small style={muted}>Import order: Masters XML first, then Voucher XML. Use Educational XML only when Tally is unlicensed.</small></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button type="button" onClick={downloadTripMastersXml} style={{ ...downloadButton, background: "#0369a1", borderColor: "#0369a1" }}><Download size={16} /> Download Masters XML</button><button type="button" onClick={downloadTripXml} disabled={!mastersDownloaded} title={mastersDownloaded ? "Download voucher transactions" : "Download and import Masters XML first"} style={{ ...downloadButton, opacity: mastersDownloaded ? 1 : 0.55, cursor: mastersDownloaded ? "pointer" : "not-allowed" }}><Download size={16} /> Download Voucher XML</button><button type="button" onClick={downloadTripEducationalXml} disabled={!mastersDownloaded} title="Uses the first day of each month because Educational Mode rejects normal transaction dates" style={{ ...downloadButton, background: "#b45309", borderColor: "#b45309", opacity: mastersDownloaded ? 1 : 0.55, cursor: mastersDownloaded ? "pointer" : "not-allowed" }}><Download size={16} /> Educational Voucher XML</button></div></div>
