...
export default function BookAppointment() {
  ...
  const initialDate = (urlDate && /^\d{4}-\d{2}-\d{2}$/.test(urlDate))
    ? urlDate
  ...
  useEffect(() => {
    ...
    console.log('[BookAppointment handoff] services loaded:', services.length,
      '— urlServiceId', urlServiceId, '→', match ? `MATCH (${match.name})` : 'NOT FOUND');
  ...
  const selectedService = formData.serviceId
    ...
    : null;
  const serviceBase = selectedService && Number(selectedService.basePrice) > 0
  ...
  const handleBookAppointment = async (e) => {
    ...
    if (paymentChoice === 'now' && !paymentBreakdown) {
      notify.error('Pay now requires a service with a fixed price. Pick "Pay after service" instead.');
      return;
    }
    ...
    if (paymentChoice === 'now') {
      let orderEnvelope;
      try {
        orderEnvelope = await fetchApi('/api/wellness/appointments/book-and-pay', {
          method: 'POST',
          body: JSON.stringify({
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (resp) => {
          try {
            const confirmRes = await fetchApi('/api/wellness/appointments/confirm-payment', {
              method: 'POST',
              body: JSON.stringify({
            ...
            if (confirmRes.success) {
              notify.success(`Payment received. Appointment confirmed (#${confirmRes.visitId}).`);
              resetFormAfterSuccess();
            } else {
              notify.error('Payment captured but confirmation failed — our team will reach out.');
            }
          } catch (err) {
            notify.error(err.message || 'Payment captured but confirmation failed. Keep the payment id: ' + resp.razorpay_payment_id);
          } finally {
            setSubmitting(false);
          }
        },
      ...
      rzp.on('payment.failed', (resp) => {
        ...
        notify.error(err.description || 'Payment failed. Try a different card or method.');
        setSubmitting(false);
      });
      rzp.open();
      return;
          ...
          <form onSubmit={handleBookAppointment} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', cursor: 'pointer' }}>
                ...
                />
                <div>
                  <div style={{ fontWeight: 500, color: 'var(--text-primary)' }}>Pay after service</div>
              ...
              <label style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 500, color: 'var(--text-primary)' }}>
                    Pay now {paymentBreakdown ? `· ₹${paymentBreakdown.total.toLocaleString('en-IN')}` : ''}
                  </div>
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>
                    {paymentBreakdown
                  ...
                  {paymentChoice === 'now' && paymentBreakdown && (
                    <div style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span>₹{paymentBreakdown.base.toLocaleString('en-IN')}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>GST (18%)</span>
                        <span>₹{paymentBreakdown.tax.toLocaleString('en-IN')}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <div style={{
                        ...
                        <span>Total</span>
                        <span>₹{paymentBreakdown.total.toLocaleString('en-IN')}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {(() => {
              ...
              const payNowLabel = paymentBreakdown
                ...
                : 'Confirm Appointment';
              const idleLabel = paymentChoice === 'now' ? payNowLabel : 'Confirm Appointment';
              const busyLabel = paymentChoice === 'now' ? 'Starting payment…' : 'Booking…';
              return (
                <button
                  type="submit"
                  disabled={!canSubmit}
