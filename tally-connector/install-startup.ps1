...
$executable = Join-Path $sourceDirectory "TallyConnector.exe"
...
Copy-Item -LiteralPath $executable -Destination (Join-Path $InstallDirectory "TallyConnector.exe") -Force
Copy-Item -LiteralPath $configuration -Destination (Join-Path $InstallDirectory "config.json") -Force

$installedExecutable = Join-Path $InstallDirectory "TallyConnector.exe"
